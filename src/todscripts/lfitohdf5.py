...
def make_od(freq, od, args):
    ...
    outFile = h5py.File(os.path.join(args.out_dir, 'LFI_' + str(freq) + '_OD' + str(od).zfill(4) + '.h5'), 'w')
    ...
    for pid, index in zip(exFile['AHF_info/PID'], range(len(exFile['AHF_info/PID']))):
        ...
        outFile.create_dataset(prefix + '/vsun', data=[velFile[1].data.xvel[velIndex], velFile[1].data.yvel[velIndex], velFile[1].data.zvel[velIndex]]) 
      
        #TODO: convert this to a faster format, ie. rotate to galactic 
        outFile[prefix + '/vsun'].attrs['info'] = '[x, y, z]'
 
        #sampling frequency
        fsamp = rimo[1].data.field('f_samp')[0]
        ...
        for horn in horns[freq]:
            ...
            for hornType in ['S', 'M']:
                ...
                #make flag data
                outFile.create_dataset(prefix + '/flag', data=fileName[str(horn) + hornType + '/FLAG'][pid_start:pid_end])

                #make pixel number
                outFile.create_dataset(prefix + '/pix', data=hp.pixelfunc.ang2pix(nside, fileName[str(horn) + hornType + '/THETA'][pid_start:pid_end], fileName[str(horn) + hornType + '/PHI'][pid_start:pid_end]))
                #make pol angle
                outFile.create_dataset(prefix + '/psi', data=fileName[str(horn) + hornType + '/PSI'][pid_start:pid_end]) 
                ...
                if args.gains_dir is not None:
                    ...
                    outFile.create_dataset(prefix + '/gain', data=gainFile[1].data.GAIN[np.where(gainFile[1].data.PID == pid)]) 
                    gainFile.close()

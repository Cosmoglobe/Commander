    ...
    for freq in [30, 44, 70]:
        ...
        for horn in horns[freq]:
            for hornType in ['S', 'M']:
                ...
                print(slData[0], horn, hornType)
     
                outFile.create_dataset(prefix + '/beam', data=complex2realAlms(beamData, mmax_b))
                outFile.create_dataset(prefix + '/sl', data=complex2realAlms(slData, mmax_s))
...
def complex2realAlms(data, mmax):
    ...
    for l in range(0, lmax):
        for m in range(0, mmax):
            ...
            outData[outI] = np.sqrt(2) * np.real(data[healpixI])
            if(m is not 0):
                outData[outJ] = np.sqrt(2) * np.imag(data[healpixI])

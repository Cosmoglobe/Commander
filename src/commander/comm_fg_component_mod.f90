module comm_model_mod
  ...
  integer(i4b), parameter :: T = 1
  integer(i4b), parameter :: E = 2
  integer(i4b), parameter :: B = 3
  ...
  integer(i4b), parameter, private :: ngrid = 100
  ...
  type fg_meta_data
     integer(i4b)                                  :: npar, ref_band, fg_id, co_band(10)
  ...
  type comm_diffuse_comp
     logical(lgt)       :: enabled, pol
     character(len=512) :: label, type, class, unit
     integer(i4b)       :: lmax_amp, lmax_ind, nside
     real(dp)           :: nu0
     logical(lgt)       :: sample_cls, apply_jeffreys
     character(len=512) :: cl_type
     real(dp),     allocatable, dimension(:,:)   :: amp
     real(dp),     allocatable, dimension(:,:,:) :: ind_map
     real(dp),     allocatable, dimension(:,:,:) :: ind_alm
     real(dp),     allocatable, dimension(:,:)   :: mask
     real(dp),     allocatable, dimension(:,:)   :: cl
     integer(i4b), allocatable, dimension(:,:)   :: cl_bins
     real(dp),     allocatable, dimension(:)     :: cl_alpha
     integer(i4b), allocatable, dimension(:)     :: cl_pivot
  end type comm_diffuse_comp

  type comm_compact_comp
     logical(lgt)       :: enabled, pol
     character(len=512) :: label, type, class, unit
     real(dp)           :: nu0
     integer(i4b)       :: nsrc
     logical(lgt)       :: apply_jeffreys
     real(dp),       allocatable, dimension(:)     :: amp
     real(dp),       allocatable, dimension(:,:)   :: ind
     type(src_temp), allocatable, dimension(:)     :: T        
  end type comm_compact_comp
  
  integer(i4b)                     :: n_diff_comp, n_src_comp 
  type(comm_diffuse_comp), allocatable, dimension(:) :: comp_diff
  type(comm_compact_comp), allocatable, dimension(:) :: comp_src

contains

  subroutine initialize_signal_mod
  ...
  subroutine init_comm_fg_component_mod(paramfile, comm_chain_in)
    ...
    real(dp)           :: delta
    integer(i4b)       :: i, j, k, m, ierr, ind(1), unit
    character(len=2)   :: i_text, j_text
    logical(lgt)       :: apply_par_rms_smooth, optimize_priors
...
!          stop
  ...
  function get_effective_fg_spectrum(fg_comp, band, fg_params, pixel, pol)
    ...
    real(dp)     :: p_low, p_high, epsilon = 1d-6
    integer(i4b) :: i
    ...
    if (trim(fg_comp%type) == 'CO_multiline') then
       ...
       get_effective_fg_spectrum = compute_CO_multiline_spectrum(band, &
            & fg_comp%co_band(1:fg_comp%npar+1), fg_params) * ant2data(band)
  ...
  function get_effective_deriv_fg_spectrum(fg_comp, band, par_id, fg_params, pixel, pol)
    ...
    real(dp)     :: delta = 1.d-10, S_eff_1, S_eff_2
    real(dp)     :: p(2)
    ...
    if (trim(fg_comp%type) == 'CO_multiline') then
       ...
       if (band == fg_comp%co_band(1)) then
          ...
          get_effective_deriv_fg_spectrum = compute_CO_multiline_spectrum(band, &
               & fg_comp%co_band(1:fg_comp%npar+1), fg_params, deriv=.true.) 
       end if
  ...
  subroutine initialize_index_map(paramfile, rng_handle, fg_param_map)
    ...
    real(dp)           :: index
    integer(i4b)       :: i, j, k, m, n, counter, npix, nmaps, p, s
    character(len=2)   :: i_text, j_text
...
!    write(*,fmt='(2e16.8,2f8.3,e16.8)') nu, nu_ref, beta, C, compute_power_law_spectrum
  ...
  function compute_power_law_break_spectrum(nu, nu_ref, dbeta, beta, nu_break)
    implicit none
...
!    end if
  ...
  function compute_CO_multiline_spectrum(band, co_band, ratios, deriv)
    implicit none
  ...
  function compute_one_comp_dust_spectrum(nu, nu_ref, beta, T_d, nu_flat, frac_flat, rms)
    ...
    real(dp)     :: w, w_tot, betap, Tp, beta_min, beta_max, dbeta
    real(dp)     :: T_min, T_max, dT, beta_rms, T_rms, x

...
contains
  ...
  function sample_ARS(handle, lnL, x_init, prior, status, neval, x_n_out, S_n_out, bounds_ok, x0)
    implicit none
...
99  if (stat /= 0) then
       ...
       if (present(x0)) then
          ! Apply Metropolis-Hastings accept-reject rule for non-log-concave functions
          y_old = lnL(x0)
          h_old = envelope_function(x0, xv, yv, n, stat)
          a = exp(y_new-y_old+min(y_old,h_old)-min(y_new,h))
          if (rand_uni(handle) > exp(a)) sample_ARS = x0  ! Reject proposal
       end if
    end if
...
!    write(*,*) 'x = ', real(x(1:n),sp)
    ...
    if (n < 3) then
       write(*,*) 'ARS_mod -- n must be larger than 2'
       stat = stat+1
       return
       !stop
    end if

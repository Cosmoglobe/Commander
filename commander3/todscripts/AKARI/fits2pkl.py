...
#     for y in range(100):  # LW→75,SW→100 変更点あり
...
# def main():
#     month = '2007_01'
#     directory_path = f'/mn/stornext/d23/cmbco/cg/AKARI/akari_TSD_pkl/lat/all'
...
# # def main():
# #     month = '2006_05'
# #     directory_path = f'/mn/stornext/d23/cmbco/cg/AKARI/akari_TSD_pkl/lon/all'
# #     txt_files = glob.glob(os.path.join(directory_path, 'FIS_SW_*.txt'))  # LW,SWで変更点あり
# #     txt_files = sorted(txt_files)
# #     with Pool(processes=25) as pool:
# #         pool.starmap(process_file, [(file_path, month) for file_path in txt_files])
...
def process_file(file_name):  # 処理を行う関数を定義
    ...
    # test #
    # data = fitsio.FITS("/mn/stornext/d23/cmbco/cg/AKARI/akari_TSD/www.ir.isas.jaxa.jp/~yamamura/DR2/2006_05/FIS_LW_20060521160000_gb.fits.gz")
    # d = data[1].read()
    # status = d['STATUS']
    # pixel = d['PIX_FLAG']

    # # 「あかり」の検出器は積分系で、キャリブレーションランプが光るとresetがかかるまでに飽和してしまう。このランプが光ってからresetがかかるまでのデータをフラグとした。
    # # 銀河面のデータが除去されるくらいで、他はあまり変わらない。
    ...
    # index_reset = np.array([i for i, x in enumerate(reset) if x == 1])
    # index_calalon = np.array([i for i, x in enumerate(calalon) if x == 1])

    # # index_calalon の各要素が index_reset のどこにあるかを検索
    # pos = np.searchsorted(index_reset, index_calalon)

    # # 各 `index_calalon[i]` から `index_reset[pos[i]]` までの範囲を作成
    # valid_mask = pos < len(index_reset)  # pos が有効な範囲に収まっているかチェック
    # ranges = [np.arange(index_calalon[i], index_reset[pos[i]]) for i in range(len(index_calalon)) if valid_mask[i]]

    # # フラットなリストに変換、重複を削除
    # if ranges:  # rangesが空でない場合のみ処理する
    #     index = np.unique(np.concatenate(ranges))
    # else:
    #     index = np.array([])  # 空のnumpy配列を設定
    ...
    #     calalon.append(test)
    # save_to_file(file_path, month, 'calalon_reset', 'status', calalon)

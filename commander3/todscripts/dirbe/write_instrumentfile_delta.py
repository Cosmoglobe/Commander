...
def write_dirbe_instrument_file(output_path: str, version: int) -> None:
    ...
    filename = f"DIRBE_instrument_{version:02}_delta.h5"

    instrument_file = commander_instrument(output_path, filename, version, "w")

    wavelengths, _ = dirbe_utils.get_dirbe_bandpass()
    ...
    for idx, detector in enumerate(dirbe_utils.BANDS):
        ...
        index = np.argmin(np.abs(wavelengths - dirbe_utils.WAVELENGHTS[idx]))
        delta = unit_impulse(wavelengths.shape, index)
        instrument_file.add_bandpass(
            detector_group_name, wavelengths, delta
        )
        ...
        for band in dirbe_utils.DETECTOR_LABELS:
            ...
            instrument_file.add_bandpass(
                band_group_name, wavelengths, delta
            )
            _add_fields(

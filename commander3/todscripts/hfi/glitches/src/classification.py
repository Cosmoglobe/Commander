...
def classify_glitches(glitch_idx, res, seconds):
    ...
    plt.scatter(seconds[glitch_idx], res[glitch_idx], color='red', label='Glitches')
    ...
    for glitch_i in glitch_idx:
        ...
        popt["short"], _ = curve_fit(templates.short_glitch, slowpart, data,
                                  p0=[0.01, 0], bounds=(0, np.inf))
        popt["long"], _ = curve_fit(templates.long_glitch, slowpart, data, p0=[0.01, 0],
                                 bounds=(0, np.inf))
        popt["slow"], _ = curve_fit(templates.slow_glitch, slowpart, data, p0=[0.01, 0],
                                 bounds=(0, np.inf))
        ...
        plt.plot(slowpart + seconds[glitch_i],
                 templates.glitch_model_func(slowpart, *popt[glitch_label],
                                             glitch_type=glitch_label), color='green', alpha=0.5)
        plt.text(seconds[glitch_i], res[glitch_i], glitch_label, fontsize=8, color='red',
                 rotation=45)

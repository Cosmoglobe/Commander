...
res = np.zeros(len(seconds))  # white noise
...
for glitch_i, glitch_idx in enumerate(glitch_indices):
    ...
    glitch_model = templates.glitch_model_func(glitch_time, 1, a0=0, band="143-2a",
...
for i in range(0, len(res), plot_window):
    ...
    plt.scatter(seconds[i:i + plot_window][glt_idx], res[i:i + plot_window][glt_idx], color='red',
                label='Glitches')
    for j, glt in enumerate(glt_idx):
        plt.text(seconds[i:i + plot_window][glt], res[i:i + plot_window][glt], glt_lbl[j],

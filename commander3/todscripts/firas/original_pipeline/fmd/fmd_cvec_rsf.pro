Pro FMD_CVEC_RSF,error
;

;
;  FMD_CVEC_RSF drives the FMD_CVECTOR procedure to compute
;  the RLFA C-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;                        RSF_DVECTOR.ISS, and RSF_EJG.ISS, and where
;                        RSF_CVECTOR_2.ISS will be sent .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CVEC_RSF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CVEC_RSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CVEC_RSF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CVEC_RSF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CVEC_RSF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RSF_DVECTOR.ISS'
RESTORE,'csdr$firas_out:rsf_dvector.iss'
;
...
cvec_rsf = $
 FMD_CVECTOR(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
             ...
             max_frac=max_frac,good_dvec=good_dvec_rsf,$
             ndf_cvec=ndf_cvec_rsf,good_cvec=good_cvec_rsf)
;

; Make RSF_CVECTOR_2.ISS Save Set
; -------------------------------
chanscan = 'RSF'
freq_dependence = 'N'
freq_rsf = f_lo
sname = 'csdr$firas_out:rsf_cvector_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rsf,cvec_rsf,ndf_cvec_rsf,good_cvec_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_CVECTOR_2.ISS" Created.'
PRINT,' '
;
...
n_rsf=0 & b_rsf=0 & l_rsf=0 & d_rsf=0 & dvec_rsf=0 & ndf_dvec_rsf=0
;

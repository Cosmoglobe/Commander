...
;  FMD_CVC_HRES creates and saves the binary records of HRES C-Vector .
;
;-
;______________________________________________________________________________
;
Pro FMD_CVC_HRES,error
;
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_CVC_HRES : Called Incorrectly : FMD_CVC_HRES,error'
 print,' '
 return
endif
;
...
PRINT,' '
nf = N_ELEMENTS(c_hres)
cvc_rec.c_vector(0:nf-1) = FLOAT(c_hres)
...
PRINT,' '
outfile = 'csdr$firas_out:fmd_cvc_hres.pass4'
rec_len = 728            ; fixed length record size in bytes
OPENW,1,outfile, rec_len, /fixed
WRITEU,1,cvc_rec(0)
CLOSE,1
;
fname = STRUPCASE(outtrans(0)) + ':FMD_CVC_HRES.PASS4'

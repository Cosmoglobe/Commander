Pro FMD_COVAR_HRES3,error
;

;
;  FMD_COVAR_HRES3 drives the FMD_COVAR procedure to create IDL
;  save sets of combined HRES "Band_3" covariance matrices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        where output HRES_3_COVAR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_COVAR_HRES3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HRES3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HRES3,error'
 PRINT,' '
...
; Frequency Cut  (Band_3)
; -----------------------
fidx = INDGEN(2+cf/3) + cf/3 + cf/3
...
IF (freq_dependence EQ 'Y') THEN BEGIN
 ...
 IF (cf2 NE N_ELEMENTS(freq_hres)) THEN BEGIN
  PRINT,'FMD_COVAR_HRES3 : C-Matrix Frequencies are NOT Compatible !'
  error = 1
  RETURN
 ENDIF
 ;
...
IF (freq_dependence NE 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid,weight=sky_wgts_ds,mask=sky_mask,$
              ...
              w_frac=frac_wgt,covar=covar_hres_3,ndf_covar=ndf_covar_hres3)
;
...
IF (freq_dependence EQ 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid,weight=sky_wgts_ds,mask=sky_mask,$
              ...
              covar=covar_hres_3,ndf_covar=ndf_covar_hres3)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HRES3 : Error Returned from FMD_COVAR !'
 RETURN
ENDIF
;
...
; --------------------------
chanscan = 'HRES_3'
sname = 'csdr$firas_out:hres_3_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_hres,covar_hres_3,ndf_covar_hres3
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_3_COVAR.ISS" Created.'
PRINT,' '
;

Pro FMD_CHI2_RLSX,error
;

;
;  FMD_CHI2_RLSX drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLSX_DSP.ISS, RLSX_WEIGHTS.ISS,
;                        and RLSX_EJG.ISS, and where output save sets 
;                        RLSX_CVECTOR.ISS, RLSX_SKY_RESID.ISS,
;                        RLSX_SKY_CHI2.ISS, RLSX_CAL_RESID.ISS,
;                        and RLSX_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RLSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RLSX,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RLSX,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLSX_DSP.ISS'
RESTORE,'csdr$firas_out:rlsx_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; ---------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'RLSX_WEIGHTS.ISS'
RESTORE,'csdr$firas_ref:rlsx_weights.iss'
;

; Restore RLSX_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLSX_EJG.ISS'
RESTORE,'csdr$firas_out:rlsx_ejg.iss'
;
...
; Make RLSX_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rlsx_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RLSX_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:rlsx_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rls,cvec_rls,ndf_cvec_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RLSX_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rlsx_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rls,cvec_rls,sky_chi2_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RLSX_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rlsx_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RLSX_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rlsx_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,cal_s0,$
                    cal_wgts_ds,cal_wgts,freq_rls,cvec_rls,cal_chi2_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_CAL_CHI2.ISS" Created.'
PRINT,' '
;

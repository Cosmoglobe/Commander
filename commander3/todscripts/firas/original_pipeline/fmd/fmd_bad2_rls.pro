Pro FMD_Bad2_RLS,badcoadd_rls,badcal_rls,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad2_RLS,badcoadd_rls,badcal_rls,error'
 error = 1
 return
endif
;
...
IF (dummy NE 0.) THEN BEGIN
 PRINT,'FMD_BAD2_RLS : Error in SKY_CHI2 Weights !'
 RETURN
ENDIF
;
...
IF (MAX(badcal_old) GE 0) THEN BEGIN
 ...
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD2_RLS : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;
...
badcoadd_lls=0 & badcoadd_lhs=0 & badcoadd_rsf=0 & badcoadd_lsf=0
badcoadd_llf=0 & badcal_lhf=0 & badcal_rhs=0 & badcal_rhf=0 & badcal_rlf=0 
badcal_llf=0 & badcal_lls=0 & badcal_lhs=0 & badcal_rsf=0 & badcal_lsf=0
;
error = 0
;

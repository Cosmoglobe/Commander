Pro FMD_COVAR_HRES1,error
;

;
;  FMD_COVAR_HRES1 drives the FMD_COVAR procedure to create IDL
;  save sets of combined HRES "Band_1" covariance matrices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        where output HRES_1_COVAR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_COVAR_HRES1,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HRES1 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HRES1,error'
 PRINT,' '
...
IF (freq_dependence EQ 'Y') THEN BEGIN
 ...
 IF (cf2 NE N_ELEMENTS(freq_hres)) THEN BEGIN
  PRINT,'FMD_COVAR_HRES1 : C-Matrix Frequencies are NOT Compatible !'
  error = 1
  RETURN
 ENDIF
 ;
...
IF (freq_dependence NE 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid,weight=sky_wgts_ds,mask=sky_mask,$
              ...
              w_frac=frac_wgt,covar=covar_hres_1,ndf_covar=ndf_covar_hres1)
;
...
IF (freq_dependence EQ 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid,weight=sky_wgts_ds,mask=sky_mask,$
              ...
              covar=covar_hres_1,ndf_covar=ndf_covar_hres1)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HRES1 : Error Returned from FMD_COVAR !'
 RETURN
ENDIF
;
...
; --------------------------
chanscan = 'HRES_1'
sname = 'csdr$firas_out:hres_1_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_hres,covar_hres_1,ndf_covar_hres1
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_1_COVAR.ISS" Created.'
PRINT,' '
;

Pro FMD_COVAR_HR_13,error
;

;
;  FMD_COVAR_HR_13 drives the FMD_COVAR procedure to create an IDL
;  save set of combined HRES Band_1/Band_3 covariance.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where output HRES_13_COVAR.ISS will be sent.
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HR_13 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HR_13,error'
 PRINT,' '
...
nfreq = [N_ELEMENTS(freq_hr_1),N_ELEMENTS(freq_hr_3)]
;
...
error =  FMD_COVAR_OFF(nfreq=nfreq,resid1=resid1,resid2=resid2,$
                       ...
                       covar=covar_hres_13,ndf_covar=ndf_covar_hr_13)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HR_13 : Error Returned from FMD_COVAR_OFF !'
 RETURN
ENDIF
;

; Create HRES_13_COVAR Save Set
; -----------------------------
chanscan = 'HRES'
freq_dependence = 'N'
n_stripes = nst
sname = 'csdr$firas_out:hres_13_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_hr_1,freq_hr_3,covar_hres_13,ndf_covar_hr_13
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_13_COVAR.ISS" Created.'
PRINT,' '
;

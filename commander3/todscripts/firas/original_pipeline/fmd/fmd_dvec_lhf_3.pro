Pro FMD_DVEC_LHF_3,error
;

;
;  FMD_DVEC_LHF_3 drives the FMD_DVECTOR procedure to compute
;  the LHFA Band_3 D-Vector .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHF_3_CVECTOR.ISS and LHF_3_EJG.ISS,
;                        where output LHF_3_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_LHF_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_LHF_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_LHF_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_LHF_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_3_CVECTOR.ISS'
...
; Frequency Cut
; -------------
freq = f_lhf * freq_corr
nf = WHERE((freq GE freq_band(0))and(freq LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(freq_lhf_3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_LHF_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_lhf_3 = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_lhf_3,ndf_dvec=ndf_dvec_lhf_3)
;

; Make LHF_3_DVECTOR.ISS Save Set
; -------------------------------
chanscan = 'LHF_3'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:lhf_3_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_lhf_3,$
                    dvec_lhf_3,good_dvec_lhf_3,ndf_dvec_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_DVECTOR.ISS" Created.'
PRINT,' '
;
...
galcut_lhlf=0 & f_lhlf=0 & d_lhlf=0 & var_lhlf=0 & cal_var_lhlf=0
galcut_lhsf=0 & f_lhsf=0 & d_lhsf=0 & var_lhsf=0 & cal_var_lhsf=0
ejg_lhf_3=0 & c_lhf_3=0 & ndf_ejg_lhf_3=0 & chi2_lhf_3=0
n_lhf=0 & b_lhf=0 & l_lhf=0 & d_lhf=0 & f_hi3=0
cvec_lhf_3=0 & ndf_cvec_lhf_3=0 & galcut_lhf=0 & cal_var_lhf=0
;

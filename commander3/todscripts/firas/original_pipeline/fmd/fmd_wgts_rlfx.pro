Pro FMD_WGTS_RLFX,error
;
;
;  FMD_WGTS_RLFX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of RLLF weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where RLFX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RLF.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_RLFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_RLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RLFX,error'
 PRINT,' '
...
; Make RLFX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'RLF'
sname = 'csdr$firas_ref:rlfx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RLFX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
fsl_idx=0 & n_rlf=0 & b_rlf=0 & l_rlf=0 & d_rlf=0

...
;  FMD_AVC_HIGH creates and saves the binary records of HIGH A-Vector .
;
;-
;______________________________________________________________________________
;
Pro FMD_AVC_HIGH,error
;
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_AVC_HIGH : Called Incorrectly : FMD_AVC_HIGH,error'
 print,' '
 return
endif
;
...
; Restore the saveset
; -------------------
PRINT,'Restoring HIGH_AVECTOR.ISS'
PRINT,' '
RESTORE,'csdr$firas_out:high_avector.iss'
;
...
PRINT,' '
avc_rec.a_vector(0:31) = FLOAT(avec_high(0:31))
;
...
PRINT,' '
outfile = 'csdr$firas_out:fmd_avc_high.pass4'
rec_len = 128            ; fixed length record size in bytes
OPENW,1,outfile, rec_len, /fixed
WRITEU,1,avc_rec(0)
CLOSE,1
;
fname = STRUPCASE(outtrans(0)) + ':FMD_AVC_HIGH.PASS4'

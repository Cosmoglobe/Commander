Pro FMD_EJG_HIGH_3,error
;

;
;  FMD_EJG_HIGH_3 drives the FMD_DESTRIPER procedure to create
;  IDL save sets of combined high channel Band_3 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        HIGH_3_CALSPEC_ZSUB.ISS, and HIGH_3_ZODI_SKYMAP.ISS .
;
...
;  Output  :
;
;    HIGH_3_EJG.ISS, contains destriper qualifiers, weights, and stripe spectra.
;
;    HIGH_3_ERRORS.ISS, contains destriper error matrices.
;
;    HIGH_3_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    HIGH_3_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_HIGH_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_HIGH_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_HIGH_3,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_3 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
;
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_HIGH_3 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
;
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_3 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_3 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'HIGH_3 Offset Destriping -- Pass 4'
PRINT,' '
...
; -------------------------------------------
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'HIGH_3_CALSPEC_ZSUB.ISS'
RESTORE,'csdr$firas_in:high_3_calspec_zsub.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_HIGH_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
nf = WHERE((fz_hi3 GE freq_band(0))and(fz_hi3 LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(fz_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_HIGH_3 : Error in ZODI_SKYMAP Frequency Array !'
 RETURN
ENDIF
;
...
c_high_3 = $
  FMD_DESTRIPER(freq=f_hi3,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_high_3,afp=s_high_3,chi2_chan=chi2_hi3ch, $
                func=func,sky_func=sky_func_high_3,cal_func=cal_func_high_3, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_high_3, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                num_coadd=nc_high,ndf=ndf_ejg_high_3,/pure,/mjy) 
;

IF (MAX(c_high_3) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_high_3,gamma=gamma_high_3,lmat=lmat
;
...
; Make HIGH_3_EJG Save Set
; ------------------------
chan_label = 'LHS_RHS_LHF_RHF'
sname='csdr$firas_out:high_3_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts_ds,f_hi3,ejg_high_3,gamma_high_3, $
 c_high_3,ndf_ejg_high_3,chi2_hi3ch,dihed_cut_min,max_frac, $
 n_stripes,freq_band
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_EJG.ISS" Created.'
;

; Make HIGH_3_ERRORS.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:high_3_errors.iss'
SAVE,filename=sname,chanscan,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi3,c_high_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_ERRORS.ISS" Created.'
;

; Make HIGH_3_SKYMAP.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:high_3_skymap.iss'
freq_high_3 = f_hi3
SAVE,filename=sname,chanscan,stripes_descrip,freq_high_3,l_high,b_high,n_high, $
                    nc_high,c_high_3,s_high_3,z_high_3,chi2_high_3, $
                    cmp_px,pcvr,rect,beta,stripe_contrib
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make HIGH_3_FUNC.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:high_3_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_high_3,$
                    cal_func_high_3,sky_idx,cal_idx,chan_label
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_FUNC.ISS" Created.'
PRINT,' '
;

Pro FMD_EJG_HIGH_4,error
;

;
;  FMD_EJG_HIGH_4 drives the FMD_DESTRIPER procedure to create
;  IDL save sets of combined high channel Band_4 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        HIGH_4_CALSPEC_ZSUB.ISS, and HIGH_4_ZODI_SKYMAP.ISS .
;
...
;  Output  :
;
;    HIGH_4_EJG.ISS, contains destriper qualifiers, weights, and stripe spectra.
;
;    HIGH_4_ERRORS.ISS, contains destriper error matrices.
;
;    HIGH_4_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    HIGH_4_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_HIGH_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_HIGH_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_HIGH_4,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_4 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
;
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_HIGH_4 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
;
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_4 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_HIGH_4 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'HIGH_4 Offset Destriping -- Pass 4'
PRINT,' '
...
; -------------------------------------------
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'HIGH_4_CALSPEC_ZSUB.ISS'
RESTORE,'csdr$firas_in:high_4_calspec_zsub.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_HIGH_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
nf = WHERE((fz_hi4 GE freq_band(0))and(fz_hi4 LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(fz_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_HIGH_4 : Error in ZODI_SKYMAP Frequency Array !'
 RETURN
ENDIF
;
...
c_high_4 = $
  FMD_DESTRIPER(freq=f_hi4,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_high_4,afp=s_high_4,chi2_chan=chi2_hi4ch, $
                func=func,sky_func=sky_func_high_4,cal_func=cal_func_high_4, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_high_4, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                num_coadd=nc_high,ndf=ndf_ejg_high_4,/pure,/mjy) 
;

IF (MAX(c_high_4) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_high_4,gamma=gamma_high_4,lmat=lmat
;
...
; Make HIGH_4_EJG Save Set
; ------------------------
chan_label = 'LHS_RHS_LHF_RHF'
sname='csdr$firas_out:high_4_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts_ds,f_hi4,ejg_high_4,gamma_high_4, $
 c_high_4,ndf_ejg_high_4,chi2_hi4ch,dihed_cut_min,max_frac, $
 n_stripes,freq_band
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_EJG.ISS" Created.'
;

; Make HIGH_4_ERRORS.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:high_4_errors.iss'
SAVE,filename=sname,chanscan,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi4,c_high_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_ERRORS.ISS" Created.'
;

; Make HIGH_4_SKYMAP.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:high_4_skymap.iss'
freq_high_4 = f_hi4
SAVE,filename=sname,chanscan,stripes_descrip,freq_high_4,l_high,b_high,n_high, $
                    nc_high,c_high_4,s_high_4,z_high_4,chi2_high_4, $
                    cmp_px,pcvr,rect,beta,stripe_contrib
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make HIGH_4_FUNC.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:high_4_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_high_4,$
                    cal_func_high_4,sky_idx,cal_idx,chan_label
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_FUNC.ISS" Created.'
PRINT,' '
;

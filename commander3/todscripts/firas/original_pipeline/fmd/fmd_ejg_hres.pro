Pro FMD_EJG_HRES,error
;

;
;  FMD_EJG_HRES drives the FMD_DESTRIPER procedure to create
;  IDL save sets of combined HRES offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_DIRBE_FUNC_HRES.ISS, and FMD_BAD_COADD_HR.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing HRES.ISS, HRES_WEIGHTS.ISS,
;                        and HRES_CALSPEC.ISS .
;
...
;  Output  :
;
;    HRES_EJG.ISS, contains destriper qualifiers, weights, and stripe spectra.
;
;    HRES_ERRORS.ISS, contains destriper error matrices.
;
;    HRES_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    HRES_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_HRES,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_HRES : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_HRES,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_HRES : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
;
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_HRES : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
;
...
; Bad Coadd Indices
; -----------------
RESTORE,'csdr$firas_ref:fmd_bad_coadd_hr.iss'
;
...
IF (n_bol EQ 1) THEN PRINT,'LLF and RLF Bolometer Stripes'
...
PRINT,' '
PRINT,'BADCOADD_LLF =',badcoadd_llf     ; Bad LLF SKY Coadds
PRINT,' '
PRINT,'BADCOADD_RLF =',badcoadd_rlf     ; Bad RLF SKY Coadds
PRINT,' '
PRINT,'BADCAL_LLF =',badcal_llf         ; Bad LLF CAL Coadds
PRINT,' '
PRINT,'BADCAL_RLF =',badcal_rlf         ; Bad RLF CAL Coadds
PRINT,' '
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_HRES : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_HRES : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'HRES Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf NE N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_HRES : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; --------------------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'FMD_DIRBE_FUNC_HRES.ISS'
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_hres
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_hres

n_tot = FMD_PIX_SUM(pix=px(gw),data=sky_wgts_ds(gw))
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_hres
...
;  -------------
c_hres = $
  FMD_DESTRIPER(freq=f_hr,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_hres,afp=s_hres,chi2_chan=chi2_hrch, $
                func=func,sky_func=sky_func_hres,cal_func=cal_func_hres, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_hres, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                num_coadd=nc_hres,ndf=ndf_ejg_hres,/pure,/mjy) 
;

IF (MAX(c_hres) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_hres,gamma=gamma_hres,lmat=lmat
;
...
; Make HRES_EJG Save Set
; ------------------------
chan_label = 'LLF_RLF'
sname='csdr$firas_out:hres_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts_ds,f_hr,ejg_hres,gamma_hres, $
 c_hres,ndf_ejg_hres,chi2_hrch,dihed_cut_min,max_frac, $
 n_stripes,freq_band
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_EJG.ISS" Created.'
;

; Make HRES_ERRORS.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:hres_errors.iss'
SAVE,filename=sname,chanscan,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hr,c_hres
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_ERRORS.ISS" Created.'
;

; Make HRES_SKYMAP.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:hres_skymap.iss'
freq_hres = f_hr
SAVE,filename=sname,chanscan,stripes_descrip,freq_hres,l_hres,b_hres,n_hres, $
                    nc_hres,c_hres,s_hres,chi2_hres,cmp_px,pcvr,rect,beta, $
                    stripe_contrib
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make HRES_FUNC.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:hres_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_hres,$
                    cal_func_hres,sky_idx,cal_idx,chan_label
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_FUNC.ISS" Created.'
PRINT,' '
;
...
freq_hres=0 & scale_factor=0 & g8x=0 & g9x=0 & g10x=0 & f8=0 & f9=0 & f10=0

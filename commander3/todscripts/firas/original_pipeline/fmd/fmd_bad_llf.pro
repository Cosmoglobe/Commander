Pro FMD_Bad_LLF,badcoadd_llf,badcal_llf,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad_LLF,badcoadd_llf,badcal_llf,error'
 error = 1
 return
endif
;
...
RESTORE,'csdr$firas_out:llfx_sky_chi2.iss'
wd0 = TEMPORARY(sky_wgts_ds)
dummy = MAX(ABS(badcoadd_old-WHERE(wd0 EQ 0.)))
IF (dummy NE 0.) THEN BEGIN
 PRINT,'FMD_BAD_LLF : Error in SKY_CHI2 Weights !'
 RETURN
ENDIF
;

RESTORE,'csdr$firas_out:llfx_cal_chi2.iss'
cwd0 = TEMPORARY(cal_wgts_ds)
IF (MAX(badcal_old) GE 0) THEN BEGIN
 dummy = MAX(ABS(cwd0(badcal_old)))
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD_LLF : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;

ct = TOTAL(sky_chi2_llf,1)
cct = TOTAL(cal_chi2_llf,1)
;

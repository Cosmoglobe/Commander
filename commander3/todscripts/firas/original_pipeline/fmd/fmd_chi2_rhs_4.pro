Pro FMD_CHI2_RHS_4,error
;

;
;  FMD_CHI2_RHS_4 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RHSS Band_4 offset destriped data, using frequency-independent
;  coadd weights.
;
;
;  ARGUMENTS (I/O)    :
...
;                        and RHS_4_EJG.ISS, and where output save sets 
;                        RHS_4_CVECTOR.ISS, RHS_4_SKYMAP_2.ISS, RHS_4_SKY_CHI2.ISS
;                        and RHS_4_CAL_CHI2.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RHS_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RHS_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RHS_4,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RHS_4,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RHS_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi4(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_RHS_4 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Make RHS_4_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_4_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rhs_4,sp_rhs_4,cvec_rhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RHS_4_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhs_4_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_4_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rhs_4_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rhs_4,cvec_rhs_4,ndf_cvec_rhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RHS_4_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_4_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rhs_4,cvec_rhs_4,sky_chi2_rhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RHS_4_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhs_4_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_4_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_4_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhs_4,cvec_rhs_4,$
                    cal_chi2_rhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_CAL_CHI2.ISS" Created.'
PRINT,' '
;

Pro FMD_Bad2_LLF,badcoadd_llf,badcal_llf,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad2_LLF,badcoadd_llf,badcal_llf,error'
 error = 1
 return
endif
;
...
nxx = WHERE(cal_idx EQ 0,cxx)
cwd0 = TEMPORARY(cal_wgts_ds(nxx))
IF (MAX(badcal_old) GE 0) THEN BEGIN
 dummy = MAX(ABS(cwd0(badcal_old)))
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD2_LLF : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;

ct = TOTAL(sky_chi2_hres(*,nx),1)
cct = TOTAL(cal_chi2_hres(*,nxx),1)
;
...
IF (cbad GT 0) THEN BEGIN
 ...
 badcoadd_llf = bad_new(SORT(bad_new))
 nq = WHERE(badcoadd_llf GE 0,cq)
 IF (cq LE 0) THEN badcoadd_llf = [-1]
 IF (cq GT 0) THEN badcoadd_llf = badcoadd_llf(nq)
ENDIF
;

bad = WHERE (cctg GT ctmax,cbad)
;
IF (cbad GT 0) THEN BEGIN
 bad_new = [badcal_old,goodc(bad)]
 badcal_llf = bad_new(SORT(bad_new))
 nq = WHERE(badcal_llf GE 0,cq)
 IF (cq LE 0) THEN badcal_llf = [-1]
 IF (cq GT 0) THEN badcal_llf = badcal_llf(nq)
ENDIF
;

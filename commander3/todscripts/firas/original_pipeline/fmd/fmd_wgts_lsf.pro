Pro FMD_WGTS_LSF,model_var,error
;
;
;  FMD_WGTS_LSF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of LLFA weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and FMD_BAD_COADD_LO.ISS, and where
;                        LSF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LSF.ISS and LSF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where LSF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_LSF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_LSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LSF,model_var,error'
 PRINT,' '
...
; -----------------------------
nr = [4,100]  ;  Range of "Good" Coadd Sizes
;
gpow = [1.,2.]   ;  Desired Powers of Glitch_Rate Functions,
                 ;  (If LE 0, not enabled) .
;
...
nscan = 5     ;  Desired Number of Scan-Angle COS,SIN Functions,
              ;  (If LE 0, not enabled) .
;
delt = 5.     ;  Time Range (days) for Short Timescale Variance Model,
              ;  (IF LE 0., no short timescale model is applied) .
;
lp_order=[1,2,3,4,5,6,7]   ;  Order of Legendre Polynomials of Time
               ;  (IF LE 0, no Legendre Terms )
;
tm_up = [200.]   ;  Start and Stop Times of Special Time Periods
tm_dn = [225.]   ;  (IF LE 0, disabled)
;
glmax = 2.4     ;  Maximum Glitch_Rate for "Good" Coadds
;
glcut = 9999.  ;  Minimum Glitch_Rate for Special High_Glitch Terms
;
tmcut = [164.,170.]  ;  Time Range for Special High_Glitch Terms
;
...
PRINT,' '
PRINT,'BADCOADD_LSF =',badcoadd_lsf     ; Bad LSF SKY Coadds
PRINT,' '
PRINT,'BADCAL_LSF =',badcal_lsf         ; Bad LSF CAL Coadds
PRINT,' '
PRINT,' '
;
...
IF (badcoadd_lsf(0) ge 0) THEN badcoadd(badcoadd_lsf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_lsf(0) ge 0) THEN badcal(badcal_lsf) = 1B
;
...
;
 ...
 ; ---------------------------
 fidx = WHERE((f_lsf GT freq_band(0))and(f_lsf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_LSF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vws = 1. / d_llfs(fidx) / d_llfs(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vwl = 1. / d_llfl(fidx) / d_llfl(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 ;
 vts = ( TRANSPOSE(var_lsf(fidx,*)) # vws ) / TOTAL(vws)           ; SKY
 cal_vts = ( TRANSPOSE(cal_var_lsf(fidx,*)) # vws ) / TOTAL(vws)   ; CAL
 ;
 vtl = ( TRANSPOSE(var_lsf(fidx,*)) # vwl ) / TOTAL(vwl)           ; SKY
 cal_vtl = ( TRANSPOSE(cal_var_lsf(fidx,*)) # vwl ) / TOTAL(vwl)   ; CAL
 ;
 ...
 nl = WHERE(sky_lbl EQ 'LLFL',cl)
 vt(nl) = vtl(nl)
 ;
 ns = WHERE(sky_lbl EQ 'LLFS',cs)
 vt(ns) = vts(ns)
 ;
 nlc = WHERE(cal_lbl EQ 'LLFL',clc)
 cal_vt(nlc) = cal_vtl(nlc)
 ;
 nsc = WHERE(cal_lbl EQ 'LLFS',csc)
 cal_vt(nsc) = cal_vts(nsc)
 ;

 ; "Good" LLFL Coadds
 ; ------------------
 nl = WHERE(sky_lbl EQ 'LLFL',cl)
 goodl = WHERE((vt(nl) GT 0.)and(fsl_sky_wgts(nl) GT 0.)and(vmask(nl) EQ 1),cgoodl)
 IF (cgoodl LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LSF : No Good LLFL Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked LLFL Coadds
 ; --------------------------------------------------
 tml = tm(nl(goodl))
 var2l = vt(nl(goodl)) * nifgs(nl(goodl))
 good2l = WHERE((tml LT tmcut(0))or(tml GT tmcut(1)),cgood2l)
 avar2l = TOTAL(var2l(good2l)) / cgood2l
 ;

 ; "Good" LLFS Coadds
 ; ------------------
 goods = WHERE((vt(ns) GT 0.)and(fsl_sky_wgts(ns) GT 0.)and(vmask(ns) EQ 1),cgoods)
 IF (cgoods LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LSF : No Good LLFS Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked LLFS Coadds
 ; --------------------------------------------------
 tms = tm(ns(goods))
 var2s = vt(ns(goods)) * nifgs(ns(goods))
 good2s = WHERE((tms LT tmcut(0))or(tms GT tmcut(1)),cgood2s)
 avar2s = TOTAL(var2s(good2s)) / cgood2s
 ;
 ...
 dummy = 0.*vt
 dummy(ns(goods)) = var2s /avar2s
 dummy(nl(goodl)) = var2l /avar2l
 ;

 ; Good Coadd Index
 ; ----------------
 goodv = WHERE(dummy GT 0.)
 ;
 ...
 sgoods = STRING(FLOAT(cgood2s))
 sgoodl = STRING(FLOAT(cgood2l))
 sgoods = STRMID(sgoods,STRPOS(sgoods,'.')-6,6)
 sgoodl = STRMID(sgoodl,STRPOS(sgoodl,'.')-6,6)
 ;
 PRINT,'Model Will Be Computed from' + sgoods + ' LLFS Coadds'
 PRINT,'                        and' + sgoodl + ' LLFL Coadds'
 PRINT,' '
 ;
 ...
 ; Normalize by FS/FL Factor
 ; -------------------------
 sky_wgts_mod(ns) = sky_wgts_mod(ns) * SQRT(avar2l/avar2s)
 sky_wgts_mod(nl) = sky_wgts_mod(nl) * SQRT(avar2s/avar2l)
 ;
 cal_wgts_mod(nsc) = cal_wgts_mod(nsc) * SQRT(avar2l/avar2s)
 cal_wgts_mod(nlc) = cal_wgts_mod(nlc) * SQRT(avar2s/avar2l)
 ;

 chanscan = 'LSF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:lsf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_lsf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod,$
                     avar2l,avar2s,sl_weight_rat,sky_lbl,cal_lbl
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'LSF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
 ...
 ; -----------------------------------
 ;
 new_fac = avar2l / avar2s / sl_weight_rat
 ;
 facs = SQRT(new_fac) * TOTAL(sky_wgts_ds(ns)) / TOTAL(fsl_sky_wgts(ns))
 ;
 sky_wgts_ds(ns) = fsl_sky_wgts(ns) * facs
 cal_wgts_ds(nsc) = fsl_cal_wgts(nsc) * facs
 ;
 facl = TOTAL(sky_wgts_ds(nl)) / TOTAL(fsl_sky_wgts(nl)) / SQRT(new_fac)
 ;
 sky_wgts_ds(nl) = fsl_sky_wgts(nl) * facl
 cal_wgts_ds(nlc) = fsl_cal_wgts(nlc) * facl
 ;
...
; Make LSF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LSF'
sname = 'csdr$firas_ref:lsf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LSF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_lsf=0 & n_lsf=0 & b_lsf=0 & l_lsf=0 & f_lsf=0 & galcut_lsf=0
galcut_llfl=0 & galcut_llfs=0 & f_llfl=0 & f_llfs=0 & var_llfl=0 & var_llfs=0
cal_var_llfl=0 & cal_var_llfs=0 & var_tm=0 & cal_var_tm=0 & var_lbl=0
cal_var_lbl=0
;

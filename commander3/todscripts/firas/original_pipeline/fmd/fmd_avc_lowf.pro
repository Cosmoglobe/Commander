...
;  FMD_AVC_LOWF creates and saves the binary records of LOWF A-Vector .
;
;-
;______________________________________________________________________________
;
Pro FMD_AVC_LOWF,error
;
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_AVC_LOWF : Called Incorrectly : FMD_AVC_LOWF,error'
 print,' '
 return
endif
;
...
; Restore the saveset
; -------------------
PRINT,'Restoring LOWF_AVECTOR.ISS'
PRINT,' '
RESTORE,'csdr$firas_out:lowf_avector.iss'
;
...
PRINT,' '
avc_rec.a_vector(0:31) = FLOAT(avec_lowf(0:31))
;
...
PRINT,' '
outfile = 'csdr$firas_out:fmd_avc_lowf.pass4'
rec_len = 128            ; fixed length record size in bytes
OPENW,1,outfile, rec_len, /fixed
WRITEU,1,avc_rec(0)
CLOSE,1
;
fname = STRUPCASE(outtrans(0)) + ':FMD_AVC_LOWF.PASS4'

Pro FMD_DVEC_RLS,error
;

;
;  FMD_DVEC_RLS drives the FMD_DVECTOR procedure to compute
;  the RLSS D-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLS_CVECTOR.ISS and RLS_EJG.ISS,
;                        where output RLS_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_RLS,error
;
;
;    HISTORY : Written by Ken Jensen, Hughes STX, 30-May-97.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
;; BEGIN
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_RLS,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_RLS,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLS_CVECTOR.ISS'
...
; Frequency Cut
; -------------
freq = f_rls * freq_corr
nf = WHERE((freq GE freq_band(0))and(freq LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(freq_rls)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_RLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_rls = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_rls,ndf_dvec=ndf_dvec_rls)
;

; Make RLS_DVECTOR.ISS Save Set
; -----------------------------
chanscan = 'RLS'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:rls_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_rls,$
                    dvec_rls,good_dvec_rls,ndf_dvec_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_rls=0 & b_rls=0 & l_rls=0 & d_rls=0 & f_lo=0
cvec_rls=0 & ndf_cvec_rls=0 & galcut_rls=0 & cal_var_rls=0
;

Pro FMD_WGTS_LHFX,error
;
;
;  FMD_WGTS_LHFX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of LHFA weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where LHFX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LHF.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_LHFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_LHFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LHFX,error'
 PRINT,' '
...
; Make LHFX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'LHF'
sname = 'csdr$firas_ref:lhfx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LHFX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
fsl_idx=0 & n_lhf=0 & b_lhf=0 & l_lhf=0 & d_lhf=0

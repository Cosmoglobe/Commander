Pro FMD_CONCAT_HRES,file_select,error
;

;
;  FMD_CONCAT_HRES reads data for LLLF and RLLF, applies a
...
;                         and where output HRES.ISS and HRES_CALSPEC.ISS
;                         will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CONCAT_HRES,[1,1],error
;
;                    (will read the xxx and xxx_CALSPEC data for LLF and RLF,
;                     obtain a frequency cut from
;                     FMD_QUALS_HR.ISS, apply the frequency cut to the
;                     real part of the calibrated spectra, concatenate the
;                     real part of the arrays, and save the concatenated 
;                     arrays in HRES.ISS and HRES_CALSPEC.ISS.)
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_CONCAT_HRES : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CONCAT_HRES,file_select,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CONCAT_HRES,[1,1],error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (TOTAL(file_select(0:1)) LE 0.) THEN BEGIN
 PRINT,'FMD_CONCAT_HRES : FILE_SELECT Error !'
 RETURN
ENDIF
;
...
; IF (FILE_SELECT(0)) THEN Make HRES.ISS
; --------------------------------------
...
;
 ...
 cal_lbl = [lbl0,TEMPORARY(cal_lbl)]
 ;
 chan_label = 'LLF_RLF'
 sname = 'csdr$firas_in:hres.iss'
 ...
 PRINT,' '
 PRINT,'IDL Save Set "' + intrans(0) + 'HRES.ISS" Created.'
 PRINT,' '
;
ENDIF
;
...
PRINT,' '
;
f_hr = f * freq_corr  ; Frequency Correction
...
;
f_hr = f * freq_corr  ; Frequency Correction
nf = WHERE((f_hr GE freq_band(0))and(f_hr LE freq_band(1)),cf2)
IF (cf2 NE cf) THEN BEGIN
 PRINT,''
 PRINT,'LLF/RLF Frequency Arrays are Not Compatible !'
 PRINT,''
 RETURN
ENDIF
;
...
sname = 'csdr$firas_in:hres_calspec.iss'
SAVE,filename=sname,f_hr,spec,cal_spec
PRINT,' '
PRINT,'IDL Save Set "' + intrans(0) + 'HRES_CALSPEC.ISS" Created.'
PRINT,' '
;
...
d_llf=0 & n_llf=0 & b_llf=0 & l_llf=0

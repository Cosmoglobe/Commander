Pro FMD_DSP_LLFX,error
;

;
;  FMD_DSP_LLFX drives the FMD_APPLY procedure to apply destriper
;  corrections to LLLF data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLFX_EJG.ISS and LLF_FUNC.ISS,
;                        and where LLFX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LLFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LLFX,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LLFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'LLF'
sname = 'csdr$firas_out:llfx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hr,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLFX_DSP.ISS" Created.'
PRINT,' '
;

Pro FMD_WGTS_RHSX,error
;
;
;  FMD_WGTS_RHSX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of RHSS weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where RHSX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RHS.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_RHSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_RHSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RHSX,error'
 PRINT,' '
...
; Make RHSX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'RHS'
sname = 'csdr$firas_ref:rhsx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RHSX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
fsl_idx=0 & n_rhs=0 & b_rhs=0 & l_rhs=0 & d_rhs=0

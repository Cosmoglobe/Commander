Pro FMD_EJG_LHS_4,error
;

;
;  FMD_EJG_LHS_4 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of LHSS Band_4 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Output  :
;
;    LHS_4_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    LHS_4_ERRORS.ISS, contains destriper error matrices.
;
;    LHS_4_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LHS_4_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LHS_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LHS_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LHS_4,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LHS_4 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LHS_4 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LHS_4 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LHS_4 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LHS_4 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LHS_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_LHS_4 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
c_lhs_4 = $
  FMD_DESTRIPER(freq=f_hi4,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_lhs_4,afp=s_lhs_4, $
                func=func,sky_func=sky_func_lhs_4,cal_func=cal_func_lhs_4, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_lhs_4, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_lhs_4,/pure,/mjy) 
;

IF (MAX(c_lhs_4) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_lhs_4,gamma=gamma_lhs_4,lmat=lmat
;
...
; Make LHS_4_EJG Save Set
; -----------------------
chan_label = 'LHS_4'
sname='csdr$firas_out:lhs_4_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi4,ejg_lhs_4, $
 c_lhs_4,ndf_ejg_lhs_4,chi2_lhs_4,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_4_EJG.ISS" Created.'
;

; Make LHS_4_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhs_4_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi4,c_lhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_4_ERRORS.ISS" Created.'
;

; Make LHS_4_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lhs_4_skymap.iss'
freq_lhs_4 = f_hi4
SAVE,filename=sname,chan_label,stripes_descrip,freq_lhs_4,l_lhs,b_lhs,n_lhs, $
                    c_lhs_4,s_lhs_4,z_lhs_4,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_4_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LHS_4_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:lhs_4_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_lhs_4,cal_func_lhs_4, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_4_FUNC.ISS" Created.'
PRINT,' '
;

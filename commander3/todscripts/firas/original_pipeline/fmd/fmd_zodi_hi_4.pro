Pro FMD_ZODI_HI_4,chanscan_array,error
;

;
;  FMD_ZODI_HI_4 creates an IDL save set of High Channel Band_4 zodi spectra.
;
;
;  ARGUMENTS (I/O)      :
;
...
;                        xxx_4_CALSPEC.ISS, and where output IDL save set
;                        xxx_4_ZODISPEC.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_ZODI_HI_4,chanscan_array,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_ZODI_HI_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_ZODI_HI_4,chanscan_array,error'
 PRINT,' '
...
IF (cgood LE 0) THEN BEGIN
 PRINT,'FMD_ZODI_HI_4 : No CHANSCANs Enabled !'
 RETURN
ENDIF
;
 ...
 IF (chanscan_array(ch) EQ 'Y') THEN BEGIN
  ...
  IF (found EQ 0) THEN BEGIN
   ...
   IF (cf LE 0) THEN BEGIN
    PRINT,''
    PRINT,'FMD_ZODI_HI_4 : Error in Frequency Array !'
    PRINT,''
    RETURN
   ENDIF
   ;
  ...
  ; CHANSCAN Band_4 Zodi Spectra
  ; ----------------------------
  zodi_spec = zspec(*,WHERE(sky_idx EQ ch))
  ;

  ; Make xxx_4_ZODISPEC Save Set
  ; ----------------------------
  chan_label = idd(ch) + '_4'
  sname='csdr$firas_in:' + chan_label + '_zodispec.iss'
  SAVE,filename=sname,f_hi4,zodi_spec
  PRINT,' '
  PRINT,'IDL Save Set "' + intrans(0) + idd(ch)+'_4_ZODISPEC.ISS" Created.'
  PRINT,' '
  ;

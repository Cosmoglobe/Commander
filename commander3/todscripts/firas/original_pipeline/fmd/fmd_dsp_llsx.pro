Pro FMD_DSP_LLSX,error
;

;
;  FMD_DSP_LLSX drives the FMD_APPLY procedure to apply destriper
;  corrections to LLSS data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLSX_EJG.ISS and LLS_FUNC.ISS,
;                        and where LLSX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LLSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LLSX,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLS_FUNC.ISS'
RESTORE,'csdr$firas_out:lls_func.iss'
;
...
; Restore IDL Save Set of LLS Undestriped Spectra
; -------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_lls],[cal_func_lls]]
;
...
dummy = FMD_APPLY(freq=f_lo,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_lls,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'LLS'
sname = 'csdr$firas_out:llsx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_DSP.ISS" Created.'
PRINT,' '
;
...
c_lls=0 & ndf_ejg_lls=0 & chi2_lls=0
;

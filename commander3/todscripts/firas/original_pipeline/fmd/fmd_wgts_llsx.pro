Pro FMD_WGTS_LLSX,error
;
;
;  FMD_WGTS_LLSX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of LLSS weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        LLSX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LLS.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_LLSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_LLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LLSX,error'
 PRINT,' '
...
; Make LLSX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'LLS'
sname = 'csdr$firas_ref:llsx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LLSX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
st_sub=0 & fsl_idx=0 & n_lls=0 & b_lls=0 & l_lls=0 & d_lls=0

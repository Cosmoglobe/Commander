Pro FMD_DSP_RHS_3X,error
;

;
;  FMD_DSP_RHS_3X drives the FMD_APPLY procedure to apply destriper
;  corrections to RHSS Band_3 data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHS_3X_EJG.ISS and RHS_3_FUNC.ISS,
;                        and where RHS_3X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RHS_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RHS_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RHS_3X,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_3_FUNC.ISS'
RESTORE,'csdr$firas_out:rhs_3_func.iss'
;
...
; Restore IDL Save Set of RHS Undestriped Spectra
; -------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RHS_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_rhs_3],[cal_func_rhs_3]]
;
...
dummy = FMD_APPLY(freq=f_hi3,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_rhs_3,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'RHS_3'
sname = 'csdr$firas_out:rhs_3x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi3,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_DSP.ISS" Created.'
PRINT,' '
;
...
c_rhs_3=0 & ndf_ejg_rhs_3=0 & chi2_rhs_3=0
;

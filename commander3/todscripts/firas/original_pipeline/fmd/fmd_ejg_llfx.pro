Pro FMD_EJG_LLFX,error
;

;
;  FMD_EJG_LLFX drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_LLF.ISS, 
;                        and LLFX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    LLFX_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LLFX,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LLFX : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LLFX : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LLFX : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LLFX : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LLFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_llf.iss'
;
...
; Make LLFX_EJG Save Set
; ------------------------
chan_label = 'LLF'
sname='csdr$firas_out:llfx_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLFX_EJG.ISS" Created.'
;
...
idd=0 & g8x=0 & g9x=0 & g10x=0 & f8=0 & f9=0 & f10=0 & st_sub=0 & fsl_idx=0
tm_up=0 & tm_dn=0 & gpow=0 & bpow=0 & nscan=0 & delt=0 & tmcut=0 & lp_orderv=0
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_llf=0 & n_llf=0 & b_llf=0 & l_llf=0
badcoadd_rhs=0 & badcoadd_rhf=0 & badcoadd_lhf=0
badcal_rhs=0 & badcal_rhf=0 & badcal_lhf=0
badcoadd_rls=0 & badcoadd_rsf=0 & badcoadd_lhs=0 & badcoadd_lsf=0
badcal_rls=0 & badcal_rsf=0 & badcal_lhs=0 & badcal_lsf=0
badcoadd_lls=0 & badcoadd_rlf=0 & badcal_lls=0 & badcal_rlf=0
;

Pro FMD_DSP_HIGH_3,error
;

;
;  FMD_DSP_HIGH_3 drives the FMD_APPLY procedure to apply destriper
;  corrections to combined high channel Band_3 data.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Required Logicals  :
;
;    CSDR$FIRAS_IN   =  Directory containing HIGH_3_CALSPEC_ZSUB.ISS
;                       and HIGH.ISS .
;
;    CSDR$FIRAS_OUT  =  Directory containing HIGH_3_EJG.ISS and HIGH_3_FUNC.ISS,
;                       and where HIGH_3_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_HIGH_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_HIGH_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_HIGH_3,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'HIGH_3_FUNC.ISS'
RESTORE,'csdr$firas_out:high_3_func.iss'
;
...
; Restore HIGH_3 Undestriped Zodi-Subtracted Spectra
; --------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_HIGH_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_high_3],[cal_func_high_3]]
;
...
dummy = FMD_APPLY(freq=f_hi3,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_high_3,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'HIGH_3'
sname = 'csdr$firas_out:high_3_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi3,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_DSP.ISS" Created.'
PRINT,' '
;
...
gain_convg=0 & gain_iter=0 & chi2_hi3ch=0 & cal_tm=0 & fsl_idx=0
scan=0 & sky_wgts_fsl=0 & cal_wgts_fsl=0 & cvec_mask=0 & cvec_cut=0
c_high_3=0 & ndf_ejf_high_3=0 & chi2_high_3=0
n_high_3=0 & l_high_3=0 & b_high_3=0 & gamma_high_3=0
...
tmin=0 & tmax=0 & good_cal_dihd=0 & ndf_ejg_high_3=0 & dihed_cut_min=0
;

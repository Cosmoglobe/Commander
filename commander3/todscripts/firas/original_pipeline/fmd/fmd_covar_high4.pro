Pro FMD_COVAR_HIGH4,error
;

;
;  FMD_COVAR_HIGH4 drives the FMD_COVAR procedure to create IDL
;  save sets of combined high-channel Band_4 covariance matrices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing HIGH_4_SKY_RESID.ISS, and
;                        where output HIGH_4_COVAR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_COVAR_HIGH4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HIGH4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HIGH4,error'
 PRINT,' '
...
; -----------------------
PRINT,'Restoring HIGH_4_SKY_RESID.ISS'
...
IF (freq_dependence EQ 'Y') THEN BEGIN
 ...
 IF (cf2 NE N_ELEMENTS(freq)) THEN BEGIN
  PRINT,'FMD_COVAR_HIGH4 : C-Matrix Frequencies are NOT Compatible !'
  error = 1
  RETURN
 ENDIF
 ;
...
IF (freq_dependence NE 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid_high4,weight=sky_wgts_ds,mask=sky_mask,$
              cvec_cut=cvec_cut,px=px,n_stripes=nst,max_frac=max_frac,$
              w_frac=frac_wgt,covar=covar_high_4,ndf_covar=ndf_covar_high4)
;
...
IF (freq_dependence EQ 'Y') THEN error = $
    FMD_COVAR(nfreq=nfreq,resid=sky_resid_high4,weight=sky_wgts_ds,mask=sky_mask,$
              ...
              covar=covar_high_4,ndf_covar=ndf_covar_high4)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HIGH4 : Error Returned from FMD_COVAR !'
 RETURN
ENDIF
;
...
; --------------------------
chanscan = 'HIGH_4'
sname = 'csdr$firas_out:high_4_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_high_4,covar_high_4,ndf_covar_high4
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_COVAR.ISS" Created.'
PRINT,' '
;

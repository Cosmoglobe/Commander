Pro FMD_WGTS_LHF,model_var,error
;
;
;  FMD_WGTS_LHF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of LHFA weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where LHF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LHF.ISS and LHF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where LHF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_LHF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_LHF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LHF,model_var,error'
 PRINT,' '
...
;
tm_up = [210.]   ;  Start and Stop Times of Special Time Periods
tm_dn = [235.]   ;  (IF LE 0, disabled)
;
...
;
skut = [44,58,53,66,8] ;  Controls Special Scan-Angle Correction, terms are :
...
IF (badcoadd_lhf(0) ge 0) THEN badcoadd(badcoadd_lhf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_lhf(0) ge 0) THEN badcal(badcal_lhf) = 1B
;
...
;
 ...
 ; ------------------------------------
 PRINT,' '
 PRINT,'Restoring ' + intrans(0) + 'LHF_VAR.ISS'
 RESTORE,'csdr$firas_in:lhf_var.iss'
 ;

 ; Index of "Good" Frequencies
 ; ---------------------------
 fidx = WHERE((f_lhf GT freq_band(0))and(f_lhf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_LHF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vws = 1. / d_lhsf(fidx) / d_lhsf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vwl = 1. / d_lhlf(fidx) / d_lhlf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 ;
 vts = ( TRANSPOSE(var_lhf(fidx,*)) # vws ) / TOTAL(vws)           ; SKY
 cal_vts = ( TRANSPOSE(cal_var_lhf(fidx,*)) # vws ) / TOTAL(vws)   ; CAL
 ;
 vtl = ( TRANSPOSE(var_lhf(fidx,*)) # vwl ) / TOTAL(vwl)           ; SKY
 cal_vtl = ( TRANSPOSE(cal_var_lhf(fidx,*)) # vwl ) / TOTAL(vwl)   ; CAL
 ;
 ...
 nl = WHERE(sky_lbl EQ 'LHLF',cl)
 vt(nl) = vtl(nl)
 ;
 ns = WHERE(sky_lbl EQ 'LHSF',cs)
 vt(ns) = vts(ns)
 ;
 nlc = WHERE(cal_lbl EQ 'LHLF',clc)
 cal_vt(nlc) = cal_vtl(nlc)
 ;
 nsc = WHERE(cal_lbl EQ 'LHSF',csc)
 ...
 IF (cgoodl LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LHF : No Good LHLF Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked LHLF Coadds
 ; --------------------------------------------------
 tml = tm(nl(goodl))
 ...
 IF (cgoods LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LHF : No Good LHSF Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked LHSF Coadds
 ; --------------------------------------------------
 tms = tm(ns(goods))
 ...
 ;
 PRINT,'Model Will Be Computed from' + sgoods + ' LHSF Coadds'
 PRINT,'                        and' + sgoodl + ' LHLF Coadds'
 PRINT,' '
 ;
 ...
 chanscan = 'LHF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:lhf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_lhf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod,$
                     avar2l,avar2s,sl_weight_rat,sky_lbl,cal_lbl
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'LHF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make LHF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LHF'
sname = 'csdr$firas_ref:lhf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LHF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
;
badcoadd_lhs=0 & badcoadd_rhs=0 & badcoadd_rhf=0
badcal_lhs=0 & badcal_rhs=0 & badcal_rhf=0
...
d_lhf=0 & n_lhf=0 & b_lhf=0 & l_lhf=0 & f_lhf=0 & galcut_lhf=0
galcut_lhlf=0 & galcut_lhsf=0 & f_lhlf=0 & f_lhsf=0 & var_lhlf=0 & var_lhsf=0
cal_var_lhlf=0 & cal_var_lhsf=0 & var_tm=0 & cal_var_tm=0 & var_lbl=0
cal_var_lbl=0
;

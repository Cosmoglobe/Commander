Pro FMD_DSP_LHS_2X,error
;

;
;  FMD_DSP_LHS_2X drives the FMD_APPLY procedure to apply destriper
;  corrections to LHSS Band_2 data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHS_2X_EJG.ISS and LHS_2_FUNC.ISS,
;                        and where LHS_2X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LHS_2X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LHS_2X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LHS_2X,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHS_2_FUNC.ISS'
RESTORE,'csdr$firas_out:lhs_2_func.iss'
;
...
IF (cf ne N_ELEMENTS(f_hi2)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LHS_2X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_lhs_2],[cal_func_lhs_2]]
;
...
dummy = FMD_APPLY(freq=f_hi2,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_lhs_2,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'LHS_2'
sname = 'csdr$firas_out:lhs_2x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi2,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2X_DSP.ISS" Created.'
PRINT,' '
;
...
c_lhs_2=0 & ndf_ejg_lhs_2=0 & chi2_lhs_2=0
;

Pro FMD_WGTS_RHF,model_var,error
;
;
;  FMD_WGTS_RHF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of RHFA weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where RHF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RHF.ISS and RHF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where RHF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_RHF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_RHF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RHF,model_var,error'
 PRINT,' '
...
IF (badcoadd_rhf(0) ge 0) THEN badcoadd(badcoadd_rhf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_rhf(0) ge 0) THEN badcal(badcal_rhf) = 1B
;
...
;
 ...
 ; ---------------------------
 fidx = WHERE((f_rhf GT freq_band(0))and(f_rhf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_RHF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vws = 1. / d_rhsf(fidx) / d_rhsf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vwl = 1. / d_rhlf(fidx) / d_rhlf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 ;
 vts = ( TRANSPOSE(var_rhf(fidx,*)) # vws ) / TOTAL(vws)           ; SKY
 cal_vts = ( TRANSPOSE(cal_var_rhf(fidx,*)) # vws ) / TOTAL(vws)   ; CAL
 ;
 vtl = ( TRANSPOSE(var_rhf(fidx,*)) # vwl ) / TOTAL(vwl)           ; SKY
 cal_vtl = ( TRANSPOSE(cal_var_rhf(fidx,*)) # vwl ) / TOTAL(vwl)   ; CAL
 ;
 ...
 nl = WHERE(sky_lbl EQ 'RHLF',cl)
 vt(nl) = vtl(nl)
 ;
 ns = WHERE(sky_lbl EQ 'RHSF',cs)
 vt(ns) = vts(ns)
 ;
 nlc = WHERE(cal_lbl EQ 'RHLF',clc)
 cal_vt(nlc) = cal_vtl(nlc)
 ;
 nsc = WHERE(cal_lbl EQ 'RHSF',csc)
 ...
 IF (cgoodl LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RHF : No Good RHLF Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked RHLF Coadds
 ; --------------------------------------------------
 tml = tm(nl(goodl))
 ...
 IF (cgoods LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RHF : No Good RHSF Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked RHSF Coadds
 ; --------------------------------------------------
 tms = tm(ns(goods))
 ...
 ;
 PRINT,'Model Will Be Computed from' + sgoods + ' RHSF Coadds'
 PRINT,'                        and' + sgoodl + ' RHLF Coadds'
 PRINT,' '
 ;
 ...
 chanscan = 'RHF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:rhf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_rhf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod,$
                     avar2l,avar2s,sl_weight_rat,sky_lbl,cal_lbl
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'RHF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make RHF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'RHF'
sname = 'csdr$firas_ref:rhf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RHF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
;
badcoadd_lhs=0 & badcoadd_rhs=0 & badcoadd_lhf=0
badcal_lhs=0 & badcal_rhs=0 & badcal_lhf=0
...
d_rhf=0 & n_rhf=0 & b_rhf=0 & l_rhf=0 & f_rhf=0 & galcut_rhf=0
galcut_rhlf=0 & galcut_rhsf=0 & f_rhlf=0 & f_rhsf=0 & var_rhlf=0 & var_rhsf=0
cal_var_rhlf=0 & cal_var_rhsf=0 & var_tm=0 & cal_var_tm=0 & var_lbl=0
cal_var_lbl=0
;

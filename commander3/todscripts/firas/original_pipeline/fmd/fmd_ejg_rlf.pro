Pro FMD_EJG_RLF,error
;

;
;  FMD_EJG_RLF drives the FMD_DESTRIPER procedure to create
;              IDL save sets of RLLF offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_HR.ISS, FMD_DIRBE_FUNC_RLF.ISS,
;                        and RLF_WEIGHTS.ISS .
;
...
;  Output  :
;
;    RLF_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    RLF_ERRORS.ISS, contains destriper error matrices.
;
;    RLF_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RLF_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RLF,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RLF : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RLF : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RLF : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RLF : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RLF Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_rlf.iss'
;
...
IF (badcoadd_rlf(0) ge 0) THEN badcoadd(badcoadd_rlf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_rlf(0) ge 0) THEN badcal(badcal_rlf) = 1B
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_rlf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_rlf

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_rlf
...
c_rlf = $
  FMD_DESTRIPER(freq=f_hr,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rlf,afp=s_rlf, $
                func=func,sky_func=sky_func_rlf,cal_func=cal_func_rlf, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rlf, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rlf,/pure,/mjy) 
;

IF (MAX(c_rlf) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rlf,gamma=gamma_rlf,lmat=lmat
;
...
; Make RLF_EJG Save Set
; -----------------------
chan_label = 'RLF'
sname='csdr$firas_out:rlf_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hr,ejg_rlf, $
 c_rlf,ndf_ejg_rlf,chi2_rlf,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_EJG.ISS" Created.'
;

; Make RLF_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rlf_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hr,c_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_ERRORS.ISS" Created.'
;

; Make RLF_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rlf_skymap.iss'
freq_rlf = f_hr
SAVE,filename=sname,chan_label,stripes_descrip,freq_rlf,l_rlf,b_rlf,n_rlf, $
                    c_rlf,s_rlf,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RLF_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rlf_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rlf,cal_func_rlf, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rlf=0

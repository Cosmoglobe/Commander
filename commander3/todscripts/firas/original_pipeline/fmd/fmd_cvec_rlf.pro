Pro FMD_CVEC_RLF,error
;

;
;  FMD_CVEC_RLF drives the FMD_CVECTOR procedure to compute
;  the RLLF C-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;                        RLF_DVECTOR.ISS, and RLF_EJG.ISS, and where
;                        RLF_CVECTOR_2.ISS will be sent .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CVEC_RLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CVEC_RLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CVEC_RLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CVEC_RLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CVEC_RLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hr(nf)
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLF_DVECTOR.ISS'
RESTORE,'csdr$firas_out:rlf_dvector.iss'
;
...
cvec_rlf = $
 FMD_CVECTOR(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
             ...
             max_frac=max_frac,good_dvec=good_dvec_rlf,$
             ndf_cvec=ndf_cvec_rlf,good_cvec=good_cvec_rlf)
;

; Make RLF_CVECTOR_2.ISS Save Set
; -------------------------------
chanscan = 'RLF'
freq_dependence = 'N'
freq_rlf = f_hr
sname = 'csdr$firas_out:rlf_cvector_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rlf,cvec_rlf,ndf_cvec_rlf,good_cvec_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_CVECTOR_2.ISS" Created.'
PRINT,' '
;
...
n_rlf=0 & b_rlf=0 & l_rlf=0 & d_rlf=0 & dvec_rlf=0 & ndf_dvec_rlf=0
;

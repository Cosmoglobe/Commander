Pro FMD_EJG_LLS,error
;

;
;  FMD_EJG_LLS drives the FMD_DESTRIPER procedure to create
;              IDL save sets of LLSS offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_LO.ISS, FMD_DIRBE_FUNC_LLS.ISS,
;                        and LLS_WEIGHTS.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing LLS.ISS and LLS_CALSPEC.ISS .
;
...
;  Output  :
;
;    LLS_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    LLS_ERRORS.ISS, contains destriper error matrices.
;
;    LLS_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LLS_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LLS,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LLS : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LLS : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LLS : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LLS : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LLS Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_lls.iss'
;
...
IF (badcoadd_lls(0) ge 0) THEN badcoadd(badcoadd_lls) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_lls(0) ge 0) THEN badcal(badcal_lls) = 1B
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_lls
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_lls

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_lls
...
c_lls = $
  FMD_DESTRIPER(freq=f_lo,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_lls,afp=s_lls, $
                func=func,sky_func=sky_func_lls,cal_func=cal_func_lls, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_lls, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_lls,/pure,/mjy) 
;

IF (MAX(c_lls) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_lls,gamma=gamma_lls,lmat=lmat
;
...
; Make LLS_EJG Save Set
; -----------------------
chan_label = 'LLS'
sname='csdr$firas_out:lls_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_lo,ejg_lls, $
 c_lls,ndf_ejg_lls,chi2_lls,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_EJG.ISS" Created.'
;

; Make LLS_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lls_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_lo,c_lls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_ERRORS.ISS" Created.'
;

; Make LLS_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lls_skymap.iss'
freq_lls = f_lo
SAVE,filename=sname,chan_label,stripes_descrip,freq_lls,l_lls,b_lls,n_lls, $
                    c_lls,s_lls,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LLS_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:lls_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_lls,cal_func_lls, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_lls=0
...
badcoadd_rls=0 & badcoadd_rsf=0 & badcoadd_lsf=0
badcal_rls=0 & badcal_rsf=0 & badcal_lsf=0
;

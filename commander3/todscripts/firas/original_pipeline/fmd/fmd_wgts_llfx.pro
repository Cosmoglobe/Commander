Pro FMD_WGTS_LLFX,error
;
;
;  FMD_WGTS_LLFX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of LLFA weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        LLFX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LLF.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_LLFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_LLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LLFX,error'
 PRINT,' '
...
; Make LLFX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'LLF'
sname = 'csdr$firas_ref:llfx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LLFX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
xtm=0 & cmn_tm=0 & step_up=0 & step_dn=0 & chan_label=0 & f_hr=0 & st_sub=0
...
fsl_idx=0 & n_llf=0 & b_llf=0 & l_llf=0 & d_llf=0
badcoadd_lhs=0 & badcoadd_lhf=0 & badcoadd_rhs=0 & badcoadd_rhf=0
badcal_lhs=0 & badcal_lhf=0 & badcal_rhs=0 & badcal_rhf=0
badcoadd_rls=0 & badcoadd_lsf=0 & badcoadd_rsf=0
badcal_rls=0 & badcal_lsf=0 & badcal_rsf=0

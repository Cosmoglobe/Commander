Pro FMD_DVEC_RLF,error
;

;
;  FMD_DVEC_RLF drives the FMD_DVECTOR procedure to compute
;  the RLLF D-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLF_CVECTOR.ISS and RLF_EJG.ISS,
;                        where output RLF_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_RLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_RLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_RLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_RLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLF_CVECTOR.ISS'
...
; Frequency Cut
; -------------
freq = f_rlf * freq_corr
nf = WHERE((freq GE freq_band(0))and(freq LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(freq_rlf)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_RLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_rlf = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_rlf,ndf_dvec=ndf_dvec_rlf)
;

; Make RLF_DVECTOR.ISS Save Set
; -----------------------------
chanscan = 'RLF'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:rlf_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_rlf,$
                    dvec_rlf,good_dvec_rlf,ndf_dvec_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_rlf=0 & b_rlf=0 & l_rlf=0 & d_rlf=0 & f_hr=0
cvec_rlf=0 & ndf_cvec_rlf=0 & galcut_rlf=0 & cal_var_rlf=0
;

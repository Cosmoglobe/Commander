Pro FMD_DVEC_LLF,error
;

;
;  FMD_DVEC_LLF drives the FMD_DVECTOR procedure to compute
;  the LLLF D-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLF_CVECTOR.ISS and LLF_EJG.ISS,
;                        where output LLF_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_LLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_LLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_LLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLF_CVECTOR.ISS'
...
; Frequency Cut
; -------------
freq = f_llf * freq_corr
nf = WHERE((freq GE freq_band(0))and(freq LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(freq_llf)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_LLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_llf = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_llf,ndf_dvec=ndf_dvec_llf)
;

; Make LLF_DVECTOR.ISS Save Set
; -----------------------------
chanscan = 'LLF'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:llf_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_llf,$
                    dvec_llf,good_dvec_llf,ndf_dvec_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_DVECTOR.ISS" Created.'
PRINT,' '
;
...
badcoadd_rlf=0 & badcoadd_llf=0 & badcal_llf=0 & badcal_rlf=0
ejg_llf=0 & c_llf=0 & ndf_ejg_llf=0 & chi2_llf=0
n_llf=0 & b_llf=0 & l_llf=0 & d_llf=0 & f_hr=0
cvec_llf=0 & ndf_cvec_llf=0 & galcut_llf=0 & cal_var_llf=0
;

Pro FMD_EJG_RSFX,error
;

;
;  FMD_EJG_RSFX drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_RSF.ISS, 
;                        and RSFX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    RSFX_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
...
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RSFX,error
;
;
;    HISTORY : Written by Ken Jensen, Hughes STX, 20-Jun-1997.
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
;; BEGIN
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RSFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RSFX,error'
 PRINT,' '
...
; ---------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'RSFX_WEIGHTS.ISS'
RESTORE,'csdr$firas_ref:rsfx_weights.iss'
;
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RSFX : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RSFX : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RSFX : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RSFX : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RSFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_rsf.iss'
;
...
; Make RSFX_EJG Save Set
; ------------------------
chan_label = 'RSF'
sname='csdr$firas_out:rsfx_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_EJG.ISS" Created.'
;
...
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rsf=0 & n_rsf=0 & b_rsf=0 & l_rsf=0
badcoadd_rhs=0 & badcoadd_rhf=0 & badcoadd_lhf=0
badcal_rhs=0 & badcal_rhf=0 & badcal_lhf=0
badcoadd_rls=0 & badcoadd_lsf=0 & badcoadd_lhs=0 & badcoadd_lls=0
badcal_rls=0 & badcal_lsf=0 & badcal_lhs=0 & badcal_lls=0

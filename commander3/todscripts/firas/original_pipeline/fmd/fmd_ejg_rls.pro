Pro FMD_EJG_RLS,error
;

;
;  FMD_EJG_RLS drives the FMD_DESTRIPER procedure to create
;              IDL save sets of RLSS offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_LO.ISS, FMD_DIRBE_FUNC_RLS.ISS,
;                        and RLS_WEIGHTS.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing RLS.ISS and RLS_CALSPEC.ISS .
;
...
;  Output  :
;
;    RLS_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    RLS_ERRORS.ISS, contains destriper error matrices.
;
;    RLS_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RLS_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RLS,error'
 PRINT,' '
...
; ---------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'RLS_WEIGHTS.ISS'
RESTORE,'csdr$firas_ref:rls_weights.iss'
;
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RLS : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RLS : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RLS : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RLS : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RLS Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_rls.iss'
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_rls
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_rls

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_rls
...
c_rls = $
  FMD_DESTRIPER(freq=f_lo,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rls,afp=s_rls, $
                func=func,sky_func=sky_func_rls,cal_func=cal_func_rls, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rls, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rls,/pure,/mjy) 
;

IF (MAX(c_rls) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rls,gamma=gamma_rls,lmat=lmat
;
...
; Make RLS_EJG Save Set
; -----------------------
chan_label = 'RLS'
sname='csdr$firas_out:rls_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_lo,ejg_rls, $
 c_rls,ndf_ejg_rls,chi2_rls,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_EJG.ISS" Created.'
;

; Make RLS_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rls_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_lo,c_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_ERRORS.ISS" Created.'
;

; Make RLS_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rls_skymap.iss'
freq_rls = f_lo
SAVE,filename=sname,chan_label,stripes_descrip,freq_rls,l_rls,b_rls,n_rls, $
                    c_rls,s_rls,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RLS_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rls_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rls,cal_func_rls, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rls=0
...
badcoadd_lls=0 & badcoadd_rsf=0 & badcoadd_lsf=0
badcal_lls=0 & badcal_rsf=0 & badcal_lsf=0
;

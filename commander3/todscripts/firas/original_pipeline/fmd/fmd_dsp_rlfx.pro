Pro FMD_DSP_RLFX,error
;

;
;  FMD_DSP_RLFX drives the FMD_APPLY procedure to apply destriper
;  corrections to RLLF data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLFX_EJG.ISS and RLF_FUNC.ISS,
;                        and where RLFX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RLFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RLFX,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RLFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'RLF'
sname = 'csdr$firas_out:rlfx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hr,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLFX_DSP.ISS" Created.'
PRINT,' '
;

Pro FMD_WGTS_LOWF,error
;

;  FMD_WGTS_LOWF drives the FMD_PIXEL_WGT procedure to create
...
;                        CVECTORS_LO.ISS, and xxx_WEIGHTS.ISS.
;
;    CSDR$FIRAS_IN    =  Directory containing LOWF.ISS, and where output
;                        LOWF_WEIGHTS.ISS and LOWF_WEIGHTS_F.ISS
;                        will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_LOWF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_LOWF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LOWF,error'
 PRINT,' '
...
IF (cx GT 0) THEN BEGIN
 PRINT,'FMD_WGTS_LOWF : Error in SKY_IDX !'
 READ,ncont
 RETURN
ENDIF
;
...
IF (cx GT 0) THEN BEGIN
 PRINT,'FMD_WGTS_LOWF : Error in CAL_IDX !'
 READ,ncont
 RETURN
ENDIF
;
...
;
scale_factor = [1., $
                TOTAL(1/cvec_rls(nf)^2) / TOTAL(1/cvec_lls(nf)^2), $
		TOTAL(1/cvec_lsf(nf)^2) / TOTAL(1/cvec_lls(nf)^2), $
		TOTAL(1/cvec_rsf(nf)^2) / TOTAL(1/cvec_lls(nf)^2)]
...
cvecs = [[cvec_lls(nf)],[cvec_rls(nf)],[cvec_lsf(nf)],[cvec_rsf(nf)]]
;
...
; Make LOWF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LOWF'
sname = 'csdr$firas_in:lowf_weights.iss'
SAVE,filename=sname,chanscan,freq_lowf,px,sky_wgts_ds,pixel_wgt,frac_wgt,$
                    cal_wgts_ds,sky_idx,cal_idx,scale_factor
PRINT,' '
PRINT,'IDL Save Set "' + intrans(0) + 'LOWF_WEIGHTS.ISS" Created.'
PRINT,' '
;

; Make LOWF_WEIGHTS_F.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_in:lowf_weights_f.iss'
SAVE,filename=sname,chanscan,px,frac_fwgt,scale_factor
PRINT,' '
PRINT,'IDL Save Set "'+ intrans(0) + 'LOWF_WEIGHTS_F.ISS" Created.'
PRINT,' '
;
...
st_sub=0 & cal_glitch=0 & cal_wgts_fsl=0 & cal_nifgs=0
...
badcoadd_rls=0 & badcoadd_lls=0 & badcoadd_rsf=0 & badcoadd_lsf=0
badcal_rls=0 & badcal_lls=0 & badcal_rsf=0 & badcal_lsf=0
;

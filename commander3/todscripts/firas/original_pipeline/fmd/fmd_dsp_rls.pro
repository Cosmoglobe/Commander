Pro FMD_DSP_RLS,error
;

;
;  FMD_DSP_RLS drives the FMD_APPLY procedure to apply destriper
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLS_EJG.ISS and RLS_FUNC.ISS,
;                        and where RLS_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RLS,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'RLS'
sname = 'csdr$firas_out:rls_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_DSP.ISS" Created.'
PRINT,' '
;

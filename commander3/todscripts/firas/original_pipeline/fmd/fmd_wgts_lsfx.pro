Pro FMD_WGTS_LSFX,error
;
;
;  FMD_WGTS_LSFX drives the FMD_PIXEL_WGT procedures to create IDL save sets
...
;                        LSFX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LSF.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_LSFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_LSFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LSFX,error'
 PRINT,' '
...
; Make LSFX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'LSF'
sname = 'csdr$firas_ref:lsfx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LSFX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
st_sub=0 & fsl_idx=0 & n_lsf=0 & b_lsf=0 & l_lsf=0 & d_lsf=0
badcoadd_lhs=0 & badcoadd_lhf=0 & badcoadd_rhs=0 & badcoadd_rhf=0
badcal_lhs=0 & badcal_lhf=0 & badcal_rhs=0 & badcal_rhf=0
badcoadd_rls=0 & badcoadd_lls=0 & badcoadd_rsf=0
badcal_rls=0 & badcal_lls=0 & badcal_rsf=0

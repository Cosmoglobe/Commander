Pro FMD_EJG_RHF_4,error
;

;
;  FMD_EJG_RHF_4 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of RHFA Band_4 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_HI.ISS, FMD_DIRBE_FUNC_RHF.ISS,
;                        and RHF_WEIGHTS.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing RHF.ISS, RHF_CALSPEC.ISS,
;                        and RHF_4_ZODISPEC.ISS .
;
...
;  Output  :
;
;    RHF_4_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    RHF_4_ERRORS.ISS, contains destriper error matrices.
;
;    RHF_4_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RHF_4_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RHF_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RHF_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RHF_4,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RHF_4 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RHF_4 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RHF_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; Restore RHF_4 ZODI Spectra
; --------------------------
RESTORE,'csdr$firas_in:rhf_4_zodispec.iss'
nq = WHERE(f_hi4 NE fx,cq)
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_RHF_4 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_rhf.iss'
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_rhf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_rhf

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_rhf
...
;
PIX2XY,cmp_px,data=dummy,res=6,/six,ras=z_rhf_4
;
...
; -------------
c_rhf_4 = $
  FMD_DESTRIPER(freq=f_hi4,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rhf_4,afp=s_rhf_4, $
                func=func,sky_func=sky_func_rhf_4,cal_func=cal_func_rhf_4, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rhf_4, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rhf_4,/pure,/mjy) 
;

IF (MAX(c_rhf_4) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rhf_4,gamma=gamma_rhf_4,lmat=lmat
;
...
; Make RHF_4_EJG Save Set
; -----------------------
chan_label = 'RHF_4'
sname='csdr$firas_out:rhf_4_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi4,ejg_rhf_4, $
 c_rhf_4,ndf_ejg_rhf_4,chi2_rhf_4,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4_EJG.ISS" Created.'
;

; Make RHF_4_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhf_4_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi4,c_rhf_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4_ERRORS.ISS" Created.'
;

; Make RHF_4_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rhf_4_skymap.iss'
freq_rhf_4 = f_hi4
SAVE,filename=sname,chan_label,stripes_descrip,freq_rhf_4,l_rhf,b_rhf,n_rhf, $
                    c_rhf_4,s_rhf_4,z_rhf_4,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RHF_4_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rhf_4_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rhf_4,cal_func_rhf_4, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rhf=0
zodi_array=0 & scan=0 & time=0 & chanscan=0 & model_var=0 & sl_weight_rat=0
idd=0 & g8x=0 & g9x=0 & g10x=0 & f8=0 & f9=0 & f10=0
tm_up=0 & tm_dn=0 & gpow=0 & bpow=0 & nscan=0 & delt=0 & tmcut=0 & lp_orderv=0
badcoadd_rhs=0 & badcoadd_lhf=0 & badcoadd_lhs=0
badcal_rhs=0 & badcal_lhf=0 & badcal_lhs=0
;

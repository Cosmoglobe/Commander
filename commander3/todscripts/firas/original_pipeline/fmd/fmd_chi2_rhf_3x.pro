Pro FMD_CHI2_RHF_3X,error
;

;
;  FMD_CHI2_RHF_3X drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RHFA Band_3 offset destriped data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHF_3X_DSP.ISS, RHFX_WEIGHTS.ISS,
;                        and RHF_3X_EJG.ISS, and where output save sets 
;                        RHF_3X_CVECTOR.ISS, RHF_3X_SKY_RESID.ISS,
;                        RHF_3X_SKY_CHI2.ISS, RHF_3X_CAL_RESID.ISS,
;                        and RHF_3X_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RHF_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RHF_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RHF_3X,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RHF_3X,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHF_3X_DSP.ISS'
RESTORE,'csdr$firas_out:rhf_3x_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RHF_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi3(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_RHF_3X : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore RHF_3X_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHF_3X_EJG.ISS'
RESTORE,'csdr$firas_out:rhf_3x_ejg.iss'
;
...
cvec_rhf_3 = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_rhf_3,resid=sky_resid_rhf_3,$
          chi2=sky_chi2_rhf_3,ndf_cvec=ndf_cvec_rhf_3)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi3,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_rhf_3,resid=cal_resid_rhf_3,$
              chi2=cal_chi2_rhf_3)
;
...
; Make RHF_3X_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rhf_3x_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_rhf_3,sky_resid_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RHF_3X_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:rhf_3x_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rhf_3,cvec_rhf_3,ndf_cvec_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RHF_3X_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rhf_3x_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rhf_3,cvec_rhf_3,sky_chi2_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RHF_3X_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rhf_3x_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhf_3,cal_resid_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RHF_3X_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rhf_3x_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhf_3,cvec_rhf_3,$
                    cal_chi2_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_CAL_CHI2.ISS" Created.'
PRINT,' '
;

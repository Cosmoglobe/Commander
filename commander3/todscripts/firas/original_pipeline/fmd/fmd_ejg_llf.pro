Pro FMD_EJG_LLF,error
;

;
;  FMD_EJG_LLF drives the FMD_DESTRIPER procedure to create
;              IDL save sets of LLLF offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_HR.ISS, FMD_DIRBE_FUNC_LLF.ISS,
;                        and LLF_WEIGHTS.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing LLF.ISS and LLF_CALSPEC.ISS .
;
...
;  Output  :
;
;    LLF_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    LLF_ERRORS.ISS, contains destriper error matrices.
;
;    LLF_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LLF_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LLF,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LLF : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LLF : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LLF : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LLF : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LLF Offset Destriping -- Pass 4'
PRINT,' '
...
; -------------------------------------------
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'LLF_CALSPEC.ISS'
RESTORE,'csdr$firas_in:llf_calspec.iss'
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_llf.iss'
;
...
IF (badcoadd_llf(0) ge 0) THEN badcoadd(badcoadd_llf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_llf(0) ge 0) THEN badcal(badcal_llf) = 1B
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_llf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_llf

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_llf
...
c_llf = $
  FMD_DESTRIPER(freq=f_hr,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_llf,afp=s_llf, $
                func=func,sky_func=sky_func_llf,cal_func=cal_func_llf, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_llf, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_llf,/pure,/mjy) 
;

IF (MAX(c_llf) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_llf,gamma=gamma_llf,lmat=lmat
;
...
; Make LLF_EJG Save Set
; -----------------------
chan_label = 'LLF'
sname='csdr$firas_out:llf_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hr,ejg_llf, $
 c_llf,ndf_ejg_llf,chi2_llf,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_EJG.ISS" Created.'
;

; Make LLF_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:llf_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hr,c_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_ERRORS.ISS" Created.'
;

; Make LLF_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:llf_skymap.iss'
freq_llf = f_hr
SAVE,filename=sname,chan_label,stripes_descrip,freq_llf,l_llf,b_llf,n_llf, $
                    c_llf,s_llf,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LLF_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:llf_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_llf,cal_func_llf, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_llf=0

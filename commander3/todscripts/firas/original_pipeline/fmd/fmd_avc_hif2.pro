...
;  FMD_AVC_HIF2 creates and saves the binary records of HIGH_2 A-Vector .
;
;-
;______________________________________________________________________________
;
Pro FMD_AVC_HIF2,error
;
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_AVC_HIF2 : Called Incorrectly : FMD_AVC_HIF2,error'
 print,' '
 return
endif
;
...
; Restore the saveset
; -------------------
PRINT,'Restoring HIGH_2_AVECTOR.ISS'
PRINT,' '
RESTORE,'csdr$firas_out:high_2_avector.iss'
;
...
PRINT,' '
avc_rec.a_vector(0:31) = FLOAT(avec_high_2(0:31))
;
...
PRINT,' '
outfile = 'csdr$firas_out:fmd_avc_hif2.pass4'
rec_len = 128            ; fixed length record size in bytes
OPENW,1,outfile, rec_len, /fixed
WRITEU,1,avc_rec(0)
CLOSE,1
;
fname = STRUPCASE(outtrans(0)) + ':FMD_AVC_HIF2.PASS4'
...
freq_high_2=0 & delta_freq_hi_2=0
;

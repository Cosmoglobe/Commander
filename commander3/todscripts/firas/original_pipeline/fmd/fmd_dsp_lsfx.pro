Pro FMD_DSP_LSFX,error
;

;
;  FMD_DSP_LSFX drives the FMD_APPLY procedure to apply destriper
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LSFX_EJG.ISS and LSF_FUNC.ISS,
;                        and where LSFX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LSFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LSFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LSFX,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LSFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'LSF'
sname = 'csdr$firas_out:lsfx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LSFX_DSP.ISS" Created.'
PRINT,' '
;

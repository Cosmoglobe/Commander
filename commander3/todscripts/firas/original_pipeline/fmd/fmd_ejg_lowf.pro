Pro FMD_EJG_LOWF,error
;

;
;  FMD_EJG_LOWF drives the FMD_DESTRIPER procedure to create
;  IDL save sets of combined low channel Band_4 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_DIRBE_FUNC_LOWF.ISS, and FMD_BAD_COADD_LO.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing LOWF.ISS, LOWF_WEIGHTS.ISS,
;                        and LOWF_CALSPEC.ISS .
;
...
;  Output  :
;
;    LOWF_EJG.ISS, contains destriper qualifiers, weights, and stripe spectra.
;
;    LOWF_ERRORS.ISS, contains destriper error matrices.
;
;    LOWF_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LOWF_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LOWF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LOWF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LOWF,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LOWF : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
;
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LOWF : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
;
...
PRINT,' '
PRINT,'BADCOADD_LLS =',badcoadd_lls     ; Bad LLS SKY Coadds
PRINT,' '
...
PRINT,' '
PRINT,'BADCAL_LLS =',badcal_lls         ; Bad LLS CAL Coadds
PRINT,' '
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LOWF : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LOWF : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LOWF Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LOWF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; --------------------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'FMD_DIRBE_FUNC_LOWF.ISS'
RESTORE,'csdr$firas_ref:fmd_dirbe_func_lowf.iss'
;
...
; Compute n_lowf, l_lowf, b_lowf
; ------------------------------
uv = COORCONV([[glon],[glat]],infmt='l',outfmt='u')

gw = WHERE(sky_wgts_ds GT 0.)
uvavg0 = FMD_PIX_SUM(pix=px(gw),data=uv(gw,0),wgt=sky_wgts_ds(gw),cmp_px=cmp_px)
uvavg1 = FMD_PIX_SUM(pix=px(gw),data=uv(gw,1),wgt=sky_wgts_ds(gw),cmp_px=cmp_px)
uvavg2 = FMD_PIX_SUM(pix=px(gw),data=uv(gw,2),wgt=sky_wgts_ds(gw),cmp_px=cmp_px)

ll = COORCONV([[uvavg0],[uvavg1],[uvavg2]],infmt='u',outfmt='l')

PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_lowf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_lowf

n_tot = FMD_PIX_SUM(pix=px(gw),data=sky_wgts_ds(gw))
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_lowf
...
;  -------------
c_lowf = $
  FMD_DESTRIPER(freq=f_lo,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_lowf,afp=s_lowf,chi2_chan=chi2_loch, $
                func=func,sky_func=sky_func_lowf,cal_func=cal_func_lowf, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_lowf, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                num_coadd=nc_lowf,ndf=ndf_ejg_lowf,/pure,/mjy) 
;

IF (MAX(c_lowf) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_lowf,gamma=gamma_lowf,lmat=lmat
;
...
; Make LOWF_EJG Save Set
; ------------------------
chan_label = 'LLS_RLS_LSF_RSF'
sname='csdr$firas_out:lowf_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts_ds,f_lo,ejg_lowf,gamma_lowf, $
 c_lowf,ndf_ejg_lowf,chi2_loch,dihed_cut_min,max_frac, $
 n_stripes,freq_band
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LOWF_EJG.ISS" Created.'
;

; Make LOWF_ERRORS.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:lowf_errors.iss'
SAVE,filename=sname,chanscan,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_lo,c_lowf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LOWF_ERRORS.ISS" Created.'
;

; Make LOWF_SKYMAP.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:lowf_skymap.iss'
freq_lowf = f_lo
SAVE,filename=sname,chanscan,stripes_descrip,freq_lowf,l_lowf,b_lowf,n_lowf, $
                    nc_lowf,c_lowf,s_lowf,chi2_lowf,cmp_px,pcvr,rect,beta, $
                    stripe_contrib
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LOWF_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LOWF_FUNC.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:lowf_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_lowf,$
                    cal_func_lowf,sky_idx,cal_idx,chan_label
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LOWF_FUNC.ISS" Created.'
PRINT,' '
;
...
frac_wgt=0 & pixel_wgt=0 & zodi_array=0 & scan=0 & sky_wgts=0 & st_sub=0
nifgs=0 & ical=0 & refh=0 & skyh=0 & cal_nifgs=0 & cal_wgts=0
freq_lowf=0 & scale_factor=0 & g8x=0 & g9x=0 & g10x=0 & f8=0 & f9=0 & f10=0

Pro FMD_DSP_LHF_3X,error
;

;
;  FMD_DSP_LHF_3X drives the FMD_APPLY procedure to apply destriper
;  corrections to LHFA Band_3 data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHF_3X_EJG.ISS and LHF_3_FUNC.ISS,
;                        and where LHF_3X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LHF_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LHF_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LHF_3X,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_3_FUNC.ISS'
RESTORE,'csdr$firas_out:lhf_3_func.iss'
;
...
; Restore IDL Save Set of LHF Undestriped Spectra
; -------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LHF_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_lhf_3],[cal_func_lhf_3]]
;
...
dummy = FMD_APPLY(freq=f_hi3,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_lhf_3,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'LHF_3'
sname = 'csdr$firas_out:lhf_3x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi3,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3X_DSP.ISS" Created.'
PRINT,' '
;
...
glon=0 & glat=0 & sky_lbl=0 & cal_lbl=0 & sl_weight_rat=0
chan_label=0 & sky_wgts=0 & sky_wgts_ds=0 & sky_idx=0 & sky_mask=0
dirbe_mask=0 & cal_wgts=0 & cal_wgts_ds=0 & st_sub=0 & fsl_idx=0
...
c_lhf_3=0 & ndf_ejg_lhf_3=0 & chi2_lhf_3=0
;

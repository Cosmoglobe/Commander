Pro FMD_EJG_LSF,error
;

;
;  FMD_EJG_LSF drives the FMD_DESTRIPER procedure to create
;              IDL save sets of LLFA offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_LO.ISS, FMD_DIRBE_FUNC_LSF.ISS,
;                        and LSF_WEIGHTS.ISS .
;
...
;  Output  :
;
;    LSF_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    LSF_ERRORS.ISS, contains destriper error matrices.
;
;    LSF_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LSF_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LSF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LSF,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LSF : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LSF : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LSF : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LSF : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LSF Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LSF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_lsf.iss'
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_lsf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_lsf

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_lsf
...
c_lsf = $
  FMD_DESTRIPER(freq=f_lo,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_lsf,afp=s_lsf, $
                func=func,sky_func=sky_func_lsf,cal_func=cal_func_lsf, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_lsf, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_lsf,/pure,/mjy) 
;

IF (MAX(c_lsf) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_lsf,gamma=gamma_lsf,lmat=lmat
;
...
; Make LSF_EJG Save Set
; -----------------------
chan_label = 'LSF'
sname='csdr$firas_out:lsf_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_lo,ejg_lsf, $
 c_lsf,ndf_ejg_lsf,chi2_lsf,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LSF_EJG.ISS" Created.'
;

; Make LSF_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lsf_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_lo,c_lsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LSF_ERRORS.ISS" Created.'
;

; Make LSF_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lsf_skymap.iss'
freq_lsf = f_lo
SAVE,filename=sname,chan_label,stripes_descrip,freq_lsf,l_lsf,b_lsf,n_lsf, $
                    c_lsf,s_lsf,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LSF_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LSF_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:lsf_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_lsf,cal_func_lsf, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LSF_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_lsf=0
...
badcoadd_rls=0 & badcoadd_rsf=0 & badcoadd_lls=0
badcal_rls=0 & badcal_rsf=0 & badcal_lls=0
;

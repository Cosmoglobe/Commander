Pro FMD_WGTS_RLF,model_var,error
;
;
;  FMD_WGTS_RLF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
...
;                        RLF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RLF.ISS and RLF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where RLF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_RLF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_RLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RLF,model_var,error'
 PRINT,' '
...
;
 ...
 ; ------------------------------------
 PRINT,' '
 PRINT,'Restoring ' + intrans(0) + 'RLF_VAR.ISS'
 RESTORE,'csdr$firas_in:rlf_var.iss'
 ;

 ; Index of "Good" Frequencies
 ; ---------------------------
 fidx = WHERE((f_rlf GT freq_band(0))and(f_rlf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_RLF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vw = 1. / d_rlf(fidx) / d_rlf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vt = ( TRANSPOSE(var_rlf(fidx,*)) # vw ) / TOTAL(vw)           ; SKY
 cal_vt = ( TRANSPOSE(cal_var_rlf(fidx,*)) # vw ) / TOTAL(vw)   ; CAL
 ;
 ...
 IF (cgood LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RLF : No Good Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;
 ...
 chanscan = 'RLF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:rlf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_rlf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'RLF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make RLF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'RLF'
sname = 'csdr$firas_ref:rlf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RLF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_rlf=0 & n_rlf=0 & b_rlf=0 & l_rlf=0 & f_rlf=0 & galcut_rlf=0

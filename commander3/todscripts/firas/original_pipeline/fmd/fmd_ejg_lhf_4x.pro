Pro FMD_EJG_LHF_4X,error
;

;
;  FMD_EJG_LHF_4X drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of LHFA Band_4 offset stripes,
;                 using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_LHF.ISS, 
;                        and LHFX_WEIGHTS.ISS.
;
;    CSDR$FIRAS_IN    =  Directory containing LHF.ISS and LHF_CALSPEC.ISS,
...
;  Output  :
;
;    LHF_4X_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                    and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LHF_4X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LHF_4X,error'
 PRINT,' '
...
; ---------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'LHFX_WEIGHTS.ISS'
RESTORE,'csdr$firas_ref:lhfx_weights.iss'
;
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LHF_4X : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LHF_4X : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LHF_4X : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LHF_4X : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LHF_4X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_LHF_4X : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_lhf.iss'
;
...
; Make LHF_4X_EJG Save Set
; ------------------------
chan_label = 'LHF_4'
sname='csdr$firas_out:lhf_4x_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_EJG.ISS" Created.'
;
...
; ------------------------------------
hot_cal=0 & gain_convg=0 & gain_iter=0 & sky_glitch=0 & cal_glitch=0
ical=0 & refh=0 & skyh=0 & pixel_wgt=0 & nifgs=0 & solution=0 & galcut=0
zodi_array=0 & scan=0 & time=0 & chanscan=0 & model_var=0 & glon=0 & glat=0
idd=0 & g8x=0 & g9x=0 & g10x=0 & f8=0 & f9=0 & f10=0 & sl_weight_rat=0
tm_up=0 & tm_dn=0 & gpow=0 & bpow=0 & nscan=0 & delt=0 & tmcut=0 & lp_orderv=0
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_lhf=0 & n_lhf=0 & b_lhf=0 & l_lhf=0
badcoadd_lhs=0 & badcoadd_rhs=0 & badcoadd_rhf=0
badcal_lhs=0 & badcal_rhs=0 & badcal_rhf=0 & st_sub=0 & fsl_idx=0

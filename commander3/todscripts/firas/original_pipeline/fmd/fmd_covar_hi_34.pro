Pro FMD_COVAR_HI_34,error
;

;
;  FMD_COVAR_HI_34 drives the FMD_COVAR procedure to create an IDL
;  save set of combined high-channel Band_3/Band_4 covariance.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and FMD_QUALS_HI_4.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing HIGH_WEIGHTS.ISS.
;
;    CSDR$FIRAS_OUT   =  Directory containing HIGH_3_SKY_RESID.ISS,
;                        and HIGH_4_SKY_RESID.ISS, and where output
;                        HIGH_34_COVAR.ISS will be sent.
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HI_34 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HI_34,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'FMD_QUALS_HI_4.ISS'
...
resid1 = TEMPORARY(sky_resid_high3)
;
PRINT,'Restoring ' + outtrans(0) + 'HIGH_4_SKY_RESID.ISS'
RESTORE,'csdr$firas_out:high_4_sky_resid.iss'
resid2 = TEMPORARY(sky_resid_high4)
;

nfreq = [N_ELEMENTS(freq_high_3),N_ELEMENTS(freq_high_4)]
;
...
error =  FMD_COVAR_OFF(nfreq=nfreq,resid1=resid1,resid2=resid2,$
                       ...
                       covar=covar_high_34,ndf_covar=ndf_covar_hi_34)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HI_34 : Error Returned from FMD_COVAR_OFF !'
 RETURN
ENDIF
;
...
freq_dependence = 'N'
n_stripes = nst
sname = 'csdr$firas_out:high_34_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_high_3,freq_high_4,covar_high_34,ndf_covar_hi_34
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_34_COVAR.ISS" Created.'
PRINT,' '
;
...
freq_high=0 & sp_high_3=0 & z_high_3=0 & sp_high_4=0 & z_high_4=0
;

Pro FMD_EJG_RHS_3,error
;

;
;  FMD_EJG_RHS_3 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of RHSS Band_3 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_HI.ISS, FMD_DIRBE_FUNC_RHS.ISS,
;                        and RHS_WEIGHTS.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing RHS.ISS, RHS_CALSPEC.ISS,
;                        and RHS_3_ZODISPEC.ISS .
;
...
;  Output  :
;
;    RHS_3_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    RHS_3_ERRORS.ISS, contains destriper error matrices.
;
;    RHS_3_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RHS_3_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RHS_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RHS_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RHS_3,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RHS_3 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RHS_3 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RHS_3 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RHS_3 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RHS_3 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RHS_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_rhs.iss'
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_rhs
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_rhs

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_rhs
...
;
PIX2XY,cmp_px,data=dummy,res=6,/six,ras=z_rhs_3
;
...
c_rhs_3 = $
  FMD_DESTRIPER(freq=f_hi3,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rhs_3,afp=s_rhs_3, $
                func=func,sky_func=sky_func_rhs_3,cal_func=cal_func_rhs_3, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rhs_3, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rhs_3,/pure,/mjy) 
;

IF (MAX(c_rhs_3) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rhs_3,gamma=gamma_rhs_3,lmat=lmat
;
...
; Make RHS_3_EJG Save Set
; -----------------------
chan_label = 'RHS_3'
sname='csdr$firas_out:rhs_3_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi3,ejg_rhs_3, $
 c_rhs_3,ndf_ejg_rhs_3,chi2_rhs_3,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_EJG.ISS" Created.'
;

; Make RHS_3_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_3_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi3,c_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_ERRORS.ISS" Created.'
;

; Make RHS_3_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rhs_3_skymap.iss'
freq_rhs_3 = f_hi3
SAVE,filename=sname,chan_label,stripes_descrip,freq_rhs_3,l_rhs,b_rhs,n_rhs, $
                    c_rhs_3,s_rhs_3,z_rhs_3,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RHS_3_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rhs_3_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rhs_3,cal_func_rhs_3, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rhs=0

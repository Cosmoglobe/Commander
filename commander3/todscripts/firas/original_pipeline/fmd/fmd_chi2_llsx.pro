Pro FMD_CHI2_LLSX,error
;

;
;  FMD_CHI2_LLSX drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLSX_DSP.ISS, LLSX_WEIGHTS.ISS,
;                        and LLSX_EJG.ISS, and where output save sets 
;                        LLSX_CVECTOR.ISS, LLSX_SKY_RESID.ISS,
;                        LLSX_SKY_CHI2.ISS, LLSX_CAL_RESID.ISS,
;                        and LLSX_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_LLSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_LLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_LLSX,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_LLSX,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLSX_DSP.ISS'
RESTORE,'csdr$firas_out:llsx_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_LLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; Restore LLSX_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLSX_EJG.ISS'
RESTORE,'csdr$firas_out:llsx_ejg.iss'
;
...
; Make LLSX_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:llsx_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make LLSX_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:llsx_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_lls,cvec_lls,ndf_cvec_lls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make LLSX_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:llsx_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_lls,cvec_lls,sky_chi2_lls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make LLSX_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:llsx_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make LLSX_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:llsx_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,cal_s0,$
                    cal_wgts_ds,cal_wgts,freq_lls,cvec_lls,cal_chi2_lls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_CAL_CHI2.ISS" Created.'
PRINT,' '
;

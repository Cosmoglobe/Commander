Pro FMD_CHI2_RLS,error
;

;
;  FMD_CHI2_RLS drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RLSS offset destriped data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLS_DSP.ISS, RLS_WEIGHTS.ISS,
;                        and RLS_EJG.ISS, and where output save sets 
;                        RLS_CVECTOR.ISS, RLS_SKYMAP_2.ISS, RLS_SKY_RESID.ISS,
;                        RLS_SKY_CHI2.ISS, RLS_CAL_RESID.ISS,
;                        and RLS_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RLS,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RLS,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLS_DSP.ISS'
RESTORE,'csdr$firas_out:rls_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; Restore RLS_EJG Save Set
; ------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLS_EJG.ISS'
RESTORE,'csdr$firas_out:rls_ejg.iss'
;
...
cvec_rls = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_rls,resid=sky_resid_rls,$
          chi2=sky_chi2_rls,ndf_cvec=ndf_cvec_rls)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_lo,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_rls,resid=cal_resid_rls,$
              chi2=cal_chi2_rls)
;
...
; Make RLS_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rls_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rls,sp_rls,cvec_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RLS_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rls_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_rls,sky_resid_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RLS_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rls_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rls,cvec_rls,ndf_cvec_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RLS_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rls_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rls,cvec_rls,sky_chi2_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RLS_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rls_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rls,cal_resid_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RLS_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rls_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,cal_s0,$
                    cal_wgts_ds,cal_wgts,freq_rls,cvec_rls,cal_chi2_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_CAL_CHI2.ISS" Created.'
PRINT,' '
;
...
ejg_rls=0 & c_rls=0 & ndf_ejg_rls=0 & chi2_rls=0
n_rls=0 & b_rls=0 & l_rls=0 & d_rls=0

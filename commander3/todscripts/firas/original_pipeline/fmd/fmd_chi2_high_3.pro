Pro FMD_CHI2_HIGH_3,error
;
;  FMD_CHI2_HIGH_3 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for combined high channel Band_3 destriped data, using frequency
...
;                        CVECTORS_HI.ISS .
;
;    CSDR$FIRAS_IN    =  Directory containing HIGH_3_ZODI_SKYMAP.ISS, HIGH.ISS,
;                        HIGH_WEIGHTS.ISS, and HIGH_WEIGHTS_F.ISS .
;
;    CSDR$FIRAS_OUT   =  Directory containing HIGH_3_EJG.ISS and
;                        HIGH_3_DSP.ISS, and where IDL save sets
;                        HIGH_3_CVECTOR.ISS, HIGH_3_SKYMAP_F.ISS
;                        or HIGH_3_SKYMAP_2.ISS, HIGH_3_SKY_CHI2.ISS,
;                        HIGH_3_CAL_RESID.ISS, and HIGH_3_CAL_CHI2.ISS
;                        will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_HIGH_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_HIGH_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_HIGH_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_HIGH_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'HIGH_3_DSP.ISS'
RESTORE,'csdr$firas_out:high_3_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_HIGH_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (freq_flag EQ 'Y') THEN BEGIN
 ...
 IF (cf NE N_ELEMENTS(freq)) THEN BEGIN
  PRINT,'FMD_CHI2_HIGH_3 : C-Vector Frequencies are NOT Compatible !'
  error = 1
  RETURN
 ENDIF
 ;
...
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'HIGH_3_ZODI_SKYMAP.ISS'
RESTORE,'csdr$firas_in:high_3_zodi_skymap.iss'
;

; Restore HIGH_3_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'HIGH_3_EJG.ISS'
...
IF (freq_flag EQ 'Y') THEN BEGIN
 ...
 cvec_high_3 = $
  FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,sky_idx=sky_idx,$
           ...
           px_spec=sp_high_3,chi2=sky_chi2_high_3,$
           ndf_cvec=ndf_cvec_high_3)
 ;

 error = $
  FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi3,cal_idx=cal_idx,$
               cal_wgts=cal_wgts_ds,cal_spec=cal_dsp,wfac=wfac,$
               cvec=cvec_high_3,resid=cal_resid_high3,chi2=cal_chi2_high_3)
 ;
...
IF (freq_flag NE 'Y') THEN BEGIN
 ...
 error = $
  FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi3,cal_spec=cal_dsp,$
               cal_wgts=cal_wgts_ds,resid=cal_resid_high3,$
               chi2=cal_chi2_high_3,cvec=cvec_high_3)
 ;
...
IF (freq_flag EQ 'Y') THEN BEGIN
 ;
 ; Make HIGH_3_SKYMAP_F.ISS Save Set
 ; ---------------------------------
 sname = 'csdr$firas_out:high_3_skymap_f.iss'
 SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                     freq_high_3,sp_high_3,z_high_3,cvec_high_3
 PRINT,' '
 PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_SKYMAP_F.ISS" Created.'
 PRINT,' '
;
ENDIF
;
...
IF (freq_flag NE 'Y') THEN BEGIN
 ;
 ; Make HIGH_3_SKYMAP_2.ISS Save Set
 ; ---------------------------------
 sname = 'csdr$firas_out:high_3_skymap_2.iss'
 SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                     freq_high_3,sp_high_3,z_high_3,cvec_high_3
 PRINT,' '
 PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_SKYMAP_2.ISS" Created.'
 PRINT,' '
;
ENDIF
;

; Make HIGH_3_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:high_3_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_high_3,cvec_high_3,ndf_cvec_high_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make HIGH_3_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:high_3_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,$
                    sky_idx,sky_wgts_ds,frac_wgt,sky_mask,freq_high_3,$
                    cvec_high_3,sky_chi2_high_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make HIGH_3_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:high_3_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    ...
                    cal_s0,cal_wgts_ds,cal_wgts,freq_high_3,cvec_high_3,$
                    cal_resid_high3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make HIGH_3_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:high_3_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,cal_idx,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_high_3,cvec_high_3,$
                    cal_chi2_high_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_3_CAL_CHI2.ISS" Created.'
PRINT,' '
;

; Re-Define Unused Restored Parameters
; ------------------------------------
nifgs=0 & cal_nifgs=0 & freq_high=0 & gamma_high_3=0
...
zodi_array=0 & tm=0 & sky_dihd=0 & sky_glitch=0 & sky_s0=0 & scan=0 & fz_hi3=0
...
ejg_high_3=0 & c_high_3=0 & ndf_ejg_high_3=0 & chi2_high_3=0 & chi2_hi3ch=0
;

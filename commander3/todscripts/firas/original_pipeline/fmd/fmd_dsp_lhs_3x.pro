Pro FMD_DSP_LHS_3X,error
;

;
;  FMD_DSP_LHS_3X drives the FMD_APPLY procedure to apply destriper
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHS_3X_EJG.ISS and LHS_3_FUNC.ISS,
;                        and where LHS_3X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LHS_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LHS_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LHS_3X,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LHS_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'LHS_3'
sname = 'csdr$firas_out:lhs_3x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi3,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3X_DSP.ISS" Created.'
PRINT,' '
;

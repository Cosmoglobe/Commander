...
;  FMD_CVC_HIGH creates and saves the binary records of HIGH C-Vector .
;
;-
;______________________________________________________________________________
;
Pro FMD_CVC_HIGH,error
;
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_CVC_HIGH : Called Incorrectly : FMD_CVC_HIGH,error'
 print,' '
 return
endif
;
...
PRINT,' '
nf = N_ELEMENTS(c_high)
cvc_rec.c_vector(0:nf-1) = FLOAT(c_high)
...
PRINT,' '
outfile = 'csdr$firas_out:fmd_cvc_high.pass4'
rec_len = 728            ; fixed length record size in bytes
OPENW,1,outfile, rec_len, /fixed
WRITEU,1,cvc_rec(0)
CLOSE,1
;
fname = STRUPCASE(outtrans(0)) + ':FMD_CVC_HIGH.PASS4'
...
lmat=0 & chanscan=0 & f_hi=0 & gamma_high=0
;

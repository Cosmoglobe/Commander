Pro FMD_CHI2_RLF,error
;

;
;  FMD_CHI2_RLF drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RLLF offset destriped data .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLF_DSP.ISS, RLF_WEIGHTS.ISS,
;                        and RLF_EJG.ISS, and where output save sets 
;                        RLF_CVECTOR.ISS, RLF_SKYMAP_2.ISS, RLF_SKY_RESID.ISS,
;                        RLF_SKY_CHI2.ISS, RLF_CAL_RESID.ISS,
;                        and RLF_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLF_DSP.ISS'
RESTORE,'csdr$firas_out:rlf_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hr(nf)
...
; Make RLF_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rlf_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rlf,sp_rlf,cvec_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RLF_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rlf_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RLF_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rlf_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rlf,cvec_rlf,ndf_cvec_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RLF_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rlf_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rlf,cvec_rlf,sky_chi2_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RLF_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rlf_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RLF_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rlf_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rlf,cvec_rlf,$
                    cal_chi2_rlf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLF_CAL_CHI2.ISS" Created.'
PRINT,' '
;

Pro FMD_Bad_LLS,badcoadd_lls,badcal_lls,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad_LLS,badcoadd_lls,badcal_lls,error'
 error = 1
 return
endif
;
...
badcoadd_old = badcoadd_lls
badcal_old = badcal_lls
;

RESTORE,'csdr$firas_out:llsx_sky_chi2.iss'
wd0 = TEMPORARY(sky_wgts_ds)
dummy = MAX(ABS(badcoadd_old-WHERE(wd0 EQ 0.)))
IF (dummy NE 0.) THEN BEGIN
 PRINT,'FMD_BAD_LLS : Error in SKY_CHI2 Weights !'
 RETURN
ENDIF
;

RESTORE,'csdr$firas_out:llsx_cal_chi2.iss'
cwd0 = TEMPORARY(cal_wgts_ds)
IF (MAX(badcal_old) GE 0) THEN BEGIN
 dummy = MAX(ABS(cwd0(badcal_old)))
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD_LLS : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;

ct = TOTAL(sky_chi2_lls,1)
cct = TOTAL(cal_chi2_lls,1)
;
...
IF (cbad GT 0) THEN BEGIN
 ...
 badcoadd_lls = bad_new(SORT(bad_new))
 nq = WHERE(badcoadd_lls GE 0,cq)
 IF (cq LE 0) THEN badcoadd_lls = [-1]
 IF (cq GT 0) THEN badcoadd_lls = badcoadd_lls(nq)
ENDIF
;
...
IF (cbad GT 0) THEN BEGIN
 ...
 badcal_lls = bad_new(SORT(bad_new))
 nq = WHERE(badcal_lls GE 0,cq)
 IF (cq LE 0) THEN badcal_lls = [-1]
 IF (cq GT 0) THEN badcal_lls = badcal_lls(nq)
ENDIF
;
...
badcal_llf=0 & badcal_lhs=0 & badcal_rls=0 & badcal_rsf=0 & badcal_lsf=0
;

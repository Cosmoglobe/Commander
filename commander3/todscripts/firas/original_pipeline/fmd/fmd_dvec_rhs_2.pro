Pro FMD_DVEC_RHS_2,error
;

;
;  FMD_DVEC_RHS_2 drives the FMD_DVECTOR procedure to compute
;  the RHSS Band_2 D-Vector .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHS_2_CVECTOR.ISS and RHS_2_EJG.ISS,
;                        where output RHS_2_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_RHS_2,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_RHS_2 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_RHS_2,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_RHS_2,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_2_CVECTOR.ISS'
...
; Frequency Cut
; -------------
freq = f_rhs * freq_corr
nf = WHERE((freq GE freq_band(0))and(freq LE freq_band(1)),cf)
IF (cf NE N_ELEMENTS(freq_rhs_2)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_RHS_2 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_rhs_2 = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_rhs_2,ndf_dvec=ndf_dvec_rhs_2)
;

; Make RHS_2_DVECTOR.ISS Save Set
; -------------------------------
chanscan = 'RHS_2'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:rhs_2_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_rhs_2,$
                    dvec_rhs_2,good_dvec_rhs_2,ndf_dvec_rhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_2_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_rhs=0 & b_rhs=0 & l_rhs=0 & d_rhs=0 & f_hi2=0
cvec_rhs_2=0 & ndf_cvec_rhs_2=0 & galcut_rhs=0 & cal_var_rhs=0
;

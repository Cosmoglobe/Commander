Pro FMD_CHI2_RHS_3,error
;

;
;  FMD_CHI2_RHS_3 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RHSS Band_3 offset destriped data, using frequency-independent
;  coadd weights.
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHS_3_DSP.ISS, RHS_WEIGHTS.ISS,
;                        and RHS_3_EJG.ISS, and where output save sets 
;                        RHS_3_CVECTOR.ISS, RHS_3_SKYMAP_2.ISS, RHS_3_SKY_CHI2.ISS
;                        and RHS_3_CAL_CHI2.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RHS_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RHS_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RHS_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RHS_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_3_DSP.ISS'
RESTORE,'csdr$firas_out:rhs_3_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RHS_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi3(nf)
...
nq = WHERE(f_hi3 NE freq,cq)
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_RHS_3 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore RHS_3_EJG Save Set
; -------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_3_EJG.ISS'
RESTORE,'csdr$firas_out:rhs_3_ejg.iss'
;
...
cvec_rhs_3 = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_rhs_3,resid=sky_resid_rhs_3,$
          chi2=sky_chi2_rhs_3,ndf_cvec=ndf_cvec_rhs_3)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi3,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_rhs_3,resid=cal_resid_rhs_3,$
              chi2=cal_chi2_rhs_3)
;
...
; Make RHS_3_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_3_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rhs_3,sp_rhs_3,cvec_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RHS_3_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhs_3_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_rhs_3,sky_resid_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_3_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rhs_3_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rhs_3,cvec_rhs_3,ndf_cvec_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RHS_3_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_3_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rhs_3,cvec_rhs_3,sky_chi2_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RHS_3_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhs_3_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhs_3,cal_resid_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_3_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_3_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhs_3,cvec_rhs_3,$
                    cal_chi2_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3_CAL_CHI2.ISS" Created.'
PRINT,' '
;
...
ejg_rhs_3=0 & c_rhs_3=0 & ndf_ejg_rhs_3=0 & chi2_rhs_3=0
zodi_array=0 & solution=0 & f=0 & galcut=0 & cal_lbl=0 & sky_lbl=0 & scan=0
time=0 & model_var=0 & lp_orderv=0 & fsl_idx=0 & st_sub=0

Pro FMD_EJG_RLSX,error
;

;
;  FMD_EJG_RLSX drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_RLS.ISS, 
;                        and RLSX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    RLSX_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RLSX,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RLSX : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RLSX : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RLSX : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RLSX : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_rls.iss'
;
...
; Make RLSX_EJG Save Set
; ------------------------
chan_label = 'RLS'
sname='csdr$firas_out:rlsx_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_EJG.ISS" Created.'
;
...
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rls=0 & n_rls=0 & b_rls=0 & l_rls=0
badcoadd_rhs=0 & badcoadd_rhf=0 & badcoadd_lhf=0
badcal_rhs=0 & badcal_rhf=0 & badcal_lhf=0
badcoadd_lls=0 & badcoadd_rsf=0 & badcoadd_lhs=0 & badcoadd_lsf=0
badcal_lls=0 & badcal_rsf=0 & badcal_lhs=0 & badcal_lsf=0

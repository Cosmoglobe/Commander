Pro FMD_CHI2_RHF_3,error
;

;
;  FMD_CHI2_RHF_3 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RHFA Band_3 offset destriped data, using frequency-independent
;  coadd weights.
;
;
;  ARGUMENTS (I/O)    :
...
;                        and RHF_3_EJG.ISS, and where output save sets 
;                        RHF_3_CVECTOR.ISS, RHF_3_SKYMAP_2.ISS, RHF_3_SKY_CHI2.ISS
;                        and RHF_3_CAL_CHI2.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RHF_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RHF_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RHF_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RHF_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RHF_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi3(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_RHF_3 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Make RHF_3_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhf_3_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rhf_3,sp_rhf_3,cvec_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RHF_3_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhf_3_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RHF_3_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rhf_3_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rhf_3,cvec_rhf_3,ndf_cvec_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RHF_3_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhf_3_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rhf_3,cvec_rhf_3,sky_chi2_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RHF_3_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rhf_3_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RHF_3_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhf_3_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhf_3,cvec_rhf_3,$
                    cal_chi2_rhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3_CAL_CHI2.ISS" Created.'
PRINT,' '
;

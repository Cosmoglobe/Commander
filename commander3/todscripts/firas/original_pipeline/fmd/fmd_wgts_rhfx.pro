Pro FMD_WGTS_RHFX,error
;
;
;  FMD_WGTS_RHFX drives the FMD_PIXEL_WGT procedures to create IDL save sets
;  of RHFA weights, using the default bad coadd indices.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where RHFX_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RHF.ISS .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_WGTS_RHFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_WGTS_RHFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RHFX,error'
 PRINT,' '
...
; Make RHFX_WEIGHTS.ISS Save Set
; ---------------------------------
chanscan = 'RHF'
sname = 'csdr$firas_ref:rhfx_weights.iss'
SAVE,filename=sname,chanscan,px,sky_wgts,sky_wgts_ds,pixel_wgt,$
                    frac_wgt,cal_wgts,cal_wgts_ds
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RHFX_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
fsl_idx=0 & n_rhf=0 & b_rhf=0 & l_rhf=0 & d_rhf=0

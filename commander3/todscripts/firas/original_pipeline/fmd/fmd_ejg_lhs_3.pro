Pro FMD_EJG_LHS_3,error
;

;
;  FMD_EJG_LHS_3 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of LHSS Band_3 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Output  :
;
;    LHS_3_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    LHS_3_ERRORS.ISS, contains destriper error matrices.
;
;    LHS_3_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    LHS_3_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_LHS_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LHS_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LHS_3,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LHS_3 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LHS_3 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LHS_3 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LHS_3 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'LHS_3 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LHS_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_LHS_3 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
c_lhs_3 = $
  FMD_DESTRIPER(freq=f_hi3,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_lhs_3,afp=s_lhs_3, $
                func=func,sky_func=sky_func_lhs_3,cal_func=cal_func_lhs_3, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_lhs_3, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_lhs_3,/pure,/mjy) 
;

IF (MAX(c_lhs_3) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_lhs_3,gamma=gamma_lhs_3,lmat=lmat
;
...
; Make LHS_3_EJG Save Set
; -----------------------
chan_label = 'LHS_3'
sname='csdr$firas_out:lhs_3_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi3,ejg_lhs_3, $
 c_lhs_3,ndf_ejg_lhs_3,chi2_lhs_3,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3_EJG.ISS" Created.'
;

; Make LHS_3_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhs_3_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi3,c_lhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3_ERRORS.ISS" Created.'
;

; Make LHS_3_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:lhs_3_skymap.iss'
freq_lhs_3 = f_hi3
SAVE,filename=sname,chan_label,stripes_descrip,freq_lhs_3,l_lhs,b_lhs,n_lhs, $
                    c_lhs_3,s_lhs_3,z_lhs_3,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make LHS_3_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:lhs_3_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_lhs_3,cal_func_lhs_3, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3_FUNC.ISS" Created.'
PRINT,' '
;

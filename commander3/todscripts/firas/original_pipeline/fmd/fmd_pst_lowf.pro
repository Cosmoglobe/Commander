...
;  FMD_PST_LOWF creates and saves the binary records of LOWF stripe arrays
...
if N_Params() ne 1 then begin
 print,' '
 print,'FMD_PST_LOWF : Called Incorrectly : FMD_PST_LOWF,error'
 print,' '
 return
endif
;
...
descrip = [descrip,'LLSS_DIHEDRAL','LLSS_BOLOMETER','LLSS_6K_HORN']
descrip = [descrip,'LLSS_4K_HORN','LLSS_MISSION']
descrip = [descrip,'RLSS_DIHEDRAL','RLSS_BOLOMETER','RLSS_6K_HORN']
descrip = [descrip,'RLSS_4K_HORN','RLSS_MISSION']
descrip = [descrip,'LLFA_DIHEDRAL','LLFA_BOLOMETER','LLFA_6K_HORN']
descrip = [descrip,'LLFA_4K_HORN','LLFA_MISSION']
descrip = [descrip,'RLFA_DIHEDRAL','RLFA_BOLOMETER','RLFA_6K_HORN']
descrip = [descrip,'RLFA_4K_HORN','RLFA_MISSION']
;
...
IF (nrec NE N_ELEMENTS(descrip)) THEN BEGIN
 PRINT,'FMD_PST_LOWF : Error in Number of Records !'
 RETURN
ENDIF
;
...
FOR i=0,nrec-1 DO BEGIN
 ...
 pst_rec(i).str_spec(0:nf-1) = FLOAT(ejg_lowf(*,i))
...
; -------------------------
sky_wgts_ds=0 & chi2_loch=0 & diag=0 & omega=0 & square=0
...
f_lo=0 & c_lowf=0 & ndf_ejg_lowf=0
;

Pro FMD_CHI2_LLF,error
;

;
;  FMD_CHI2_LLF drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for LLLF offset destriped data .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLF_DSP.ISS, LLF_WEIGHTS.ISS,
;                        and LLF_EJG.ISS, and where output save sets 
;                        LLF_CVECTOR.ISS, LLF_SKYMAP_2.ISS, LLF_SKY_RESID.ISS,
;                        LLF_SKY_CHI2.ISS, LLF_CAL_RESID.ISS,
;                        and LLF_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_LLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_LLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_LLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLF_DSP.ISS'
RESTORE,'csdr$firas_out:llf_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_LLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hr(nf)
...
; Restore LLF_EJG Save Set
; ------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLF_EJG.ISS'
RESTORE,'csdr$firas_out:llf_ejg.iss'
;
...
; Make LLF_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:llf_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_llf,sp_llf,cvec_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make LLF_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:llf_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make LLF_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:llf_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_llf,cvec_llf,ndf_cvec_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make LLF_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:llf_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_llf,cvec_llf,sky_chi2_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make LLF_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:llf_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make LLF_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:llf_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_llf,cvec_llf,$
                    cal_chi2_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_CAL_CHI2.ISS" Created.'
PRINT,' '
;

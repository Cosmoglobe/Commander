Pro FMD_CHI2_LHF_4X,error
;

;
;  FMD_CHI2_LHF_4X drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for LHFA Band_4 offset destriped data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHF_4X_DSP.ISS, LHFX_WEIGHTS.ISS,
;                        and LHF_4X_EJG.ISS, and where output save sets 
;                        LHF_4X_CVECTOR.ISS, LHF_4X_SKY_RESID.ISS,
;                        LHF_4X_SKY_CHI2.ISS, LHF_4X_CAL_RESID.ISS,
;                        and LHF_4X_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_LHF_4X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_LHF_4X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_LHF_4X,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_LHF_4X,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_4X_DSP.ISS'
RESTORE,'csdr$firas_out:lhf_4x_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_LHF_4X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi4(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_LHF_4X : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore LHF_4X_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_4X_EJG.ISS'
RESTORE,'csdr$firas_out:lhf_4x_ejg.iss'
;
...
; Make LHF_4X_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:lhf_4x_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make LHF_4X_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:lhf_4x_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_lhf_4,cvec_lhf_4,ndf_cvec_lhf_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make LHF_4X_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:lhf_4x_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_lhf_4,cvec_lhf_4,sky_chi2_lhf_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make LHF_4X_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:lhf_4x_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make LHF_4X_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:lhf_4x_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_lhf_4,cvec_lhf_4,$
                    cal_chi2_lhf_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_4X_CAL_CHI2.ISS" Created.'
PRINT,' '
;

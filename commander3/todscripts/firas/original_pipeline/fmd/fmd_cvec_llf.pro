Pro FMD_CVEC_LLF,error
;

;
;  FMD_CVEC_LLF drives the FMD_CVECTOR procedure to compute
;  the LLLF C-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;                        LLF_DVECTOR.ISS, and LLF_EJG.ISS, and where
;                        LLF_CVECTOR_2.ISS will be sent .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CVEC_LLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CVEC_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CVEC_LLF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CVEC_LLF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CVEC_LLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hr(nf)
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLF_DVECTOR.ISS'
RESTORE,'csdr$firas_out:llf_dvector.iss'
;
...
cvec_llf = $
 FMD_CVECTOR(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
             ...
             max_frac=max_frac,good_dvec=good_dvec_llf,$
             ndf_cvec=ndf_cvec_llf,good_cvec=good_cvec_llf)
;

; Make LLF_CVECTOR_2.ISS Save Set
; -------------------------------
chanscan = 'LLF'
freq_dependence = 'N'
freq_llf = f_hr
sname = 'csdr$firas_out:llf_cvector_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_llf,cvec_llf,ndf_cvec_llf,good_cvec_llf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_CVECTOR_2.ISS" Created.'
PRINT,' '
;
...
n_llf=0 & b_llf=0 & l_llf=0 & d_llf=0 & dvec_llf=0 & ndf_dvec_llf=0
;

Pro FMD_DSP_RHS_4X,error
;

;
;  FMD_DSP_RHS_4X drives the FMD_APPLY procedure to apply destriper
;  corrections to RHSS Band_4 data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHS_4X_EJG.ISS and RHS_4_FUNC.ISS,
;                        and where RHS_4X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RHS_4X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RHS_4X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RHS_4X,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_4_FUNC.ISS'
RESTORE,'csdr$firas_out:rhs_4_func.iss'
;
...
IF (cf ne N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RHS_4X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_rhs_4],[cal_func_rhs_4]]
;
...
dummy = FMD_APPLY(freq=f_hi4,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_rhs_4,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'RHS_4'
sname = 'csdr$firas_out:rhs_4x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi4,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4X_DSP.ISS" Created.'
PRINT,' '
;
...
c_rhs_4=0 & ndf_ejg_rhs_4=0 & chi2_rhs_4=0
;

Pro FMD_Bad_RSF,badcoadd_rsf,badcal_rsf,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad_RSF,badcoadd_rsf,badcal_rsf,error'
 error = 1
 return
endif
;
...
RESTORE,'csdr$firas_out:rsfx_sky_chi2.iss'
...
RESTORE,'csdr$firas_out:rsfx_cal_chi2.iss'
cwd0 = TEMPORARY(cal_wgts_ds)
IF (MAX(badcal_old) GE 0) THEN BEGIN
 dummy = MAX(ABS(cwd0(badcal_old)))
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD_RSF : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;

ct = TOTAL(sky_chi2_rsf,1)
cct = TOTAL(cal_chi2_rsf,1)
;
...
freq_rsf=0 & cvec_rsf=0
badcoadd_rhf=0 & badcoadd_lhs=0 & badcoadd_rhs=0 & badcoadd_rlf=0
badcoadd_lls=0 & badcoadd_rls=0 & badcoadd_lsf=0 & badcoadd_lhf=0
badcoadd_llf=0 & badcal_rhf=0 & badcal_lhs=0 & badcal_rhs=0 & badcal_rlf=0 
badcal_llf=0 & badcal_lls=0 & badcal_rls=0 & badcal_lsf=0 & badcal_lhf=0
;

Pro FMD_CVEC_RLS,error
;

;
;  FMD_CVEC_RLS drives the FMD_CVECTOR procedure to compute
;  the RLSS C-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;                        RLS_DVECTOR.ISS, and RLS_EJG.ISS, and where
;                        RLS_CVECTOR_2.ISS will be sent .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CVEC_RLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CVEC_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CVEC_RLS,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CVEC_RLS,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CVEC_RLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLS_DVECTOR.ISS'
RESTORE,'csdr$firas_out:rls_dvector.iss'
;
...
cvec_rls = $
 FMD_CVECTOR(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
             ...
             max_frac=max_frac,good_dvec=good_dvec_rls,$
             ndf_cvec=ndf_cvec_rls,good_cvec=good_cvec_rls)
;

; Make RLS_CVECTOR_2.ISS Save Set
; -------------------------------
chanscan = 'RLS'
freq_dependence = 'N'
freq_rls = f_lo
sname = 'csdr$firas_out:rls_cvector_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rls,cvec_rls,ndf_cvec_rls,good_cvec_rls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLS_CVECTOR_2.ISS" Created.'
PRINT,' '
;
...
n_rls=0 & b_rls=0 & l_rls=0 & d_rls=0 & dvec_rls=0 & ndf_dvec_rls=0
;

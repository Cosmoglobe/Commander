Pro FMD_DSP_HIGH_4,error
;

;
;  FMD_DSP_HIGH_4 drives the FMD_APPLY procedure to apply destriper
;  corrections to combined high channel Band_4 data.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Required Logicals  :
;
;    CSDR$FIRAS_IN   =  Directory containing HIGH_4_CALSPEC_ZSUB.ISS
;                       and HIGH.ISS .
;
;    CSDR$FIRAS_OUT  =  Directory containing HIGH_4_EJG.ISS and HIGH_4_FUNC.ISS,
;                       and where HIGH_4_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_HIGH_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_HIGH_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_HIGH_4,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'HIGH_4_FUNC.ISS'
RESTORE,'csdr$firas_out:high_4_func.iss'
;
...
; Restore HIGH_4 Undestriped Zodi-Subtracted Spectra
; --------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_HIGH_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_high_4],[cal_func_high_4]]
;
...
dummy = FMD_APPLY(freq=f_hi4,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_high_4,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'HIGH_4'
sname = 'csdr$firas_out:high_4_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi4,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_DSP.ISS" Created.'
PRINT,' '
;
...
gain_convg=0 & gain_iter=0 & chi2_hi4ch=0 & cal_tm=0 & fsl_idx=0
scan=0 & sky_wgts_fsl=0 & cal_wgts_fsl=0 & cvec_mask=0 & cvec_cut=0
c_high_4=0 & ndf_ejf_high_4=0 & chi2_high_4=0
n_high_4=0 & l_high_4=0 & b_high_4=0 & gamma_high_4=0
...
tmin=0 & tmax=0 & good_cal_dihd=0 & ndf_ejg_high_4=0 & dihed_cut_min=0
;

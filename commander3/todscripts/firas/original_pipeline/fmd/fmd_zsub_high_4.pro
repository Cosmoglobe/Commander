...
;
;    CSDR$FIRAS_IN     =  Directory containing xxx_4_CALSPEC.ISS and
;                         xxx_4_ZODISPEC.ISS, and where output IDL save
;                         set HIGH_4_CALSPEC_ZSUB.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_ZSUB_HIGH_4,error
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_ZSUB_HIGH_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_ZSUB_HIGH_4,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_ZSUB_HIGH_4,error'
 PRINT,' '
 RETURN
ENDIF
;
...
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'LHS_4_CALSPEC.ISS'
RESTORE,'csdr$firas_in:lhs_4_calspec.iss'
PRINT,' '
;
fx = f_hi4
...
;
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'LHS_4_ZODISPEC.ISS'
RESTORE,'csdr$firas_in:lhs_4_zodispec.iss'
PRINT,' '
;
nf = WHERE(fx NE f_hi4,cf)
IF (cf GT 0) THEN BEGIN
 PRINT,''
 PRINT,'LHS_4 : CALSPEC/ZODISPEC Frequency Mismatch !'
 PRINT,''
 RETURN
ENDIF
;
...
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'RHS_4_CALSPEC.ISS'
RESTORE,'csdr$firas_in:rhs_4_calspec.iss'
PRINT,' '
;
fx = f_hi4
nf = WHERE((fx ge freq_band(0))and(fx le freq_band(1)),cf)
...
;
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'RHS_4_ZODISPEC.ISS'
...
IF (cf GT 0) THEN BEGIN
 PRINT,''
 PRINT,'RHS_4 : CALSPEC/ZODISPEC Frequency Mismatch !'
 PRINT,''
 RETURN
ENDIF
;
...
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'LHF_4_CALSPEC.ISS'
RESTORE,'csdr$firas_in:lhf_4_calspec.iss'
PRINT,' '
;
fx = f_hi4
nf = WHERE((fx ge freq_band(0))and(fx le freq_band(1)),cf)
...
;
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'LHF_4_ZODISPEC.ISS'
...
IF (cf GT 0) THEN BEGIN
 PRINT,''
 PRINT,'LHF_4 : CALSPEC/ZODISPEC Frequency Mismatch !'
 PRINT,''
 RETURN
ENDIF
;
...
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'RHF_4_CALSPEC.ISS'
RESTORE,'csdr$firas_in:rhf_4_calspec.iss'
PRINT,' '
;
fx = f_hi4
nf = WHERE((fx ge freq_band(0))and(fx le freq_band(1)),cf)
...
;
PRINT,' '
PRINT,'Restoring ' + intrans(0) + 'RHF_4_ZODISPEC.ISS'
...
IF (cf GT 0) THEN BEGIN
 PRINT,''
 PRINT,'RHF_4 : CALSPEC/ZODISPEC Frequency Mismatch !'
 PRINT,''
 RETURN
ENDIF
;
...
sname = 'csdr$firas_in:high_4_calspec_zsub.iss'
SAVE,filename=sname,f_hi4,spec,cal_spec
PRINT,' '
PRINT,'IDL Save Set "' + intrans(0) + 'HIGH_4_CALSPEC_ZSUB.ISS" Created.'
PRINT,' '
;

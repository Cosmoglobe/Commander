Pro FMD_WGTS_LHS,model_var,error
;
;
;  FMD_WGTS_LHS drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of LHSS weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where LHS_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LHS.ISS and LHS_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where LHS_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_LHS,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_LHS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LHS,model_var,error'
 PRINT,' '
...
;
 ...
 ; ---------------------------
 fidx = WHERE((f_lhs GT freq_band(0))and(f_lhs LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_LHS : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vw = 1. / d_lhs(fidx) / d_lhs(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vt = ( TRANSPOSE(var_lhs(fidx,*)) # vw ) / TOTAL(vw)           ; SKY
 cal_vt = ( TRANSPOSE(cal_var_lhs(fidx,*)) # vw ) / TOTAL(vw)   ; CAL
 ;
 ...
 IF (cgood LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LHS : No Good Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;
 ...
 chanscan = 'LHS'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:lhs_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_lhs,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'LHS_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make LHS_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LHS'
sname = 'csdr$firas_ref:lhs_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LHS_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_lhs=0 & n_lhs=0 & b_lhs=0 & l_lhs=0 & f_lhs=0 & galcut_lhs=0

Pro FMD_CHI2_RSFX,error
;

;
;  FMD_CHI2_RSFX drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RLFA offset destriped data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_IN    =  Directory containing RSF.ISS
;
;    CSDR$FIRAS_OUT   =  Directory containing RSFX_DSP.ISS, RSFX_WEIGHTS.ISS,
;                        and RSFX_EJG.ISS, and where output save sets 
;                        RSFX_CVECTOR.ISS, RSFX_SKY_RESID.ISS,
;                        RSFX_SKY_CHI2.ISS, RSFX_CAL_RESID.ISS,
;                        and RSFX_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RSFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RSFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RSFX,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RSFX,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RSFX_DSP.ISS'
RESTORE,'csdr$firas_out:rsfx_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RSFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; Restore RSFX_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RSFX_EJG.ISS'
RESTORE,'csdr$firas_out:rsfx_ejg.iss'
;
...
cvec_rsf = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_rsf,resid=sky_resid_rsf,$
          chi2=sky_chi2_rsf,ndf_cvec=ndf_cvec_rsf)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_lo,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_rsf,resid=cal_resid_rsf,$
              chi2=cal_chi2_rsf)
;
...
; Make RSFX_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rsfx_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_rsf,sky_resid_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RSFX_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:rsfx_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rsf,cvec_rsf,ndf_cvec_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RSFX_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rsfx_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rsf,cvec_rsf,sky_chi2_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RSFX_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rsfx_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rsf,cal_resid_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RSFX_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rsfx_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rsf,cvec_rsf,cal_chi2_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_CAL_CHI2.ISS" Created.'
PRINT,' '
;
...
n_rsf=0 & b_rsf=0 & l_rsf=0 & d_rsf=0

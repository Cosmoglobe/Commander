Pro FMD_DVEC_LHS_3,error
;

;
;  FMD_DVEC_LHS_3 drives the FMD_DVECTOR procedure to compute
;  the LHSS Band_3 D-Vector .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHS_3_CVECTOR.ISS and LHS_3_EJG.ISS,
;                        where output LHS_3_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_LHS_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_LHS_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_LHS_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_LHS_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHS_3_CVECTOR.ISS'
...
IF (cf NE N_ELEMENTS(freq_lhs_3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_LHS_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_lhs_3 = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_lhs_3,ndf_dvec=ndf_dvec_lhs_3)
;

; Make LHS_3_DVECTOR.ISS Save Set
; -------------------------------
chanscan = 'LHS_3'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:lhs_3_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_lhs_3,$
                    dvec_lhs_3,good_dvec_lhs_3,ndf_dvec_lhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_3_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_lhs=0 & b_lhs=0 & l_lhs=0 & d_lhs=0 & f_hi3=0
cvec_lhs_3=0 & ndf_cvec_lhs_3=0 & galcut_lhs=0 & cal_var_lhs=0
;

Pro FMD_DSP_RSFX,error
;

;
;  FMD_DSP_RSFX drives the FMD_APPLY procedure to apply destriper
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RSFX_EJG.ISS and RSF_FUNC.ISS,
;                        and where RSFX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RSFX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RSFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RSFX,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RSFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'RSF'
sname = 'csdr$firas_out:rsfx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSFX_DSP.ISS" Created.'
PRINT,' '
;

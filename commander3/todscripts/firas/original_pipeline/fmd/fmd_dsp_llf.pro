Pro FMD_DSP_LLF,error
;

;
;  FMD_DSP_LLF drives the FMD_APPLY procedure to apply destriper
;  corrections to LLLF data .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLF_EJG.ISS and LLF_FUNC.ISS,
;                        and where LLF_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LLF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LLF,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLF_FUNC.ISS'
RESTORE,'csdr$firas_out:llf_func.iss'
;
...
; Restore IDL Save Set of LLF Undestriped Spectra
; -------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_hr)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LLF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_llf],[cal_func_llf]]
;
...
dummy = FMD_APPLY(freq=f_hr,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_llf,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'LLF'
sname = 'csdr$firas_out:llf_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hr,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLF_DSP.ISS" Created.'
PRINT,' '
;
...
c_llf=0 & ndf_ejg_llf=0 & chi2_llf=0
;

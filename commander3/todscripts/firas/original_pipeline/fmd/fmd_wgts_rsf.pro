Pro FMD_WGTS_RSF,model_var,error
;
;
;  FMD_WGTS_RSF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of RLFA weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        RSF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RSF.ISS and RSF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where RSF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_RSF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_RSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RSF,model_var,error'
 PRINT,' '
...
; -----------------------------
nr = [7,100]  ;  Range of "Good" Coadd Sizes
;
...
PRINT,' '
PRINT,'BADCOADD_RSF =',badcoadd_rsf     ; Bad RSF SKY Coadds
PRINT,' '
PRINT,'BADCAL_RSF =',badcal_rsf         ; Bad RSF CAL Coadds
PRINT,' '
PRINT,' '
;
...
IF (badcoadd_rsf(0) ge 0) THEN badcoadd(badcoadd_rsf) = 1B
;

; Build BADCAL Index
; --------------------
badcal = BYTARR(n_cal)
IF (badcal_rsf(0) ge 0) THEN badcal(badcal_rsf) = 1B
;
...
;
 ...
 ; ---------------------------
 fidx = WHERE((f_rsf GT freq_band(0))and(f_rsf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_RSF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vws = 1. / d_rlfs(fidx) / d_rlfs(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vwl = 1. / d_rlfl(fidx) / d_rlfl(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 ;
 vts = ( TRANSPOSE(var_rsf(fidx,*)) # vws ) / TOTAL(vws)           ; SKY
 cal_vts = ( TRANSPOSE(cal_var_rsf(fidx,*)) # vws ) / TOTAL(vws)   ; CAL
 ;
 vtl = ( TRANSPOSE(var_rsf(fidx,*)) # vwl ) / TOTAL(vwl)           ; SKY
 cal_vtl = ( TRANSPOSE(cal_var_rsf(fidx,*)) # vwl ) / TOTAL(vwl)   ; CAL
 ;
 ...
 nl = WHERE(sky_lbl EQ 'RLFL',cl)
 vt(nl) = vtl(nl)
 ;
 ns = WHERE(sky_lbl EQ 'RLFS',cs)
 vt(ns) = vts(ns)
 ;
 nlc = WHERE(cal_lbl EQ 'RLFL',clc)
 cal_vt(nlc) = cal_vtl(nlc)
 ;
 nsc = WHERE(cal_lbl EQ 'RLFS',csc)
 ...
 IF (cgoodl LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RSF : No Good RLFL Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked RLFL Coadds
 ; --------------------------------------------------
 tml = tm(nl(goodl))
 ...
 IF (cgoods LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RSF : No Good RLFS Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;

 ; Mean Variance per IFG for Good Unmasked RLFS Coadds
 ; --------------------------------------------------
 tms = tm(ns(goods))
 ...
 ;
 PRINT,'Model Will Be Computed from' + sgoods + ' RLFS Coadds'
 PRINT,'                        and' + sgoodl + ' RLFL Coadds'
 PRINT,' '
 ;
 ...
 chanscan = 'RSF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:rsf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_rsf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod,$
                     avar2l,avar2s,sl_weight_rat,sky_lbl,cal_lbl
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'RSF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make RSF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'RSF'
sname = 'csdr$firas_ref:rsf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RSF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_rsf=0 & n_rsf=0 & b_rsf=0 & l_rsf=0 & f_rsf=0 & galcut_rsf=0
galcut_rlfl=0 & galcut_rlfs=0 & f_rlfl=0 & f_rlfs=0 & var_rlfl=0 & var_rlfs=0
cal_var_rlfl=0 & cal_var_rlfs=0 & var_tm=0 & cal_var_tm=0 & var_lbl=0
cal_var_lbl=0
;

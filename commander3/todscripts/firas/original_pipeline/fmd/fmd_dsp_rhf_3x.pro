Pro FMD_DSP_RHF_3X,error
;

;
;  FMD_DSP_RHF_3X drives the FMD_APPLY procedure to apply destriper
;  corrections to RHFA Band_3 data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHF_3X_EJG.ISS and RHF_3_FUNC.ISS,
;                        and where RHF_3X_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RHF_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RHF_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RHF_3X,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHF_3_FUNC.ISS'
RESTORE,'csdr$firas_out:rhf_3_func.iss'
;
...
IF (cf ne N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RHF_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_rhf_3],[cal_func_rhf_3]]
;
...
dummy = FMD_APPLY(freq=f_hi3,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_rhf_3,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'RHF_3'
sname = 'csdr$firas_out:rhf_3x_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_hi3,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_3X_DSP.ISS" Created.'
PRINT,' '
;
...
glon=0 & glat=0 & sky_lbl=0 & cal_lbl=0
...
c_rhf_3=0 & ndf_ejg_rhf_3=0 & chi2_rhf_3=0
;

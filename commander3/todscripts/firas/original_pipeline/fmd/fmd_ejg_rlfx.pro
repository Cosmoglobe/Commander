Pro FMD_EJG_RLFX,error
;

;
;  FMD_EJG_RLFX drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_RLF.ISS, 
;                        and RLFX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    RLFX_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RLFX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RLFX,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RLFX : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RLFX : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RLFX : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RLFX : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RLFX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_rlf.iss'
;
...
; Make RLFX_EJG Save Set
; ------------------------
chan_label = 'RLF'
sname='csdr$firas_out:rlfx_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLFX_EJG.ISS" Created.'
;
...
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rlf=0 & n_rlf=0 & b_rlf=0 & l_rlf=0
...
badcoadd_lls=0 & badcoadd_llf=0 & badcal_lls=0 & badcal_llf=0
;

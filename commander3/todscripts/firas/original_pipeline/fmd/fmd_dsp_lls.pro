Pro FMD_DSP_LLS,error
;

;
;  FMD_DSP_LLS drives the FMD_APPLY procedure to apply destriper
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LLS_EJG.ISS and LLS_FUNC.ISS,
;                        and where LLS_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_LLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_LLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_LLS,error'
 PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_LLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------
chanscan = 'LLS'
sname = 'csdr$firas_out:lls_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_DSP.ISS" Created.'
PRINT,' '
;

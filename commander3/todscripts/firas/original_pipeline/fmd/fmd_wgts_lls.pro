Pro FMD_WGTS_LLS,model_var,error
;
;
;  FMD_WGTS_LLS drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of LLSS weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        LLS_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LLS.ISS and LLS_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where LLS_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_LLS,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_LLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LLS,model_var,error'
 PRINT,' '
...
;
 ...
 ; ---------------------------
 fidx = WHERE((f_lls GT freq_band(0))and(f_lls LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_LLS : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vw = 1. / d_lls(fidx) / d_lls(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vt = ( TRANSPOSE(var_lls(fidx,*)) # vw ) / TOTAL(vw)           ; SKY
 cal_vt = ( TRANSPOSE(cal_var_lls(fidx,*)) # vw ) / TOTAL(vw)   ; CAL
 ;
 ...
 IF (cgood LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LLS : No Good Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;
 ...
 chanscan = 'LLS'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:lls_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_lls,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'LLS_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make LLS_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LLS'
sname = 'csdr$firas_ref:lls_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LLS_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_lls=0 & n_lls=0 & b_lls=0 & l_lls=0 & f_lls=0 & galcut_lls=0

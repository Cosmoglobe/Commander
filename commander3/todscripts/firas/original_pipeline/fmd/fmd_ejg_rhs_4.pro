Pro FMD_EJG_RHS_4,error
;

;
;  FMD_EJG_RHS_4 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of RHSS Band_4 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Output  :
;
;    RHS_4_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    RHS_4_ERRORS.ISS, contains destriper error matrices.
;
;    RHS_4_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RHS_4_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RHS_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RHS_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RHS_4,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RHS_4 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RHS_4 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RHS_4 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RHS_4 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RHS_4 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RHS_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_RHS_4 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
;
PIX2XY,cmp_px,data=dummy,res=6,/six,ras=z_rhs_4
;
...
c_rhs_4 = $
  FMD_DESTRIPER(freq=f_hi4,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rhs_4,afp=s_rhs_4, $
                func=func,sky_func=sky_func_rhs_4,cal_func=cal_func_rhs_4, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rhs_4, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rhs_4,/pure,/mjy) 
;

IF (MAX(c_rhs_4) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rhs_4,gamma=gamma_rhs_4,lmat=lmat
;
...
; Make RHS_4_EJG Save Set
; -----------------------
chan_label = 'RHS_4'
sname='csdr$firas_out:rhs_4_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi4,ejg_rhs_4, $
 c_rhs_4,ndf_ejg_rhs_4,chi2_rhs_4,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_EJG.ISS" Created.'
;

; Make RHS_4_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhs_4_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi4,c_rhs_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_ERRORS.ISS" Created.'
;

; Make RHS_4_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rhs_4_skymap.iss'
freq_rhs_4 = f_hi4
SAVE,filename=sname,chan_label,stripes_descrip,freq_rhs_4,l_rhs,b_rhs,n_rhs, $
                    c_rhs_4,s_rhs_4,z_rhs_4,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RHS_4_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rhs_4_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rhs_4,cal_func_rhs_4, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_4_FUNC.ISS" Created.'
PRINT,' '
;

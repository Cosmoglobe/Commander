Pro FMD_CVEC_LLS,error
;

;
;  FMD_CVEC_LLS drives the FMD_CVECTOR procedure to compute
;  the LLSS C-Vector .
;
;
;  ARGUMENTS (I/O)    :
...
;                        LLS_DVECTOR.ISS, and LLS_EJG.ISS, and where
;                        LLS_CVECTOR_2.ISS will be sent .
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CVEC_LLS,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CVEC_LLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CVEC_LLS,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CVEC_LLS,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CVEC_LLS : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LLS_DVECTOR.ISS'
RESTORE,'csdr$firas_out:lls_dvector.iss'
;
...
cvec_lls = $
 FMD_CVECTOR(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
             ...
             max_frac=max_frac,good_dvec=good_dvec_lls,$
             ndf_cvec=ndf_cvec_lls,good_cvec=good_cvec_lls)
;

; Make LLS_CVECTOR_2.ISS Save Set
; -------------------------------
chanscan = 'LLS'
freq_dependence = 'N'
freq_lls = f_lo
sname = 'csdr$firas_out:lls_cvector_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_lls,cvec_lls,ndf_cvec_lls,good_cvec_lls
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLS_CVECTOR_2.ISS" Created.'
PRINT,' '
;
...
n_lls=0 & b_lls=0 & l_lls=0 & d_lls=0 & dvec_lls=0 & ndf_dvec_lls=0
;

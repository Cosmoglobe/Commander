Pro FMD_DSP_RLSX,error
;

;
;  FMD_DSP_RLSX drives the FMD_APPLY procedure to apply destriper
;  corrections to RLSS data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RLSX_EJG.ISS and RLS_FUNC.ISS,
;                        and where RLSX_DSP.ISS will be sent.
;
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DSP_RLSX,error
;
;
...
IF N_Params() ne 1 THEN BEGIN
 PRINT,'FMD_DSP_RLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DSP_RLSX,error'
 PRINT,' '
...
;
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RLS_FUNC.ISS'
RESTORE,'csdr$firas_out:rls_func.iss'
;
...
; Restore IDL Save Set of RLS Undestriped Spectra
; -------------------------------------------------
PRINT,' '
...
IF (cf ne N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DSP_RLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; ---------------------------
func = [[sky_func_rls],[cal_func_rls]]
;
...
dummy = FMD_APPLY(freq=f_lo,func=func,n_krnl=n_krnl,n_cmn=n_cmn, $
   		  ...
   		  ejg=ejg_rls,sky_idx=sky_idx,cal_idx=cal_idx,$
...
; -----------------
chanscan = 'RLS'
sname = 'csdr$firas_out:rlsx_dsp.iss'
SAVE,filename=sname,chanscan,stripes_descrip,f_lo,dsp,cal_dsp
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RLSX_DSP.ISS" Created.'
PRINT,' '
;
...
c_rls=0 & ndf_ejg_rls=0 & chi2_rls=0
;

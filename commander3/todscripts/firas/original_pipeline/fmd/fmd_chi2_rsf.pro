Pro FMD_CHI2_RSF,error
;

;
;  FMD_CHI2_RSF drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RSF_DSP.ISS, RSF_WEIGHTS.ISS,
;                        and RSF_EJG.ISS, and where output save sets 
;                        RSF_CVECTOR.ISS, RSF_SKYMAP_2.ISS, RSF_SKY_RESID.ISS,
;                        RSF_SKY_CHI2.ISS, RSF_CAL_RESID.ISS,
;                        and RSF_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RSF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RSF,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RSF,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RSF_DSP.ISS'
RESTORE,'csdr$firas_out:rsf_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_lo)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RSF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_lo(nf)
...
; Make RSF_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rsf_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_rsf,sp_rsf,cvec_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make RSF_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rsf_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RSF_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:rsf_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rsf,cvec_rsf,ndf_cvec_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RSF_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rsf_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rsf,cvec_rsf,sky_chi2_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RSF_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:rsf_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RSF_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rsf_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,cal_s0,$
                    cal_wgts_ds,cal_wgts,freq_rsf,cvec_rsf,cal_chi2_rsf
                    
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_CAL_CHI2.ISS" Created.'
PRINT,' '
;

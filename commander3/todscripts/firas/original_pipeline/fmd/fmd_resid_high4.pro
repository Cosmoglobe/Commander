Pro FMD_RESID_HIGH4,error
;
;
;  FMD_RESID_HIGH4 drives the FMD_RESID procedure to compute pixel spectra
;  and coadd residuals for combined high channel Band_4 destriped data,
...
;                        HIGH_4_DSP.ISS, and where IDL save set
;                        HIGH_4_SKY_RESID.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_RESID_HIGH4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_RESID_HIGH4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_RESID_HIGH4,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_RESID_HIGH4,error'
 PRINT,' '
 RETURN
ENDIF
;
...
IF (cf NE N_ELEMENTS(f_hi4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_RESID_HIGH4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (freq_dependence EQ 'Y') THEN BEGIN
 ...
 IF (cf NE N_ELEMENTS(freq)) THEN BEGIN
  PRINT,'FMD_RESID_HIGH4 : C-Vector Frequencies are NOT Compatible !'
  error = 1
  RETURN
 ENDIF
 ;
...
IF (freq_dependence EQ 'Y') THEN error = $
    FMD_RESID(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,sky_idx=sky_idx,$
              ...
              px_spec=sp_high_4,resid=sky_resid_high4)
;
...
; Make HIGH_4_SKY_RESID.ISS Save Set
; ----------------------------------
chanscan = 'HIGH_4'
chan_label = 'LHS_RHS_LHF_RHF'
freq_high_4 = f_hi4
;
sname = 'csdr$firas_out:high_4_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,$
                    ...
                    sky_resid_high4,sp_high_4,z_high_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HIGH_4_SKY_RESID.ISS" Created.'
PRINT,' '
;

Pro FMD_EJG_LLSX,error
;

;
;  FMD_EJG_LLSX drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_LLS.ISS, 
;                        and LLSX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    LLSX_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_LLSX : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_LLSX,error'
 PRINT,' '
...
; ---------------------
PRINT,' '
PRINT,'Restoring ' + reftrans(0) + 'LLSX_WEIGHTS.ISS'
RESTORE,'csdr$firas_ref:llsx_weights.iss'
;
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_LLSX : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_LLSX : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_LLSX : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_LLSX : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_LLSX : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_lls.iss'
;
...
; Make LLSX_EJG Save Set
; ------------------------
chan_label = 'LLS'
sname='csdr$firas_out:llsx_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LLSX_EJG.ISS" Created.'
;
...
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_lls=0 & n_lls=0 & b_lls=0 & l_lls=0

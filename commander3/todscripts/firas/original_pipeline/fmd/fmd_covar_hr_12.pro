Pro FMD_COVAR_HR_12,error
;

;
;  FMD_COVAR_HR_12 drives the FMD_COVAR procedure to create an IDL
;  save set of combined HRES Band_1/Band_2 covariance.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        and where output HRES_12_COVAR.ISS will be sent.
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_COVAR_HR_12 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_COVAR_HR_12,error'
 PRINT,' '
...
freq_hr_1 = freq_hres(fidx1)
fidx2 = INDGEN(60) + 60
resid2 = TEMPORARY(sky_resid_hres(fidx2,*))
freq_hr_2 = freq_hres(fidx2)
;

nfreq = [N_ELEMENTS(freq_hr_1),N_ELEMENTS(freq_hr_2)]
;
...
error =  FMD_COVAR_OFF(nfreq=nfreq,resid1=resid1,resid2=resid2,$
                       ...
                       covar=covar_hres_12,ndf_covar=ndf_covar_hr_12)
;

IF (error NE 0) THEN BEGIN
 PRINT,'FMD_COVAR_HR_12 : Error Returned from FMD_COVAR_OFF !'
 RETURN
ENDIF
;

; Create HRES_12_COVAR Save Set
; -----------------------------
chanscan = 'HRES'
freq_dependence = 'N'
n_stripes = nst
sname = 'csdr$firas_out:hres_12_covar.iss'
SAVE,filename=sname,chanscan,freq_dependence,cvec_cut,n_stripes,max_frac,$
                    freq_hr_1,freq_hr_2,covar_hres_12,ndf_covar_hr_12
;
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'HRES_12_COVAR.ISS" Created.'
PRINT,' '
;

Pro FMD_EJG_RHF_2,error
;

;
;  FMD_EJG_RHF_2 drives the FMD_DESTRIPER procedure to create
;                 IDL save sets of RHFA Band_2 offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;  Output  :
;
;    RHF_2_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                  and C-Vector.
;
;    RHF_2_ERRORS.ISS, contains destriper error matrices.
;
;    RHF_2_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RHF_2_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RHF_2,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RHF_2 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RHF_2,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RHF_2 : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RHF_2 : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RHF_2 : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RHF_2 : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RHF_2 Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RHF_2 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_RHF_2 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
;
PIX2XY,cmp_px,data=dummy,res=6,/six,ras=z_rhf_2
;
...
c_rhf_2 = $
  FMD_DESTRIPER(freq=f_hi2,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rhf_2,afp=s_rhf_2, $
                func=func,sky_func=sky_func_rhf_2,cal_func=cal_func_rhf_2, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rhf_2, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rhf_2,/pure,/mjy) 
;

IF (MAX(c_rhf_2) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rhf_2,gamma=gamma_rhf_2,lmat=lmat
;
...
; Make RHF_2_EJG Save Set
; -----------------------
chan_label = 'RHF_2'
sname='csdr$firas_out:rhf_2_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_hi2,ejg_rhf_2, $
 c_rhf_2,ndf_ejg_rhf_2,chi2_rhf_2,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_2_EJG.ISS" Created.'
;

; Make RHF_2_ERRORS.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:rhf_2_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_hi2,c_rhf_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_2_ERRORS.ISS" Created.'
;

; Make RHF_2_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rhf_2_skymap.iss'
freq_rhf_2 = f_hi2
SAVE,filename=sname,chan_label,stripes_descrip,freq_rhf_2,l_rhf,b_rhf,n_rhf, $
                    c_rhf_2,s_rhf_2,z_rhf_2,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_2_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RHF_2_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rhf_2_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rhf_2,cal_func_rhf_2, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_2_FUNC.ISS" Created.'
PRINT,' '
;

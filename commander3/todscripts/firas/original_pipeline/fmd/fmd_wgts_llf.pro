Pro FMD_WGTS_LLF,model_var,error
;
;
;  FMD_WGTS_LLF drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
...
;                        FMD_BAD_COADD_HR.ISS, and where
;                        LLF_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing LLF.ISS and LLF_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where LLF_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_LLF,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_LLF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_LLF,model_var,error'
 PRINT,' '
...
; -----------------------------
nr = [5,100]  ;  Range of "Good" Coadd Sizes
;
...
;
 ...
 ; ------------------------------------
 PRINT,' '
 PRINT,'Restoring ' + intrans(0) + 'LLF_VAR.ISS'
 RESTORE,'csdr$firas_in:llf_var.iss'
 ;

 ; Index of "Good" Frequencies
 ; ---------------------------
 fidx = WHERE((f_llf GT freq_band(0))and(f_llf LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_LLF : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vw = 1. / d_llf(fidx) / d_llf(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vt = ( TRANSPOSE(var_llf(fidx,*)) # vw ) / TOTAL(vw)           ; SKY
 cal_vt = ( TRANSPOSE(cal_var_llf(fidx,*)) # vw ) / TOTAL(vw)   ; CAL
 ;
 ...
 IF (cgood LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_LLF : No Good Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;
 ...
 chanscan = 'LLF'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:llf_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_llf,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'LLF_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make LLF_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'LLF'
sname = 'csdr$firas_ref:llf_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'LLF_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_llf=0 & n_llf=0 & b_llf=0 & l_llf=0 & f_llf=0 & galcut_llf=0

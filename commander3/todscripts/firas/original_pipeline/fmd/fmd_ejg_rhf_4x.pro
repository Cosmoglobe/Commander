Pro FMD_EJG_RHF_4X,error
;

;
;  FMD_EJG_RHF_4X drives the FMD_DESTRIPER procedure to create
...
;                        FMD_QUALS_DEFAULT.ISS, FMD_DIRBE_FUNCX_RHF.ISS, 
;                        and RHFX_WEIGHTS.ISS.
;
...
;  Output  :
;
;    RHF_4X_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                    and C-Vector.
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RHF_4X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RHF_4X,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4X : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RHF_4X : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4X : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RHF_4X : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RHF_4X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_EJG_RHF_4X : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_funcx_rhf.iss'
;
...
; Make RHF_4X_EJG Save Set
; ------------------------
chan_label = 'RHF_4'
sname='csdr$firas_out:rhf_4x_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4X_EJG.ISS" Created.'
;
...
cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rhf=0 & n_rhf=0 & b_rhf=0 & l_rhf=0
badcoadd_lhs=0 & badcoadd_rhs=0 & badcoadd_lhf=0
badcal_lhs=0 & badcal_rhs=0 & badcal_lhf=0 & st_sub=0 & fsl_idx=0

Pro FMD_Bad2_LSF,badcoadd_lsf,badcal_lsf,error
;

if N_Params() ne 3 then begin
 print,'FMD_Bad2_LSF,badcoadd_lsf,badcal_lsf,error'
 error = 1
 return
endif
;
...
IF (dummy NE 0.) THEN BEGIN
 PRINT,'FMD_BAD2_LSF : Error in SKY_CHI2 Weights !'
 RETURN
ENDIF
;
...
IF (MAX(badcal_old) GE 0) THEN BEGIN
 ...
 IF (dummy NE 0.) THEN BEGIN
  PRINT,'FMD_BAD2_LSF : Error in CAL_CHI2 Weights !'
  RETURN
 ENDIF
ENDIF
;
...
badcoadd_lhs=0 & badcoadd_rls=0 & badcoadd_rsf=0 & badcoadd_lls=0
badcoadd_llf=0 & badcal_lhf=0 & badcal_rhs=0 & badcal_rhf=0 & badcal_rlf=0 
badcal_llf=0 & badcal_lhs=0 & badcal_rls=0 & badcal_rsf=0 & badcal_lls=0
;
error = 0
;

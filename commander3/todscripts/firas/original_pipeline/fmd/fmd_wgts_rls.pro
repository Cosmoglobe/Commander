Pro FMD_WGTS_RLS,model_var,error
;
;
;  FMD_WGTS_RLS drives the FMD_MODEL_WGT and FMD_PIXEL_WGT procedures
;  to create IDL save sets of RLSS weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_LO.ISS, and where
;                        RLS_WEIGHTS.ISS will be sent.
;
;    CSDR$FIRAS_IN    =  Directory containing RLS.ISS and RLS_VAR.ISS
;
;    CSDR$FIRAS_OUT   =  Directory where RLS_VAR_MODEL.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> model_var = 'Y'
;              UIDL> FMD_WGTS_RLS,model_var,error
;
;
...
IF N_Params() ne 2 THEN BEGIN 
 PRINT,'FMD_WGTS_RLS : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_WGTS_RLS,model_var,error'
 PRINT,' '
...
;
lp_order=[0]   ;  Order of Legendre Polynomials of Time
                           ;  (IF LE 0, no Legendre Terms )
;
tm_up = [0.]   ;  Start and Stop Times of Special Time Periods
tm_dn = [310.]   ;  (IF LE 0, disabled)
;
glmax = 2.6     ;  Maximum Glitch_Rate for "Good" Coadds
;
glcut = 0.55    ;  Minimum Glitch_Rate for Special High_Glitch Terms
;
...
;
 ...
 ; ------------------------------------
 PRINT,' '
 PRINT,'Restoring ' + intrans(0) + 'RLS_VAR.ISS'
 RESTORE,'csdr$firas_in:rls_var.iss'
 ;

 ; Index of "Good" Frequencies
 ; ---------------------------
 fidx = WHERE((f_rls GT freq_band(0))and(f_rls LT freq_band(1)),nf)
 IF (nf LT 1) THEN BEGIN
  PRINT,'FMD_WGTS_RLS : No Data in Frequency Range !'
  RETURN
 ENDIF
 ;
...
; Frequency Averaged Variances
 ; ----------------------------
 ;
 vw = 1. / d_rls(fidx) / d_rls(fidx)  ;  Frequency Weight = 1. / DVEC^2.
 vt = ( TRANSPOSE(var_rls(fidx,*)) # vw ) / TOTAL(vw)           ; SKY
 cal_vt = ( TRANSPOSE(cal_var_rls(fidx,*)) # vw ) / TOTAL(vw)   ; CAL
 ;
 ...
 IF (cgood LE 0) THEN BEGIN
  PRINT,'FMD_WGTS_RLS : No Good Coadds for Variance Model !'
  error = 1
  RETURN
 ENDIF
 ;
 ...
 var_coeff_rls = $
  FMD_MODEL_WGT(var2n=var2n,good=goodv,nifgs=nifgs,tm=tm,sky_glitch=sky_glitch,$
                ...
                glcut=glcut,sky_wgts=sky_wgts_mod,cal_wgts=cal_wgts_mod)
 ;

 chanscan = 'RLS'
 lp_orderv = lp_order
 sname = 'csdr$firas_out:rls_var_model.iss'
 SAVE,filename=sname,chanscan,freq_band,lp_orderv,tm_up,tm_dn,gpow,bpow,$
                     ...
                     condition,inv_stat,var_coeff_rls,delt,dtmod,skut,dscmod,$
                     sky_wgts_mod,cal_wgts_mod
                      
 PRINT,' '
 PRINT,'IDL Save Set "' + outtrans(0) + 'RLS_VAR_MODEL.ISS" Created.'
 PRINT,' '
 ;
...
; Make RLS_WEIGHTS.ISS Save Set
; --------------------------------
chanscan = 'RLS'
sname = 'csdr$firas_ref:rls_weights.iss'
...
PRINT,' '
PRINT,'IDL Save Set "' + reftrans(0) + 'RLS_WEIGHTS.ISS" Created.'
PRINT,' '
;
...
d_rls=0 & n_rls=0 & b_rls=0 & l_rls=0 & f_rls=0 & galcut_rls=0

Pro FMD_CHI2_LHS_2,error
;

;
;  FMD_CHI2_LHS_2 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for LHSS Band_2 offset destriped data, using frequency-independent
;  coadd weights.
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHS_2_DSP.ISS, LHS_WEIGHTS.ISS,
;                        and LHS_2_EJG.ISS, and where output save sets 
;                        LHS_2_CVECTOR.ISS, LHS_2_SKYMAP_2.ISS, LHS_2_SKY_CHI2.ISS
;                        and LHS_2_CAL_CHI2.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_LHS_2,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_LHS_2 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_LHS_2,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_LHS_2,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHS_2_DSP.ISS'
RESTORE,'csdr$firas_out:lhs_2_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi2)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_LHS_2 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi2(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_LHS_2 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore LHS_2_EJG Save Set
; -------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHS_2_EJG.ISS'
RESTORE,'csdr$firas_out:lhs_2_ejg.iss'
;
...
cvec_lhs_2 = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_lhs_2,resid=sky_resid_lhs_2,$
          chi2=sky_chi2_lhs_2,ndf_cvec=ndf_cvec_lhs_2)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi2,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_lhs_2,resid=cal_resid_lhs_2,$
              chi2=cal_chi2_lhs_2)
;
...
; Make LHS_2_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhs_2_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_lhs_2,sp_lhs_2,cvec_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make LHS_2_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:lhs_2_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_lhs_2,sky_resid_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make LHS_2_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:lhs_2_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_lhs_2,cvec_lhs_2,ndf_cvec_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make LHS_2_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhs_2_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_lhs_2,cvec_lhs_2,sky_chi2_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make LHS_2_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:lhs_2_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_lhs_2,cal_resid_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make LHS_2_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhs_2_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_lhs_2,cvec_lhs_2,$
                    cal_chi2_lhs_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHS_2_CAL_CHI2.ISS" Created.'
PRINT,' '
;
...
ejg_lhs_2=0 & c_lhs_2=0 & ndf_ejg_lhs_2=0 & chi2_lhs_2=0

Pro FMD_DVEC_RHF_4,error
;

;
;  FMD_DVEC_RHF_4 drives the FMD_DVECTOR procedure to compute
;  the RHFA Band_4 D-Vector .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHF_4_CVECTOR.ISS and RHF_4_EJG.ISS,
;                        where output RHF_4_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_RHF_4,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_RHF_4 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_RHF_4,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_RHF_4,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHF_4_CVECTOR.ISS'
...
IF (cf NE N_ELEMENTS(freq_rhf_4)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_RHF_4 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_rhf_4 = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_rhf_4,ndf_dvec=ndf_dvec_rhf_4)
;

; Make RHF_4_DVECTOR.ISS Save Set
; -------------------------------
chanscan = 'RHF_4'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:rhf_4_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_rhf_4,$
                    dvec_rhf_4,good_dvec_rhf_4,ndf_dvec_rhf_4
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHF_4_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_rhf=0 & b_rhf=0 & l_rhf=0 & d_rhf=0 & f_hi4=0
cvec_rhf_4=0 & ndf_cvec_rhf_4=0 & galcut_rhf=0 & cal_var_rhf=0
;

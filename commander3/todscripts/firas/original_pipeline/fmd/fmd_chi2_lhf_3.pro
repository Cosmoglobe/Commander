Pro FMD_CHI2_LHF_3,error
;

;
;  FMD_CHI2_LHF_3 drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for LHFA Band_3 offset destriped data, using frequency-independent
;  coadd weights.
;
;
;  ARGUMENTS (I/O)    :
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHF_3_DSP.ISS, LHF_WEIGHTS.ISS,
;                        and LHF_3_EJG.ISS, and where output save sets 
;                        LHF_3_CVECTOR.ISS, LHF_3_SKYMAP_2.ISS, LHF_3_SKY_CHI2.ISS
;                        and LHF_3_CAL_CHI2.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_LHF_3,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_LHF_3 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_LHF_3,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_LHF_3,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_3_DSP.ISS'
RESTORE,'csdr$firas_out:lhf_3_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_LHF_3 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi3(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_LHF_3 : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore LHF_3_EJG Save Set
; -------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_3_EJG.ISS'
RESTORE,'csdr$firas_out:lhf_3_ejg.iss'
;
...
cvec_lhf_3 = $
 FMD_CHI2(pixel=px,spec=dsp,sky_wgts_ds=sky_wgts_ds,w_frac=frac_wgt,$
          ...
          max_frac=max_frac,px_spec=sp_lhf_3,resid=sky_resid_lhf_3,$
          chi2=sky_chi2_lhf_3,ndf_cvec=ndf_cvec_lhf_3)
;
error = $
 FMD_CAL_CHI2(xcal=xcal,del_temp=del_temp,freq=f_hi3,cal_wgts=cal_wgts,$
              cal_spec=cal_dsp,cvec=cvec_lhf_3,resid=cal_resid_lhf_3,$
              chi2=cal_chi2_lhf_3)
;
...
; Make LHF_3_SKYMAP_2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhf_3_skymap_2.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,$
                    freq_lhf_3,sp_lhf_3,cvec_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_SKYMAP_2.ISS" Created.'
PRINT,' '
;

; Make LHF_3_SKY_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:lhf_3_sky_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    ...
                    sky_mask,freq_lhf_3,sky_resid_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make LHF_3_CVECTOR.ISS Save Set
; -----------------------------
sname = 'csdr$firas_out:lhf_3_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_lhf_3,cvec_lhf_3,ndf_cvec_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make LHF_3_SKY_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhf_3_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_lhf_3,cvec_lhf_3,sky_chi2_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make LHF_3_CAL_RESID.ISS Save Set
; -------------------------------
sname = 'csdr$firas_out:lhf_3_cal_resid.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_lhf_3,cal_resid_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make LHF_3_CAL_CHI2.ISS Save Set
; ------------------------------
sname = 'csdr$firas_out:lhf_3_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_lhf_3,cvec_lhf_3,$
                    cal_chi2_lhf_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_3_CAL_CHI2.ISS" Created.'
PRINT,' '
;
...
ejg_lhf_3=0 & c_lhf_3=0 & ndf_ejg_lhf_3=0 & chi2_lhf_3=0

Pro FMD_EJG_RSF,error
;

;
;  FMD_EJG_RSF drives the FMD_DESTRIPER procedure to create
;              IDL save sets of RLFA offset stripes.
;
;
;  ARGUMENTS (I/O)    :
;
...
;                        FMD_BAD_COADD_LO.ISS, FMD_DIRBE_FUNC_RSF.ISS,
;                        and RSF_WEIGHTS.ISS .
;
...
;  Output  :
;
;    RSF_EJG.ISS, contains destriper qualifiers, weights, stripe spectra,
;                 and C-Vector.
;
;    RSF_ERRORS.ISS, contains destriper error matrices.
;
;    RSF_SKYMAP.ISS, contains skymap spectra and associated errors.
;
;    RSF_FUNC.ISS, contains destriper kernals.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_EJG_RSF,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_EJG_RSF : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_EJG_RSF,error'
 PRINT,' '
...
IF (n_tophat NE N_ELEMENTS(step_dn)) THEN BEGIN
 PRINT,'FMD_EJG_RSF : STEP_UP and STEP_DN are NOT Compatible !'
 RETURN
ENDIF
;
...
IF (cgood GT 1) THEN BEGIN
 ...
 IF (MAX(hlp) GT 1) THEN BEGIN
  PRINT,'FMD_EJG_RSF : Elements of LP_ORDER Are Degenerate !'
  RETURN
 ENDIF
...
IF (N_ELEMENTS(stripe_order) NE 5) THEN BEGIN
 PRINT,'FMD_EJG_RSF : STRIPE_ORDER Must Have 5 Elements !'
; STOP
 RETURN
ENDIF
;
...
IF (stripe_err EQ 1) THEN BEGIN
 PRINT,'FMD_EJG_RSF : Error in STRIPE_ORDER !'
; STOP
 RETURN
ENDIF
;
...
; ---------------------------
PRINT,' '
PRINT, 'RSF Offset Destriping -- Pass 4'
PRINT,' '
...
IF (cf LE 0) THEN BEGIN
 PRINT,''
 PRINT,'FMD_EJG_RSF : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
; -----------------------
RESTORE,'csdr$firas_ref:fmd_dirbe_func_rsf.iss'
;
...
PIX2XY,cmp_px,data=ll(*,0),res=6,/six,ras=l_rsf
PIX2XY,cmp_px,data=ll(*,1),res=6,/six,ras=b_rsf

n_tot = FMD_PIX_SUM(pix=pxg,data=wg)
PIX2XY,cmp_px,data=n_tot,res=6,/six,ras=n_rsf
...
c_rsf = $
  FMD_DESTRIPER(freq=f_lo,pixel=px,tm=tm,cal_tm=cal_tm, $
	        ...
	        ejg=ejg_rsf,afp=s_rsf, $
                func=func,sky_func=sky_func_rsf,cal_func=cal_func_rsf, $
                pcvr=pcvr,rect=rect,diag=diag,chi2_map=chi2_rsf, $
                n_krnl=n_krnl,n_cmn=n_cmn,square=square,d_inv=d_inv, $
                ndf=ndf_ejg_rsf,/pure,/mjy) 
;

IF (MAX(c_rsf) LT 0.) THEN BEGIN
 PRINT,''
...
FMD_ERRMAT,pcvr=pcvr,rect=rect,diag=diag,beta=beta,omega=omega,$
           stripe_conv=stripe_conv,ejg=ejg_rsf,gamma=gamma_rsf,lmat=lmat
;
...
; Make RSF_EJG Save Set
; -----------------------
chan_label = 'RSF'
sname='csdr$firas_out:rsf_ejg.iss'
SAVE,filename=sname,chan_label,stripes_descrip,stripe_order,stripe_id, $
 ...
 dirbe_mask,tmin,tmax,good_cal_dihd,cal_wgts,cal_wgts_ds,f_lo,ejg_rsf, $
 c_rsf,ndf_ejg_rsf,chi2_rsf,dihed_cut_min,max_frac,n_stripes, $
 freq_band,cvec_cut
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_EJG.ISS" Created.'
;

; Make RSF_ERRORS.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rsf_errors.iss'
SAVE,filename=sname,chan_label,stripes_descrip,cmp_px,pcvr,rect,diag,beta,omega, $
                    ...
                    lmat,f_lo,c_rsf
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_ERRORS.ISS" Created.'
;

; Make RSF_SKYMAP.ISS Save Set
; ----------------------------
sname = 'csdr$firas_out:rsf_skymap.iss'
freq_rsf = f_lo
SAVE,filename=sname,chan_label,stripes_descrip,freq_rsf,l_rsf,b_rsf,n_rsf, $
                    c_rsf,s_rsf,cmp_px,pcvr,rect,beta
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_SKYMAP.ISS" Created.'
PRINT,' '
;

; Make RSF_FUNC.ISS Save Set
; --------------------------
sname = 'csdr$firas_out:rsf_func.iss'
SAVE,filename=sname,chan_label,stripes_descrip,sky_func_rsf,cal_func_rsf, $
                    sky_idx,cal_idx
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RSF_FUNC.ISS" Created.'
PRINT,' '
;
...
cal_glitch=0 & chanscan_id=0 & cal_nifgs=0 & cal_lbl=0 & sky_lbl=0 & d_rsf=0
...
badcoadd_rls=0 & badcoadd_lsf=0 & badcoadd_lls=0
badcal_rls=0 & badcal_lsf=0 & badcal_lls=0
;

Pro FMD_CHI2_RHS_3X,error
;

;
;  FMD_CHI2_RHS_3X drives the FMD_CHI2 and FMD_CAL_CHI2 procedures to
;  compute C-Vector, pixel spectra, coadd residuals, and coadd chi-squared
;  for RHSS Band_3 offset destriped data, using default coadd weights.
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing RHS_3X_DSP.ISS, RHSX_WEIGHTS.ISS,
;                        and RHS_3X_EJG.ISS, and where output save sets 
;                        RHS_3X_CVECTOR.ISS, RHS_3X_SKY_RESID.ISS,
;                        RHS_3X_SKY_CHI2.ISS, RHS_3X_CAL_RESID.ISS,
;                        and RHS_3X_CAL_CHI2.ISS, will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_CHI2_RHS_3X,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_CHI2_RHS_3X : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_CHI2_RHS_3X,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_CHI2_RHS_3X,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; -----------------------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_3X_DSP.ISS'
RESTORE,'csdr$firas_out:rhs_3x_dsp.iss'
;
...
IF (cf NE N_ELEMENTS(f_hi3)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_CHI2_RHS_3X : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
freq = f_hi3(nf)
...
IF (cq GT 0) THEN BEGIN
 PRINT,' '
 PRINT,'FMD_CHI2_RHS_3X : Zodi Frequency Array is Mismatched !'
 PRINT,' '
 RETURN
ENDIF
;
...
; Restore RHS_3X_EJG Save Set
; ---------------------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'RHS_3X_EJG.ISS'
RESTORE,'csdr$firas_out:rhs_3x_ejg.iss'
;
...
; Make RHS_3X_SKY_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rhs_3x_sky_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_SKY_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_3X_CVECTOR.ISS Save Set
; --------------------------------
sname = 'csdr$firas_out:rhs_3x_cvector.iss'
SAVE,filename=sname,chanscan,stripes_descrip,freq_dependence,cvec_cut,$
                    freq_rhs_3,cvec_rhs_3,ndf_cvec_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_CVECTOR.ISS" Created.'
PRINT,' '
;

; Make RHS_3X_SKY_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rhs_3x_sky_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,px,tm,$
                    sky_s0,sky_dihd,sky_glitch,sky_wgts_ds,frac_wgt,$
                    sky_mask,cvec_mask,freq_rhs_3,cvec_rhs_3,sky_chi2_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_SKY_CHI2.ISS" Created.'
PRINT,' '
;

; Make RHS_3X_CAL_RESID.ISS Save Set
; ----------------------------------
sname = 'csdr$firas_out:rhs_3x_cal_resid.iss'
...
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_CAL_RESID.ISS" Created.'
PRINT,' '
;

; Make RHS_3X_CAL_CHI2.ISS Save Set
; ---------------------------------
sname = 'csdr$firas_out:rhs_3x_cal_chi2.iss'
SAVE,filename=sname,chanscan,chan_label,stripes_descrip,freq_dependence,$
                    xcal,cal_tm,ical,skyh,refh,dihd,cal_glitch,$
                    cal_s0,cal_wgts_ds,cal_wgts,freq_rhs_3,cvec_rhs_3,$
                    cal_chi2_rhs_3
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'RHS_3X_CAL_CHI2.ISS" Created.'
PRINT,' '
;

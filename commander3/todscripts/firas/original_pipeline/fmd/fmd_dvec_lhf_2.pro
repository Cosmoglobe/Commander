Pro FMD_DVEC_LHF_2,error
;

;
;  FMD_DVEC_LHF_2 drives the FMD_DVECTOR procedure to compute
;  the LHFA Band_2 D-Vector .
;
;
;  ARGUMENTS (I/O)    :
;
...
;
;    CSDR$FIRAS_OUT   =  Directory containing LHF_2_CVECTOR.ISS and LHF_2_EJG.ISS,
;                        where output LHF_2_DVECTOR.ISS will be sent.
;
;
;    EXAMPLE : $ uidl
;              UIDL> FMD_DVEC_LHF_2,error
;
;
...
IF N_Params() ne 1 THEN BEGIN 
 PRINT,'FMD_DVEC_LHF_2 : Incorrect invocation !'
 PRINT,' '
 PRINT,'Correct invocation is :'
 PRINT,' '
 PRINT,'IDL> FMD_DVEC_LHF_2,error'
 PRINT,' '
 PRINT,'Returning with Error.'
 PRINT,' '
 PRINT,'Try again with valid invocation.'
 PRINT,' '
 PRINT,'Example : IDL> FMD_DVEC_LHF_2,error'
 PRINT,' '
 RETURN
ENDIF
;
...
; ----------------
PRINT,' '
PRINT,'Restoring ' + outtrans(0) + 'LHF_2_CVECTOR.ISS'
...
IF (cf NE N_ELEMENTS(freq_lhf_2)) THEN BEGIN
 PRINT,''
 PRINT,'FMD_DVEC_LHF_2 : Error in Frequency Array !'
 PRINT,''
 RETURN
ENDIF
;
...
dvec_lhf_2 = $
 FMD_DVECTOR(var=var,mask=mask,nifgs=nifgs,st_sub=st_sub,sky_wgts=sky_wgts_ds,$
             good=good_dvec_lhf_2,ndf_dvec=ndf_dvec_lhf_2)
;

; Make LHF_2_DVECTOR.ISS Save Set
; -------------------------------
chanscan = 'LHF_2'
freq_dependence = 'N'
dvec_cut = cvec_cut
sname = 'csdr$firas_out:lhf_2_dvector.iss'
SAVE,filename=sname,chanscan,freq_dependence,dvec_cut,max_frac,freq_lhf_2,$
                    dvec_lhf_2,good_dvec_lhf_2,ndf_dvec_lhf_2
PRINT,' '
PRINT,'IDL Save Set "'+outtrans(0)+'LHF_2_DVECTOR.ISS" Created.'
PRINT,' '
;
...
n_lhf=0 & b_lhf=0 & l_lhf=0 & d_lhf=0 & f_hi2=0
cvec_lhf_2=0 & ndf_cvec_lhf_2=0 & galcut_lhf=0 & cal_var_lhf=0
;

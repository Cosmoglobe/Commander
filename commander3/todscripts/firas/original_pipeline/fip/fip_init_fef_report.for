	integer * 4 function  fip_init_fef_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_FEF_REPORT
c
c	This function initializes the FIP_FEF processing report.  The report
...
c
  ...
  70	   format (x, 'Error File Extension:      ', a)
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_nofef)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_FEF:', /)
 120	   format (4x, 'No FEF file specified for /FILE_EXT qualifier.')

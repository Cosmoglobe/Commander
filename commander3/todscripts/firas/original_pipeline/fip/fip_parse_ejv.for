	integer * 4 function  fip_parse_ejv (current_gmt)

c-------------------------------------------------------------------------------
c
c	Function FIP_PARSE_EJV
c
c	This function parses the command line for FIP_EJV.
c
c	Author:   Gene Eplee
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fip_noejv)
	   call lib$signal (fip_noejv)
	endif
...
c
	...
	if (status .eq. upm_negated) then
	   ...
	   if (status .eq. upm_absent) then  !  Set default report file name
	      ...
	      fcc_report_file = 'FIP_EJV_' // fcc_scan_mode // '_' //

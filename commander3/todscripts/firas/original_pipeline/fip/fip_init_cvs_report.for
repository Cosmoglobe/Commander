	integer * 4 function  fip_init_cvs_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_CVS_REPORT
c
c	This function initializes the FIP_CVS processing report.  The report
...
c
  ...
  70	   format (x, 'C-Vector File Extension:   ', a)
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_nocvs)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_CVS:', /)
 120	   format (4x, 'No C-Vector file specified for /FILE_EXT qualifier.')

	integer * 4 function  fip_init_dust_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_DUST_REPORT
c
c	This function initializes the FIP_DUST processing report.  The report
c	is opened, then the erroror handler is invoked.  Finally, the account
...
c
  ...
  70	   format (x, 'Dust File Extension:       ', a)
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_nodust)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_DUST:', /)
 120	   format (4x, 'No DUST file specified for /FILE_EXT qualifier.')

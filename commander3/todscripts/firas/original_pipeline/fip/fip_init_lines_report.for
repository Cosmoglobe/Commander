	integer * 4 function  fip_init_lines_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_LINES_REPORT
c
c	This function initializes the FIP_LINES processing report.  The report
...
c
     ...
     .					'FIRAS Facility FIP_LINES')
	   ...
	   write (fut_report_lun,70,iostat=io_stat) fcc_lines_ext
  ...
  70	   format (x, 'Lines File Extension:           ', a)
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_nolines)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_LINES:', /)
 120	   format (4x, 'No lines file specified for /LINES_EXT qualifier.')

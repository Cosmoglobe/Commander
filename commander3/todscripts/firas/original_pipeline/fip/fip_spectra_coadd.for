	Program FIP_SPECTRA_COADD

c------------------------------------------------------------------------------
c
c	Program FIP_SPECTRA_COADD
c
c	This program reformats FCF and FIC sky and cal data into the Initial
c	Release ADB format.  FIP reformats FCF calibrated spectra and
c       differential spectra skymaps into FIP skymaps, and calibration data
c       sets.  The FIP output is ready for conversion to FITS files.  The
c       input/output data sets are:
c          InputFile         Cal/Sky           Output
c          ----------        -------        --------------
c          FCF_SKY_xxxx        Sky           FIP_CSK_xxxx
c          FCF_CAL_xxxx        Cal           FIP_CCL_xxxx
c          FCF_DSK_xxxx        Sky           FIP_DSK_xxxx
c          FCF_DCL_xxxx        Cal           FIP_DCL_xxxx
c          FIC_SKY_xxxx        Sky           FIP_ISK_xxxx
c          FIC_CAL_xxxx        Cal           FIP_ICL_xxxx
c
c	Author:   Larry P. Rosen, Hughes STX, July 1994 & October-November 1994
c       Based in part on FIP_SKY by Gene Eplee, GSC
c
...
c		fip_sc_parse
c		fip_sc_init_report
c		fip_sc_process_sky
c		fip_sc_process_cal
c		fip_sc_update_report
c		ct_init
c		lib$signal
c
c	Include files:
...
c Functions
	...
	Integer*4	FIP_SC_Parse
	Integer*4	FIP_SC_Init_Report
	Integer*4	FIP_SC_Process_Sky
	Integer*4	FIP_SC_Process_Cal
	Integer*4	FIP_SC_Update_Report
...
c
	rstatus = FIP_SC_Parse ( current_gmt, data_type, input, sfile, slen,
...
c
	If ( fcc_report .EQ. fac_present ) Then
	   ...
	   status = FIP_SC_Init_Report ( rstatus, current_gmt, input, sfile,
	...
	1    (rstatus .EQ. %loc(FIP_Normal)) ) Then
C
...
c     FCF_DCL     CAL       fac_present       .FALSE.     DIFFERENTIAL     CAL
c
c  Processing for sky data is done with CSA funtions for skymaps.
c  Processing for cal data is done with Cobetrieve reads and writes.
C
	      If ( data_type .EQ. 'SKY' ) Then
	         status = FIP_SC_Process_Sky ( input, filexts, fnum, archin,
	1                                      archout, nrec, npix )
	      Else
	         status = FIP_SC_Process_Cal ( input, filexts, fnum, archin,
	1                                      archout, nrec )
	         npix = 0
	      EndIf
...
c
	...
	1    (fcc_report .EQ. fac_present) ) Then
	   status = FIP_SC_Update_Report ( data_type, nrec, npix, input,
	1                                  filexts, fnum )
	EndIf
c

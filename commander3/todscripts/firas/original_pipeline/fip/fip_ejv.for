...
c	Program FIP_EJV
c
c	This program reformats the destriper correction files into the
...
c		fip_init_ejv_report
c		fip_frequency_cut
c		fip_parse_ejv
c		fip_read_nyquist
c		fip_reformat_ejv
c		fip_update_ejv_report
c		fut_free_lun
c		lib$signal
c
c	Include files:
...
c-------------------------------------------------------------------------------
	...
	integer * 4	fip_init_ejv_report
	integer * 4	fip_parse_ejv
	integer * 4	fip_read_nyquist
	integer * 4	fip_reformat_ejv
	integer * 4	fip_update_ejv_report
...
c
	parse_status = fip_parse_ejv (current_gmt)
...
c
	if (fcc_report .eq. fac_present) then
	   status = fip_init_ejv_report (current_gmt, parse_status)
...
c
c  Process the offset correction spectra.
c
	      if (status .eq. %loc(fip_normal)) then
	         status = fip_reformat_ejv ()
	      endif

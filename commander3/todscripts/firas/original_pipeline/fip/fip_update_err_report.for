	integer * 4 function  fip_update_err_report ()

c-------------------------------------------------------------------------------
c
c	Function FIP_UPDATE_ERR_REPORT
c
c	This function writes the input and output file names and the data
c	selection cuts to the FIP_ERR processing report.
c
c	Author:   Gene Eplee
...
c
	...
	write (fut_report_lun,20,iostat=io_stat) fcc_fer_file(1:fcc_ferlen)
	write (fut_report_lun,30,iostat=io_stat) fcc_fip_file(1:fcc_fiplen)
  10	format (//, x, 'FIP_ERR Processing Summary:', /)
  20	format (4x, 'FER Error File:           ', a)
  30	format (4x, 'FIP Error File:           ', a)
...
c
	if (io_stat .ne. 0) then
	   fip_update_err_report = %loc(fip_repwrite)
	   call lib$signal (fip_repwrite, %val(2),
     .			    fcc_report_file(1:fcc_replen), %val(io_stat))
	else
	   fip_update_err_report = %loc(fip_normal)
	endif

...
c	Program FIP_LINES
c
c	This program reformats a FIRAS spectral lines profile file into the
c	Initial Release ADB format.
c
c	Author:   Gene Eplee
...
c		fip_init_lines_report
c		fip_frequency_cut
c		fip_parse_lines
c		fip_read_nyquist
c		fip_reformat_hilines
c		fip_reformat_lolines
c		fip_update_lines_report
c		fut_free_lun
c		lib$signal
c
c	Include files:
...
c	Changes:
c
c	Replaced the call to FIP_REFORMAT_LINES with a call to either
c	FIP_REFORMAT_HILINES (for high frequency channels) or
c	FIP_REFORMAT_LOLINES (for low frequency channels).
c	Replaced FIP_CONFIG_LINES.TXT by FIP_CONFIG_FREQ.TXT
c	Gene Eplee, GSC, 10 February 1994.  SER 11728.
c
c-------------------------------------------------------------------------------
	...
	complex * 8	buffer(148)	!  dummy data buffer
	...
	integer * 4	fip_init_lines_report
	integer * 4	fip_parse_lines
	integer * 4	fip_read_nyquist
	integer * 4	fip_reformat_hilines
	integer * 4	fip_reformat_lolines
	integer * 4	fip_update_lines_report
...
c
	parse_status = fip_parse_lines (current_gmt)
...
c
	if (fcc_report .eq. fac_present) then
	   status = fip_init_lines_report (current_gmt, parse_status)
...
c
c  Process the lines file for the high frequency or low frequency channels.
c	Process the low frequency long fast channels correctly.
c
	      if (status .eq. %loc(fip_normal)) then
	         if ((fcc_chan .eq. 1)  .or.  (fcc_chan .eq. 3)) then
	            status = fip_reformat_hilines ()
	         else
	            if (fcc_smode .eq. 4) then
	               status = fip_reformat_lolines (buffer, 148)
	            else
	               status = fip_reformat_lolines (buffer, 34)
	            endif
	         endif
	      endif
...
c
	if ((status .eq. %loc(fip_normal))  .and.
     .	    (fcc_report .eq. fac_present)) then
	   status = fip_update_lines_report ()
	endif

...
c	Program FIP_MODEL
c
c	This program reformats a FIRAS Production calibration model solution
c	into the Initial Release ADB format.  The program also inserts various
c	reference datasets into the Initial Release model solution.
c
c	Author:   Gene Eplee
...
c		fip_calc_responsivity
c		fip_init_model_report
c		fip_frequency_cut
c		fip_parse_model
c		fip_read_model
c		fip_read_reference
c		fip_update_model_report
c		fip_write_model
c		fut_free_lun
c		lib$signal
c
...
c-------------------------------------------------------------------------------
	...
	integer * 4	fip_calc_responsivity
	integer * 4	fip_frequency_cut
	integer * 4	fip_init_model_report
	integer * 4	fip_parse_model
	integer * 4	fip_read_model
	integer * 4	fip_read_reference
	integer * 4	fip_update_model_report
	integer * 4	fip_write_model
...
c
	parse_status = fip_parse_model (current_gmt)
...
c
	if (fcc_report .eq. fac_present) then
	   status = fip_init_model_report (current_gmt, parse_status)
...
C
C  Read the model solution files.
C

c
c    The reference datsets.
c
	   status = fip_read_reference ()

c
c    The model solution files.
c
	   if (status .eq. %loc(fip_normal)) then
	      status = fip_read_model ()
	   endif


C
C  Process the model solution.
C
...
c
	if ((status .eq. %loc(fip_normal))  .and.
     .	    (fcc_report .eq. fac_present)) then
	   status = fip_update_model_report ()
	endif

	integer * 4 function  fip_update_fef_report (num)

c-------------------------------------------------------------------------------
c
c	Function FIP_UPDATE_FEF_REPORT
c
c	This function writes the input and output file names and the data
c	selection cuts to the FIP_FEF processing report.
c
c	Author:   Gene Eplee
...
c
c	Input:
c		num	integer * 4		number of records processed
c
c	Output:
c		none
c
...

	...
	integer * 4	num		!  number of records processed
...
c
	...
	write (fut_report_lun,20,iostat=io_stat) fcc_fef_file(1:fcc_feflen)
	write (fut_report_lun,30,iostat=io_stat) fcc_fip_file(1:fcc_fiplen)
  10	format (//, x, 'FIP_FEF Processing Summary:', /)
  20	format (4x, 'FEF Spectrum File:           ', a)
  30	format (4x, 'FIP Spectrum File:           ', a)
...
c
c  Write the number of records processed:
c
	write (fut_report_lun,80,iostat=io_stat) num
  80	format (4x, 'Number of comparison records processed:    ', I2)

c
c  Check the status of the writes:
c
	if (io_stat .ne. 0) then
	   fip_update_fef_report = %loc(fip_repwrite)
	   call lib$signal (fip_repwrite, %val(2),
     .			    fcc_report_file(1:fcc_replen), %val(io_stat))
	else
	   fip_update_fef_report = %loc(fip_normal)
	endif

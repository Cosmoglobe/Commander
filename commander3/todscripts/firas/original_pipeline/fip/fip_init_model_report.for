	integer * 4 function  fip_init_model_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_MODEL_REPORT
c
c	This function initializes the FIP_MODEL processing report.  The report
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_boltemp)) then
	         write (fut_report_lun,120,iostat=io_stat)
	      elseif (parse_status .eq. %loc(fip_bolvolt)) then
	         write (fut_report_lun,130,iostat=io_stat)
	      elseif (parse_status .eq. %loc(fip_cmdbias)) then
	         write (fut_report_lun,140,iostat=io_stat)
	      elseif (parse_status .eq. %loc(fip_nomodel)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_MODEL:', /)
 120	   format (4x,

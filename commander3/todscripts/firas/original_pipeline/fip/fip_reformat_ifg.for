	integer * 4 function  fip_reformat_ifg (in_rec, out_rec)

c------------------------------------------------------------------------------
c	Function FIP_REFORMAT_IFG
c
c	This function reformats a FIRAS Pipeline coadded ifg into an FIP_ISK
c	coadded ifg.
c
c	Author:   Gene Eplee
...
c------------------------------------------------------------------------------
c	Input:
c		in_rec		FIC_SKY			input coadd record
c
c	Output:
c		out_rec		FIP_ISK			output coadd record
c
...
c	Fred Shuman, HSTX, 1995 Apr 10, SPR 12174.
c
c       Steve Brodd, HSTX, 6/26/95, SPR 12208.  Remove the moon_angle and
c       galactic attitude fields; add the datatype field (8 for sky coadds
c       and 9 for cal coadds).
c------------------------------------------------------------------------------

	implicit none

	include '(fut_params)'
	include '(fip_invoc_sky)'		! invocation parameters
	...
	dictionary 'fic_sky'
	record /fic_sky/ in_rec
	dictionary 'fip_isk'
	record /fip_isk/ out_rec
...
c
	...
	out_rec.num_ifgs = in_rec.coad_spec_head.num_ifgs
	...
	if (time .ge. 0.0D0) then
	   ...
	   fip_reformat_ifg = %loc(fip_adttime)
...
c
	...
	out_rec.bath_temp     =  in_rec.coad_spec_data.bol_assem(1)

	fip_reformat_ifg = %loc(fip_normal)

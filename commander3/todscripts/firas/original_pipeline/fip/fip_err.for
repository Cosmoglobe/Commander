...
c	Program FIP_ERR
c
c	This program reformats the FIRAS error files into the Project Dataset
c	format.
c
c	Author:   Gene Eplee
...
c		fip_init_err_report
c		fip_frequency_cut
c		fip_parse_err
c		fip_read_nyquist
c		fip_reformat_err
c		fip_update_err_report
c		fut_free_lun
c		lib$signal
c
c	Include files:
...
c-------------------------------------------------------------------------------
	...
	integer * 4	fip_init_err_report
	integer * 4	fip_parse_err
	integer * 4	fip_read_nyquist
	integer * 4	fip_reformat_err
	integer * 4	fip_update_err_report
...
c
	parse_status = fip_parse_err (current_gmt)
...
c
	if (fcc_report .eq. fac_present) then
	   status = fip_init_err_report (current_gmt, parse_status)
...
c
c  Process the error file..
c
	      if (status .eq. %loc(fip_normal)) then
	         status = fip_reformat_err ()
	      endif

	integer * 4 function  fip_parse_err (current_gmt)

c-------------------------------------------------------------------------------
c
c	Function FIP_PARSE_ERR
c
c	This function parses the command line for FIP_ERR.
c
c	Author:   Gene Eplee
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fip_noerr)
	   call lib$signal (fip_noerr)
	endif
...
c
	...
	if (status .eq. upm_negated) then
	   ...
	   if (status .eq. upm_absent) then  !  Set default report file name
	      ...
	      fcc_report_file = 'FIP_ERR_' // fcc_scan_mode // '_' //

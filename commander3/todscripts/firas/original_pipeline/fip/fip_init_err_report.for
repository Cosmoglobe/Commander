	integer * 4 function  fip_init_err_report (current_gmt, parse_status)

c-------------------------------------------------------------------------------
c
c	Function FIP_INIT_ERR_REPORT
c
c	This function initializes the FIP_ERR processing report.  The report
...
c
	   if (parse_status .ne. %loc(fip_normal)) then
	      ...
	      if (parse_status .eq. %loc(fip_noerr)) then
 ...
 110	   format (//, x, 'Error from FIP_PARSE_ERR:', /)
 120	   format (4x, 'No err file specified for /FILE_EXT qualifier.')

	integer * 4 function  fip_parse_dust (current_gmt)

c-------------------------------------------------------------------------------
c
c	Function FIP_PARSE_DUST
c
c	This function parses the command line for FIP_DUST.
c
c	Author:   Gene Eplee
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fip_nodust)
	   call lib$signal (fip_nodust)
	endif
c
...
c
	...
	if (status .eq. upm_negated) then
	   ...
	   if (status .eq. upm_absent) then  !  Set default report file name
	      ...
	      fcc_report_file = 'FIP_DUST_' // fcc_scan_mode // '_' //

	integer * 4 function  fip_parse_fef (current_gmt)

c-------------------------------------------------------------------------------
c
c	Function FIP_PARSE_FEF
c
c	This function parses the command line for FIP_FEF.
c
c	Author:   Gene Eplee
...
c	Changes:
c
c	Merged CHANNEL and SCAN_MODE qualifiers so that merged skymaps could
c	be processed.  Gene Eplee, GSC, 5 December 1994.
c
c-------------------------------------------------------------------------------
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fip_nofef)
	   call lib$signal (fip_nofef)
	endif
...
c
	...
	if (status .eq. upm_pres) then
	   status = upm_present ('SCAN_MODE.RHSS')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'RHSS'
	      fcc_chan = 1
	      fcc_smode = 1
	   ...
	   status = upm_present ('SCAN_MODE.RHFA')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'RHFA'
	      fcc_chan = 1
	      fcc_smode = 2
	   ...
	   status = upm_present ('SCAN_MODE.RLSS')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'RLSS'
	      fcc_chan = 2
	      fcc_smode = 1
	   ...
	   status = upm_present ('SCAN_MODE.RLFA')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'RLFA'
	      fcc_chan = 2
	      fcc_smode = 2
	   ...
	   status = upm_present ('SCAN_MODE.RLLF')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'RLLF'
	      fcc_chan = 2
	      fcc_smode = 4
	   ...
	   status = upm_present ('SCAN_MODE.LHSS')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'LHSS'
	      fcc_chan = 3
	      fcc_smode = 1
	   ...
	   status = upm_present ('SCAN_MODE.LHFA')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'LHFA'
	      fcc_chan = 3
	      fcc_smode = 2
	   ...
	   status = upm_present ('SCAN_MODE.LLSS')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'LLSS'
	      fcc_chan = 4
	      fcc_smode = 1
	   ...
	   status = upm_present ('SCAN_MODE.LLFA')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'LLFA'
	      fcc_chan = 4
	      fcc_smode = 2
	   ...
	   status = upm_present ('SCAN_MODE.LLLF')
	   if (status .eq. upm_pres) then
	      fcc_scan_mode  = 'LLLF'
	      fcc_chan = 4
	      fcc_smode = 4
...
c
	...
	if (status .eq. upm_negated) then
	   ...
	   if (status .eq. upm_absent) then  !  Set default report file name
	      ...
	      fcc_report_file = 'FIP_FEF_' // fcc_scan_mode // '_' //

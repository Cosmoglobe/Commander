	integer * 4 function  fip_update_dust_report (nspec)

c-------------------------------------------------------------------------------
c
c	Function FIP_UPDATE_DUST_REPORT
c
c	This function writes the input and output file names and the data
c	selection cuts to the FIP_DUST processing report.
c
c	Author:   Gene Eplee
...
c
c	Input:
c		nspec	integer * 4		number of spectra processed
c
c	Output:
c		none
c
...
c
	...
	write (fut_report_lun,20,iostat=io_stat) fcc_fla_file(1:fcc_flalen)
	write (fut_report_lun,30,iostat=io_stat) fcc_fip_file(1:fcc_fiplen)
  10	format (//, x, 'FIP_DUST Processing Summary:', /)
  20	format (4x, 'FLA Dust Spectrum File:      ', a)
  30	format (4x, 'FIP Dust Spectrum File:      ', a)
...
c
	write (fut_report_lun,80,iostat=io_stat) nspec
  80	format (4x, 'Number of dust spectra processed:    ', I4)

c
c  Check the status of the writes:
c
	if (io_stat .ne. 0) then
	   fip_update_dust_report = %loc(fip_repwrite)
	   call lib$signal (fip_repwrite, %val(2),
     .			    fcc_report_file(1:fcc_replen), %val(io_stat))
	else
	   fip_update_dust_report = %loc(fip_normal)
	endif

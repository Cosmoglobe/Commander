	integer*4 Function Fut_Qget_grawt ( Ct_lun, grawt_rec)
c----------------------------------------------------------------------
c Program Name : Fut_Qget_grawt
c
c Programmer : N. Gonzales/STX, September 4, 1991
c              
c Program description: This program read Fex_grtrawwt record and formatted
...
c--------------------------------------------------------------------------
	...
	external Fut_normal
        ...
        Call ct_query_get (, ct_lun,grawt_rec,ct_stat)
          If (ct_stat(1) .ne. ctp_normal) then
            If (ct_stat(1) .eq. ctp_endoffile) then
	      fut_qget_grawt = %loc(fut_eof)
	    Else
              Call lib$signal (fut_ctqget_err, %val(1), %val(ct_stat(1)))
              fut_qget_grawt = %loc(fut_ctqget_err)
            endif
            return
           endif
	Fut_qget_grawt = %loc(fut_normal)
        return
        end

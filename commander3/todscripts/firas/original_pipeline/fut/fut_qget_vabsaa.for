	integer*4 Function Fut_Qget_VabSaa ( Ct_lun, VabSaa_rec)
c----------------------------------------------------------------------
c Program Name : Fut_Qget_Facr
c
c Programmer : N. Gonzales/STX, September 23, 1991
c              
c Program description: This program read Fex_VabSaa record and formatted
...
c--------------------------------------------------------------------------
	...
	external Fut_normal
        ...
        Call ct_query_get (, ct_lun,VabSaa_rec,ct_stat)
          If (ct_stat(1) .ne. ctp_normal) then
            If (ct_stat(1) .eq. ctp_endoffile) then
	      fut_qget_VabSaa = %loc(fut_eof)
	    Else
              Call lib$signal (fut_ctqget_err, %val(1), %val(ct_stat(1)))
              fut_qget_VabSaa = %loc(fut_ctqget_err)
            endif
            return
           endif
	Fut_qget_VabSaa = %loc(fut_normal)
        return
        end

	integer*4 Function Fut_Qget_englim ( Ct_lun, englim_rec)
c----------------------------------------------------------------------
c Program Name : Fut_Qget_englim
c
c Programmer : Quoc C Chung/STX
c              Oct 1989 SPR 4750 Formatted dumped out Firas Reference files.
c              
c Program description: This program read Fex_englim record and formatted
...
c--------------------------------------------------------------------------
	...
	external Fut_normal
   	...
   	Record     /Fex_englim/ englim_rec
        ...
        Call ct_query_get (, ct_lun,englim_rec,ct_stat)
          If (ct_stat(1) .ne. ctp_normal) then
            If (ct_stat(1) .eq. ctp_endoffile) then
	      fut_qget_englim = %loc(fut_eof)
	    Else
              Call lib$signal (fut_ctqget_err, %val(1), %val(ct_stat(1)))
              fut_qget_englim = %loc(fut_ctqget_err)
            endif
            return
           endif
	Fut_qget_englim = %loc(fut_normal)
        return
        end

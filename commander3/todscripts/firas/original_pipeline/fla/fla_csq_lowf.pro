...
;  FLA_CSQ_LOWF computes the combined chi-squared per pixel for the
;               LOWF merged skymap.
;
;DESCRIPTION:
;     An IDL procedure to compute the combined chi-squared per pixel for
;     the merge of LOSL and LOFA skymaps, and to write the computed values
;     and associated data to the FIP_CSQ_LOWF.F16_93HYBRID binary file and
;     to IDL Save Set FIP_CSQ_LOWF.ISS
;
;CALLING SEQUENCE:
...
;     FLA_CSQ_LOWF,error
;     if(error ne 0)then print,'Error Returned from FLA_CSQ_LOWF !'
;
;#
;COMMON BLOCKS:
;     None
...
if N_Params() ne 1 then begin
 ...
 print,'Correct Invocation is "FLA_CSQ_LOWF,error" '
 error = 1
 return
endif
;
...
; Fetch C-Variances
; -----------------
openr,lunr,'csdr$firas_ref:fms_cvs_losl.f16_93hybrid',/get_lun,/share
r=assoc(lunr,fla_fex_cvs_st(1))
cvs=r(0)
c1=cvs.cvector(4:37)
close,lunr & free_lun,lunr
;
openr,lunr,'csdr$firas_ref:fms_cvs_lofa.f16_93hybrid',/get_lun,/share
r=assoc(lunr,fla_fex_cvs_st(1))
cvs=r(0)
c2=cvs.cvector(4:37)
close,lunr & free_lun,lunr
...
; LOSL Correction Spectra
; -----------------------
filename = 'fex_mcs_losl.f16_93hybrid'
...
; LOFA Correction Spectra
; -----------------------
filename = 'fex_mcs_lofa.f16_93hybrid'
OPENR,10,'csdr$firas_mcs:' + filename,rec_len,/FIXED
READU, 10, fex_struct
fex2 = fex_struct
CLOSE,10
...
; Offset Corrections
; ------------------
o1=float(fex1.off_spec(4:37))   ; LOSL Offset Correction
o2=float(fex2.off_spec(4:37))   ; LOFA Offset Correction
;

; Fetch LOSL Skymap Records
; -------------------------
fname='csdr$firas_in:fms_sky_losl.fad_8932800_9026409'
openr,lunr,fname,/get_lun,/share
finfo=fstat(lunr)
nrec=finfo.size/finfo.rec_len
r=assoc(lunr,fla_fms_sky_st(nrec))
srec=strcompress(string(nrec))
print,'Reading'+srec+' LOSL Sky Records'
f1=r(0)
close,lunr & free_lun,lunr
;

; Fetch LOFA Skymap Records
; -------------------------
fname='csdr$firas_in:fms_sky_lofa.fad_8932800_9026409'
openr,lunr,fname,/get_lun,/share
finfo=fstat(lunr)
nrec=finfo.size/finfo.rec_len
r=assoc(lunr,fla_fms_sky_st(nrec))
srec=strcompress(string(nrec))
print,'Reading'+srec+' LOFA Sky Records'
f2=r(0)
close,lunr & free_lun,lunr
;
...
res1=fltarr(34) & res2=res1
s12=fltarr(34) & denom=s12
;

; Pixel Numbers and Weights for LOSL
; ----------------------------------
...
; Pixel Numbers and Weights for LOFA
; ----------------------------------
...
s1 = float(f1(idx1).spec_data.spec(4:37))
cvar1 = f1(idx1).spec_data.cvs_var(4:37)
s2 = float(f2(idx2).spec_data.spec(4:37))
cvar2 = f2(idx2).spec_data.cvs_var(4:37)
;

; For Each Matched Pixel, Compute Combined_Chi_Squared
; ----------------------------------------------------
cv1 = fltarr(34)  &  cv2 = cv1
...
;
 s12 = ((w1*n1(kk)*(s1(*,kk)-o1(*)))+(w2*n2(kk)*(s2(*,kk)-o2(*))))/denom
       ;  Merged Spectrum
;
 res1(*) = s1(*,kk)-o1(*)-s12(*)   ; LOSL Residual
 res2(*) = s2(*,kk)-o2(*)-s12(*)   ; LOFA Residual
;
 cv1(*) = cvar1(*,kk)  &  cv2(*) = cvar2(*,kk)     ; Variances(nu)
;
 csqx = (res1*res1/cv1)+(res2*res2/cv2)            ; Chi_Squared(nu)
;
...
; Create IDL Save Set
; -------------------
sname='csdr$firas_save:fla_csq_lowf.iss'
...
; ----------------------
print,' '
print,'Writing the FIP_CSQ_LOWF file.'
print,' '
;
...
; -----------------
error = 0
print,'FLA_CSQ_LOWF : Successful Completion'
print,' '
;

...
;  FLA_CSQ_LRES computes the combined chi-squared per pixel for the
;               LRES merged skymap.
;
;DESCRIPTION:
;     An IDL procedure to compute the combined chi-squared per pixel for
;     the merge of HIGH and LOWF skymaps, and to write the computed values
;     and associated data to the FIP_CSQ_LRES.F16_93HYBRID binary file and
;     to IDL Save Set FIP_CSQ_LRES.ISS
;
;CALLING SEQUENCE:
...
;     FLA_CSQ_LRES,error
;     if(error ne 0)then print,'Error Returned from FLA_CSQ_LRES !'
;
;#
;COMMON BLOCKS:
;     None
...
if N_Params() ne 1 then begin
 ...
 print,'Correct Invocation is "FLA_CSQ_LRES,error" '
 error = 1
 return
endif
;
...
; Fetch C-Variances
; -----------------
openr,lunr,'csdr$firas_ref:fms_cvs_high.f16_93hybrid',/get_lun,/share
r=assoc(lunr,fla_fex_cvs_st(1))
cvs=r(0)
c1=cvs.cvector(4:37)
close,lunr & free_lun,lunr
;
openr,lunr,'csdr$firas_ref:fms_cvs_lowf.f16_93hybrid',/get_lun,/share
r=assoc(lunr,fla_fex_cvs_st(1))
cvs=r(0)
c2=cvs.cvector(4:37)
close,lunr & free_lun,lunr
...
; -----------------------
filename = 'fex_mcs_high.f16_93hybrid'
...
; Offset Corrections
; ------------------
o1=float(fex1.off_spec(4:37))   ; HIGH Offset Correction
o2=fltarr(34)                   ;  No offset for LOWF
;

; Fetch HIGH Skymap Records
; -------------------------
fname='csdr$firas_in:fms_sky_high.fad_8932800_9026409'
openr,lunr,fname,/get_lun,/share
finfo=fstat(lunr)
nrec=finfo.size/finfo.rec_len
r=assoc(lunr,fla_fms_sky_st(nrec))
srec=strcompress(string(nrec))
print,'Reading'+srec+' HIGH Sky Records'
f1=r(0)
close,lunr & free_lun,lunr
;

; Fetch LOWF Skymap Records
; -------------------------
fname='csdr$firas_in:fms_sky_lowf.fad_8932800_9026409'
openr,lunr,fname,/get_lun,/share
finfo=fstat(lunr)
nrec=finfo.size/finfo.rec_len
r=assoc(lunr,fla_fms_sky_st(nrec))
srec=strcompress(string(nrec))
print,'Reading'+srec+' LOWF Sky Records'
f2=r(0)
close,lunr & free_lun,lunr
;
...
; Pixel Numbers and Weights for HIGH
; ----------------------------------
...
; Pixel Numbers and Weights for LOWF
; ----------------------------------
...
;
 s12 = ((w1*n1(kk)*(s1(*,kk)-o1(*)))+(w2*n2(kk)*(s2(*,kk)-o2(*))))/denom
       ;  Merged Spectrum
;
 res1(*) = s1(*,kk)-o1(*)-s12(*)   ; HIGH Residual
 res2(*) = s2(*,kk)-o2(*)-s12(*)   ; LOWF Residual
;
...
; Create IDL Save Set
; -------------------
sname='csdr$firas_save:fla_csq_lres.iss'
...
; ----------------------
print,' '
print,'Writing the FIP_CSQ_LRES file.'
print,' '
;
...
; -----------------
error = 0
print,'FLA_CSQ_LRES : Successful Completion'
print,' '
;

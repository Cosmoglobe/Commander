	integer * 4 function  fla_parse_gain (current_gmt)

c-------------------------------------------------------------------------------
c
c	Function FLA_PARSE_GAIN
c
c	This function parses the command line for FLA_GAIN.
c
c	Author:   Gene Eplee
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fla_normal)
	else
	   pstatus = %loc(fla_nomodel)
	   call lib$signal (fla_nomodel)
	endif
...
c
	...
	if ((fcc_flight .eq. fac_present)  .or.
     .	     (fcc_flight .eq. fac_not_present)) then
	   ...
	   pstatus = %loc(fla_noreftime)
	   call lib$signal (fla_noreftime)
	endif
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fla_cmdbias)
	   call lib$signal (fla_cmdbias)
	endif
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fla_bolvolt)
	   call lib$signal (fla_bolvolt)
	endif
...
c
	...
	if (status .eq. upm_pres) then
	   ...
	   pstatus = %loc(fla_boltemp)
	   call lib$signal (fla_boltemp)
	endif
...
c
	...
	if (status .eq. upm_negated) then
	   ...
	   if (status .eq. upm_absent) then  !  Set default report file name
	      ...
	      fcc_report_file = 'FLA_GAIN_' // fcc_scan_mode // '_' //

Pro FLA_Fit_LLR,chanscan,error
;
...
if N_Params() ne 2 then begin
 print,'FLA_Fit_LLR,chanscan,error'
 return
endif
;
...
speed = strmid(chanscan,3,1)
IF (speed eq 'S') THEN idd = strmid(chanscan,0,3)
IF (speed eq 'F') THEN idd = strmid(chanscan,0,1) + strmid(chanscan,2,2)
if((idd ne 'RLS')and(idd ne 'RSF')and(idd ne 'LLS')and(idd ne 'LSF'))then begin
...
if(idd eq 'RLS')then begin
 cxxx=crls & s_xxx=s_rls & b_xxx=b_rls & l_xxx=l_rls & n_xxx=n_rls
 tau_rls=0 & ejv_rls=0 & d_rls=0 & urls=0
endif
if(idd eq 'RSF')then begin
 cxxx=crsf & s_xxx=s_rsf & b_xxx=b_rsf & l_xxx=l_rsf & n_xxx=n_rsf
 tau_rsf=0 & ejv_rsf=0 & d_rsf=0 & d_rlsf=0 & d_rlfl=0 & ursf=0
endif
if(idd eq 'LLS')then begin
 cxxx=clls & s_xxx=s_lls & b_xxx=b_lls & l_xxx=l_lls & n_xxx=n_lls
 tau_lls=0 & ejv_lls=0 & d_lls=0 & ulls=0
endif
if(idd eq 'LSF')then begin
 cxxx=clsf & s_xxx=s_lsf & b_xxx=b_lsf & l_xxx=l_lsf & n_xxx=n_lsf
 tau_lsf=0 & ejv_lsf=0 & d_lsf=0 & d_llsf=0 & d_llfl=0 & ulsf=0
endif
;
...
zidx = INDGEN(34)
FOR i=0,nnpix-1 DO $
        spec(*,nx(i)) = spec(*,nx(i)) - ( modzodi(zidx(*),nz(i)) * m2ep )
;
...
; ---------------------------------------------------
FOR i=0,nnpix-1 DO $
    dust_spec(*,nx(i)) = dust_spec(*,nx(i)) + ( modzodi(zidx(*),nz(i)) * m2ep )
;

	Integer*4  Function  FFP_SC_INIT_REPORT ( parse_status, current_gmt,
	1                                         input, sfile, slen,
	2                                         cmdline, filexts, fnum,
	3                                         archin, archout )
c------------------------------------------------------------------------------
c
c	Function FFP_SC_INIT_REPORT
c
c	This function initializes the FFP_Spectra_Coadd processing report.  The
...
C
	...
	If (rstatus .NE. %loc(FUT_Normal)) Then
	   Call LIB$Signal (FFP_lunerr, %val(1), %val(rstatus))
	Else
	...
	1         iostat=io_stat )
	   ...
	   If (io_stat .EQ. 0) Then
	      status = %loc(FFP_Normal)
	   Else
	      status = %loc(FFP_RepOpen)
	      call Lib$Signal ( FFP_RepOpen, %val(2),
...
c
c  Get the username of the account invoking the program.
c
...
c
  ...
  50	   Format (x, 'Processing Report File:     ', a, // )
	   If (io_stat .NE. 0) Then
	      status = %loc (FFP_RepWrite)
	      Call Lib$Signal ( FFP_RepWrite, %val(2),
	1                       fcc_report_file(1:fcc_replen), %val(io_stat) )
	      FFP_SC_Init_Report = status
	      Return
	   EndIf
...
c
	   If (parse_status .NE. %loc(FFP_normal)) Then
	      If (parse_status .EQ. %loc (FFP_parserr)) Then
	         call Lib$Signal (FFP_parserr, %val(1), '<see log file>')
	      ElseIf (parse_status .EQ. %loc (FFP_galexc)) Then
	         call Lib$Signal (FFP_galexc, %val(1), 0)
	      ElseIf (parse_status .EQ. %loc (FFP_lunerr)) Then
	         call Lib$Signal (FFP_lunerr, %val(1), 0)
	      ElseIf (parse_status .EQ. %loc (FFP_openerr)) Then
	         call Lib$Signal (FFP_openerr, %val(2), sfile(1:slen), 0)
	      ElseIf (parse_status .EQ. %loc (FFP_readerr)) Then
	         call Lib$Signal (FFP_readerr, %val(2), sfile(1:slen), 0)
	      ElseIf (parse_status .EQ. %loc (FFP_closerr)) Then
	         call Lib$Signal (FFP_closerr, %val(2), sfile(1:slen), 0)
	      ElseIf (parse_status .EQ. %loc (FFP_nofilext)) Then
	         call Lib$Signal (FFP_nofilext)
	      ElseIf (parse_status .EQ. %loc (FFP_noinput)) Then

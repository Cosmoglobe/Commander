      integer*4 function fcl_write(out_cov_name,out_cov,start,stop)
c-------------------------------------------------------------------------------
c
...
c
      external fcl_normal
      external fcl_ctopen
      external fcl_ctwrite
      external fcl_ctclose
c
...
c
     ...
     .     useropen=ct_connect_write)
      if (status .ne. 0) then
         fcl_write = %loc(fcl_ctopen)
         call str$trim(out_cov_name,out_cov_name,len)
         call lib$signal(fcl_ctopen,%val(2),out_cov_name(:len),%val(status))
         return
      end if
c
...
c
      do rec = 1,257
         ...
         if (ct_status(1) .ne. ctp_normal) then
            fcl_write = %loc(fcl_ctwrite)
            call str$trim(out_cov_name,out_cov_name,len)
            call lib$signal(fcl_ctwrite,%val(2),out_cov_name(:len),
     .                      %val(ct_status(1)))
            return
         end if
      end do
...
c
      ...
      if (ct_status(1) .ne. ctp_normal) then
         fcl_write = %loc(fcl_ctclose)
         call str$trim(out_cov_name,out_cov_name,len)
         call lib$signal(fcl_ctclose,%val(2),out_cov_name(:len),
     .                   %val(ct_status(1)))
         return
      end if

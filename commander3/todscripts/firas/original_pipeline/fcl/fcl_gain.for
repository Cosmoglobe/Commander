      integer*4 function fcl_gain(scan_mode,bol_avg,cbias_avg,volt_avg,
     .                            nyquistl_hz,model,s0,tau,tbol,gain)
c-------------------------------------------------------------------------------
c
...
c              S. Brodd, HSTX, 12/95, SPR 12291
c
c     Input: scan_mode    i*4  Scan mode = 1, 2, 4, 5, or 6.
...
c            nyquistl_hz  r*4  Nyquist frequency in Hertz.
...
c
      integer*4 scan_mode
      real*4 bol_avg,cbias_avg,volt_avg,nyquistl_hz
...
c
      do pos = 1,257
         w = 2.0 * fac_pi * (pos-1) * (nyquistl_hz /
     .                                 (fac_fft_length(scan_mode)/2.0))

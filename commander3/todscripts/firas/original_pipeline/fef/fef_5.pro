...
;
;DESCRIPTION:  
;     A driver to process FEF Left-Right Channel Comparison
;
;CALLING SEQUENCE:  
...
;   CSDR$FIRAS_SAVE will contain the output xxx_FEF_5.ISS IDL Save Set.
;
;EXAMPLE:
;   $ UPRIDL
;   chanscan='LLSS'
;   FEF_5,chanscan,error
;
;#
;COMMON BLOCKS:
;   None
...
; Restore Save Sets
; -----------------
if ((chanscan eq 'RHSS')or(chanscan eq 'LHSS')) then begin
   id1 = 'RHS'  &  id2 = 'LHS'
;
...
;
;
if ((chanscan eq 'RHSF')or(chanscan eq 'RHLF')or(chanscan eq 'LHSF')$
  or(chanscan eq 'LHLF')) then begin
   id1 = 'RHF'  &  id2 = 'LHF'
;
   restore,'csdr$firas_in:rhf.iss'
   restore,'csdr$firas_in:rhf_destriped.iss'
   n1 = n_rhf  &  s1 = s_rhf  &  c1 = crhf(*,0)
   pcvr1 = pcvr  &  diag1 = diag  &  rect1 = rect
...
;
;
if((chanscan eq 'RLSS')or(chanscan eq 'LLSS'))then begin
   id1 = 'RLS'  &  id2 = 'LLS'
;
...
;
;
if ((chanscan eq 'RLSF')or(chanscan eq 'RLFL')or(chanscan eq 'LLSF')$
  or(chanscan eq 'LLFL')) then begin
   id1 = 'RSF'  &  id2 = 'LSF'
;
   restore,'csdr$firas_in:rsf.iss'
   restore,'csdr$firas_in:rsf_destriped.iss'
   n1 = n_rsf  &  s1 = s_rsf  &  c1 = crsf(*,0)
   pcvr1 = pcvr  &  diag1 = diag  &  rect1 = rect
...
;
  ...
  n1 = n_rlf  &  s1 = s_rlf  &  c1 = crlf(*,0)
  pcvr1 = pcvr  &  diag1 = diag  &  rect1 = rect
  restore,'csdr$firas_errors:pep_errors.rllf'
  pep1 = gain
;
  restore,'csdr$firas_in:llf.iss'
  restore,'csdr$firas_in:llf_destriped.iss'
  n2 = n_llf  &  s2 = s_llf  &  c2 = cllf(*,0)
  pcvr2 = pcvr  &  diag2 = diag  &  rect2 = rect
  restore,'csdr$firas_errors:pep_errors.lllf'
  pep2 = gain
;
endif
;
...
; ----------------
tau = [61.,153.]
sname='csdr$firas_save:'+id1+'_fef_5.iss'

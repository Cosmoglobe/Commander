      integer*4 function fic_read(input,channel,last_channel,next_channel,
     .                            scan_mode_input,rse_input,rse,single,
     .                            short_lun,sci_lun,eng_lun,num_recs,
     .                            sci_recs,eng_recs)
c------------------------------------------------------------------------------
c
...
c
      external fic_normal
      external fic_csaread
      external fic_ctread
      external fic_ctkeyread
      external fic_ctquery
c
c     Functions.
c
...
c     Input parameters.
c
      integer*4 input,channel,scan_mode_input,rse_input,single
...
c
      ...
      integer*4 status,num_pixels,num_pixel_recs
...
c
     ...
     .                                     num_pixels,0)
                  if (.not. status) then
                     fic_read = %loc(fic_csaread)
                     inquire(short_lun,name=filename)
                     call str$trim(filename,filename,file_len)
                     call lib$signal(fic_csaread,%val(2),filename(:file_len),
     .                               %val(status))
                     return
                  end if
...
c
     ...
     .                                  short_recs(rec).mtm_scan_speed + 1
                        if (rec_scan_mode .eq. scan_mode_input) then
...
c
c     Find the beginning of a coadd group by finding the first transition flag
c     equal to 2.
c
                  trans_flag = 0
...
c     group.  A transition flag of 1 will indicate a non-coaddable group.  A
...
c
     ...
     .                      (num_recs .lt. fac_max_num))
                     ...
                     trans_flag = short_recs(num_recs).transition_flag
c
c     If this is the last group in the pixel, then this loop will pass beyond
c     the last record for the pixel.  In this case, return the coadd group and
c     go on to the next pixel for the next read.
c
                     if (pixel_rec .ge. num_pixel_recs) then
                        next_pixel = .true.
                     end if
...
c
     ...
     .                                short_recs(rec).time(1),ct_status)
               if (ct_status(1) .ne. ctp_normal) then
                  fic_read = %loc(fic_ctkeyread)
                  call ct_binary_to_gmt(short_recs(rec).time,jtime)
                  inquire(sci_lun,name=filename)
                  call str$trim(filename,filename,file_len)
                  call lib$signal(fic_ctkeyread,%val(3),filename(:file_len),
...
c
     ...
     .       (channel .ne. last_channel)) then
            ...
            if (ct_status(1) .ne. ctp_normal) then
               fic_read = %loc(fic_ctquery)
               inquire(eng_lun,name=filename)
               call str$trim(filename,filename,file_len)
               call lib$signal(fic_ctquery,%val(2),filename(:file_len),
     .                         %val(ct_status(1)))
               return
            end if
         end if

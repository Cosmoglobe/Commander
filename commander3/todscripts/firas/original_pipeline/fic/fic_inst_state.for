      integer*4 function fic_inst_state(input,channel,scan_mode,pixel_no,
     .                                  mincoadd,grttrans,grtrawwt,cth,
     .                                  num_recs,sci_recs,eng_recs,num_good,
...
c     Input parameters.
c
      integer*4 input,channel,scan_mode,pixel_no,num_recs
...
c
      integer*4 num_good,num_sci_fail(4,4,32)
...
c
      temp_pos = 1
      do while ((num_good .ge. min_good) .and. (temp_pos .le. 5))
         con_pos = temp_pos + 9
...
c
      if ((num_good .ge. min_good) .and. ((input .eq. fac_cal) .or. 
     .    ((input .eq. fac_raw) .and. (xcal_pos .eq. fac_xcalin)))) then
         rec = 1
         do while ((num_good .ge. min_good) .and. (rec .le. num_recs))
            if (con_recs(rec).con_check .eq. 0) then
...
c
      temp_pos = 2
      do while ((num_good .ge. min_good) .and. (temp_pos .le. 4))
         rec = 1
         do while ((num_good .ge. min_good) .and. (rec .le. num_recs))
            if (con_recs(rec).con_check .eq. 0) then
c
...
c
      stat_pos = 1
      do while ((num_good .ge. min_good) .and. (stat_pos .le. 8))
...
c     pixel number consistency check is enabled.
c
      if ((num_good .ge. min_good) .and. ((input .eq. fac_sky) .or. 
     .    ((input .eq. fac_raw) .and. (xcal_pos .eq. fac_xcalout)))) then

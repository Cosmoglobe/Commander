      integer*4 function fic_qual_check(channel,scan_mode_input,scan_mode,
...
c
      ...
      integer*4 fut_nyquist
c
c     Input parameters.
c
      integer*4 channel,scan_mode_input,qual_inst,qual_att
      integer*4 single,deglitch,num_recs
...
c
c     Determine earliest and latest science records.
c
...
c
      if (single .eq. fac_present) then
         ...
         status = fut_ave_bin_times(time_array,num_recs,time_weights,
...
c
     ...
     .        sci_recs(last_sci_rec).ct_head.mjr_frm_no
      ...
      num_good = num_recs
      coa_rec.coad_spec_head.num_ifgs = num_recs
c
...
c
         if (fake_it .eq. 1) then
            if (adds_per_group .eq. 1) then
               coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(9)
            else if (adds_per_group .eq. 2) then
               coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(10)
            else if (adds_per_group .eq. 3) then
               coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(11)
            else if (adds_per_group .eq. 8) then
               coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(12)
            else if (adds_per_group .eq. 12) then
               coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(13)
            end if
         else
            index = 4*(mod(channel-1,2)) + scan_mode
            coa_rec.coad_spec_data.nyquist_icm = nyquist.icm(index)
            coa_rec.coad_spec_data.nyquist_hertz = nyquist.hz(index)
         end if
      end if
c

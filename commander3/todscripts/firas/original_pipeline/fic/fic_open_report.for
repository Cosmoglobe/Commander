      integer*4 function fic_open_report(channel,scan_mode,report_name,account,
...
c
      ...
      fic_open_report = %loc(fic_normal)
c
...
c
      ...
      if (.not. status) then
         fic_open_report = status
         return
      end if

      open(unit=lun,file=filename,status='new',iostat=status)
      if (status .ne. 0) then
         fic_open_report = %loc(fic_repopen)
         call lib$signal(fic_repopen,%val(2),filename(:file_len),%val(status))
         return
      end if
c
...
c
     ...
     .'FIRAS Facility FIC_Interferogram_Coadd Consistency Check Deglitch Report'
     .)

      write(lun,10,iostat=status) 'Report File: ',filename(:file_len)
10    format(/x,2a)
      if (status .ne. 0) then
         fic_open_report = %loc(fic_repwrite)

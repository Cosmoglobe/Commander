      integer*4 function fic_close_summary(covar,deglitch,output,report,
...
c
      ...
      integer*4 num_coadds(4,4)

      character*72 output_filenames(4,4,7),report_filenames(4,4)
c
c     Local variables.
c
      integer*4 status,pos,channel,scan_mode
      ...
      fic_close_summary = %loc(fic_normal)
c
...
10    format(/x,'Failure Reasons: ',/x,
     ...
     .       '32 - Too Few IFGs Left;')
      if (status .ne. 0) then
         fic_close_summary = %loc(fic_repwrite)
...
c
               tot_sci_noise = tot_sci_noise + num_sci_noise(channel,scan_mode)
...
c
               num_fail = 0
               do pos = 1,32
                  num_fail = num_fail + num_sci_fail(channel,scan_mode,pos)
                  percent_fail(channel,scan_mode,pos) = 
     .              nint(num_sci_fail(channel,scan_mode,pos) * 100.0 / num_recs)
               end do
               num_good = num_recs - num_fail
               percent_good = nint(num_good * 100.0 / num_recs)
               if (num_sci_noise(channel,scan_mode) .gt. 0) then
                  noise = tot_noise(channel,scan_mode) / 
     ...
     .                       (num_sci_fail(channel,scan_mode,pos), pos =  1,16),
     .                       (percent_fail(channel,scan_mode,pos), pos =  1,16),
     .                       (num_sci_fail(channel,scan_mode,pos), pos = 17,32),
     .                       (percent_fail(channel,scan_mode,pos), pos = 17,32)
50    format(/x,'Number and Percent of Records Failing for Each Reason:',/,
     ...
     .                   (num_sci_good(channel,scan_mode,pos), pos =  1,10),
     .                   (num_sci_good(channel,scan_mode,pos), pos = 11,20)
...
c
     ...
     .             (num_sci_deglitch(channel,scan_mode) .gt. 0)) then
                  iter = nint(float(glitch_tot_iter(channel,scan_mode)) / 
     .                        num_sci_deglitch(channel,scan_mode))
                  signal = glitch_tot_signal(channel,scan_mode) / 
     .                     num_sci_deglitch(channel,scan_mode)

      integer*4 function fic_read_covar(channel,scan_mode_input,covar_new,
...
c
      external fic_normal
      external fic_ctopen,fic_ctread,fic_ctclose
c
...
c
      ...
      fic_read_covar = %loc(fic_normal)
c
c     Initialize 771 covariance matrix records, 257 for each of the scan modes
c     SS, SF, LF.
c
      do rec = 1,257
         call lib$movc5(0,,0,fac_covar_size,cov_recs(rec))
...
c
      if (covar_new .ne. fac_present) then
         ...
         if (scan_mode_input .eq. 0) then
            first_mode = 1
            last_mode = 3
         else if (scan_mode_input .eq. 4) then
            first_mode = 3
            last_mode = 3
         else 
         ...
         do mode = first_mode,last_mode
            ...
            query_ttg_cat.dataset_name = 'FIC_COV_'//fac_channel_ids(channel)//
...
c
               ...
               if (ct_status(1) .ne. ctp_normal) then
                  fic_read_covar = %loc(fic_ctclose)
                  call str$trim(filename,filename,file_len)

	integer * 4 function  fsl_read_sky_coadd (ct_lun, num, coadd_recs)

c-------------------------------------------------------------------------------
c
c	Function FSL_READ_SKY_COADD
c
...
c       Modified FCF_Read_Sky_Coadd to FSL_Read_Sky_Coadd for the new FIRAS 
	...
	if (status .eq. %loc(fsl_normal)) then
	   if (cstatus .eq. %loc(csa_normal)) then
	      ...
	      elseif (pixel_no .eq. 6143) then
	         ...
	         call lib$signal (fsl_csapixnoinval, %val(2), %val(pixel_no),

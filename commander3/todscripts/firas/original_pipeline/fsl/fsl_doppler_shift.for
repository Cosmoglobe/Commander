	integer * 4 function  fsl_doppler_shift (vbary, cspec)

c-------------------------------------------------------------------------------
c
c	Function FSL_DOPPLER_SHIFT
c
...
c       Modified FCF_Doppler_Shift to FSL_Doppler_Shift for the new FIRAS 
...
c              solution to process emissivity array.
c
c-------------------------------------------------------------------------------

	implicit none
	...
	real	*  8	freqsp(361)	!  spline frequencies
	real	*  8	ibreak(361)	!  imaginary spline break points
	real	*  8	idopspec(361)	!  imaginary Doppler-shifted spectrum
	real	*  8	ispec(361)	!  imaginary part of calibrated spectrum
	real	*  8	ispline(4,361)	!  imaginary spline coefficients
	real	*  8	rbreak(361)	!  real spline break points
	real	*  8	rdopspec(361)	!  real Doppler-shifted spectrum
	real	*  8	rspec(361)	!  real part of calibrated spectrum
	real	*  8	rspline(4,361)	!  real spline coefficients
...
c     The emissivity arrays in the FSL_Model include file only have 257
c     frequency points. The non-zero value start and stop indices have already 
c     been determined. The spectra arrays are dimensioned for 361 points.
c
	call lib$movc5 (0,,0,2888,freqsp)
	call lib$movc5 (0,,0,2888,rspec)
	call lib$movc5 (0,,0,2888,ispec)
	npts = 0
	first_time = .true.
...
c
	call lib$movc5 (0,,0,2888,rbreak)
	call lib$movc5 (0,,0,2888,ibreak)
	call lib$movc5 (0,,0,11552,rspline)
	call lib$movc5 (0,,0,11552,ispline)
...
c
	...
	do j = 1, npts
	   ...
	   if (freqsp(j) .ge. freq(fcc_spec_length)) then
               freqsp(j) = freq(fcc_spec_length)
           endif
	enddo
...
c
	call lib$movc5 (0,,0,2888,rdopspec)
	call lib$movc5 (0,,0,2888,idopspec)
	do j = 1, npts
...
c
	...
	fsl_doppler_shift = %loc(fsl_normal)

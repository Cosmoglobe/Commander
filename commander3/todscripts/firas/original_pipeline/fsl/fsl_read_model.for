	integer * 4 function  fsl_read_model ()

c-------------------------------------------------------------------------------
c
c	Function FSL_READ_MODEL
c
...
c	file FSL_MODEL for use by FSL.  The routine optionally calls
c	FSL_DISPLAY_MODEL to plot the model solution.
c
...
c       Modified FCF_Read_Model to FSL_Read_Model for the new FIRAS 
...
c               new pipeline are  unchanged, the array sizes for reading the 
c               model are unchanged for FSL.
c            2. Determine the start and stop frequency indices of non-zero
c               data points for the model solution. The external calibrator,
c	        the first emissivity array, is used for the determination.               
c
c-------------------------------------------------------------------------------

	implicit none
	...
	logical * 1     found           !  flag for found start and stop index

	integer * 4	fsl_display_model
	...
	external	fsl_modelclose
	external	fsl_modelinval
	external	fsl_modelopen
	external	fsl_modelread
	external	fsl_normal
	external	fut_normal

...
c
	   ...
	   if (io_stat .ne. 0) then
	      status = %loc(fsl_modelread)
	      call lib$signal (fsl_modelread, %val(2),
...
c
	   ...
	   if (io_stat .ne. 0) then
	      status = %loc(fsl_modelclose)
	      call lib$signal (fsl_modelclose, %val(2),
     .			       fcc_model_file(1:fcc_modlen), %val(io_stat))
	   endif
	   ...
	   status = %loc(fsl_modelopen)
	   call lib$signal (fsl_modelopen, %val(2),
     .			    fcc_model_file(1:fcc_modlen), %val(io_stat))
	endif	!  (open status
...
C
	   ...
	   if (mod_ver .eq. .false.) then
	      status = %loc(fsl_modelinval)
	      call lib$signal (fsl_modelinval, %val(1),
...
c
c  Determine the start and stop frequency indices for non-zero values in the 
c  model solution. Use the external calibrator, the first emissivity array,
...
C
	if ((status .eq. %loc(fsl_normal))  .and.
     .	    (fcc_plot .eq. fac_present)) then
	   status = fsl_display_model ()
	endif


	if (status .eq. %loc(fsl_normal)) type 10, model_label(1:mod_lablen)
 10	format (x, 'Retrieved model solution  ', a, /)


	fsl_read_model = status

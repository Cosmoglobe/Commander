	integer * 4 function  ffi_read_reference (fakeit, upmode)

c-------------------------------------------------------------------------------
c
c	Function FFI_READ_REFERENCE
c
...
c-------------------------------------------------------------------------------
	...
	parameter	(norm = 512.0D0**2)	!    zapodization function
	real	* 8	norm_short		!  normalization factor for
	parameter	(norm_short = 128.0D0**2)  !    zapodization function
	...
	external	ffi_normal
	external	ffi_readapod
	external	ffi_readetf

...
c
	...
	if (io_stat .ne. 0) then
	   status = %loc(ffi_readapod)
	   call lib$signal (ffi_readapod, %val(2), %val(arecno), %val(io_stat))
	endif
...
c
	   ...
	   if (io_stat .ne. 0) then
	      status = %loc(ffi_readetf)
	      call lib$signal (ffi_readetf, %val(1), %val(erecno),
     .					    %val(io_stat))
	   endif

	integer * 4 function  fcf_read_sky_coadd_list (ct_lun, num, coadd_recs)

c-------------------------------------------------------------------------------
c
c	Function FCF_READ_SKY_COADD_LIST
c
...
C
	...
	if (status .eq. %loc(fcf_normal)) then
	   if (cstatus .eq. %loc(csa_normal)) then
	      ...
	      elseif (num_pix .eq. fcc_num_pix) then
	         ...
	         call lib$signal (fcf_csapixnoinval, %val(2),
	...
	fcf_read_sky_coadd_list = status

	integer * 4 function  fcf_calc_responsivity (bol_volt, cmd_bias, Tdet,
     .						     Tbol, Qrad)

c-------------------------------------------------------------------------------
c
c	Function FCF_CALC_RESPONSIVITY
c
...
c
	...
	if (dabs(Tbol-Tconv) .ge. delta_t) then
	   fcf_calc_responsivity = %loc(fcf_dettempconv)
	   call lib$signal (fcf_dettempconv, %val(2), display.gmt,
...
c
	...
	fcf_calc_responsivity = %loc(fcf_normal)

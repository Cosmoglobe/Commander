	Program frd_elex_transfcnl
c
c  This routine creates the data file ELEX_TRANSFCNL.DAT containing the FIRAS
...
c  Changed array size from 257 points to 341 points to recover full frequency
c     resolution.  Gene Eplee, GSC, 12 April 1994.
c  Changed actual array size to length of output spectra as mandated by
c     FFT length in FUT_PARAMS file.  Alice Trenholme, GSC, 3 February 1995,
c     SER 12244.
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	character *12	out_file /'fex_etfl.dat'/
...
c
	...
	open (unit=lun,name=out_file,status='new',form='unformatted',
     .			iostat=ios,access='direct',recl=1536)
...
c
		 if(status .eq. ss$_normal)then
		    ...
		    nrec = nrec + 1
		    dfhz = fsamp/ncompress/fac_fft_length(1)
		    ampmax = -1.0D0
                    call lib$movc5(0,,0,1444,ztrans)

		    do k=1,fac_spec_length(1)

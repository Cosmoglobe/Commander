	integer * 4 function  frd_read_modell ()

c-------------------------------------------------------------------------------
c
c	Function FRD_READ_MODELL
c
c	This function reads Dale Fixsens Firas calibration model solution from
...
c	throughput of the instrument.  Kirchoffs law is applied to calculate
...
c	SER 11394
c
c       Number of parameters written to the file is ALWAYS mpar (currently
c       32) and number of emissivities may vary from FISH run to FISH run.
c       Consequently, change to always read all 32 parameters and pick up
c       the upper limit on the number of emissivities from the file.
c             Alice Trenholme, GSC, 12 December 1995, SPR 12282.
c
c       S. Brodd, HSTX, 8/1/96, SPR 12335. Fixed model label for Pass 4.
c-------------------------------------------------------------------------------
	implicit none
	...
	integer *  4	nlines			!  number of lines in model file
        integer *  4    uplim                   !  number of emissivities
...
c
	in_file = 'csdr$firas_in:fex_mod_' // fac_channel_ids(fcc_chan) //
     .				 fac_scan_mode_idsl(fcc_smode) // '.TXT_' //
     .				 fcc_infile_ext
...
c
	   ...
	   enddo
  10	   format (3(e19.12,x),e19.12)
	   if (io_stat .lt. 0) then
	      uplim = (nlines - npar/4)/3
...
c
     ...
     .		 ((xferr(j,k),xferi(j,k),j=1,6),k=1,uplim)
  20	      format (a)
  30	      format (3(e19.12,x),e19.12)
...
C
c
c  Fill in the model header label.
c
	         fex_model.mod_head.label = fcc_infile_ext

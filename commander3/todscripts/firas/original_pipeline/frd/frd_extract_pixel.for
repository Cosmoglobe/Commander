...
c-------------------------------------------------------------------------------
	...
	parameter	(rec_size = fac_coad_spec_size / 4)  !  long words
...
c
	...
	file = 'csdr$firas_ref:frd_pix_scr_' // fac_scan_mode_ids(smode)
     .          // '.txt'
...
c
	      ...
	      out_file = 'CSDR$FIRAS_OUT:FIC_SKY_' // fac_channel_ids(chan)
     .			  // fac_scan_mode_ids(smode) // '.EP'
...
c
	            ...
	            in_file = 'CSDR$FIRAS_IN:FIC_SKY_' // fac_channel_ids(chan)
     .			       // fac_scan_mode_ids(smode) // '.' // in_ext(pix)

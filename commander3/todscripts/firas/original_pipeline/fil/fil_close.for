      integer*4 function fil_close(input,short_lun,sci_lun,eng_lun,output_luns,
     .                             report_luns)
c----------------------------------------------------------------------------
c
...
c
      external fil_normal
      external fil_csaclose
      external fil_ctclose
      external fil_repclose
c
c     Functions.
c
...
c
      ...
      integer*4 output_luns(7,6),report_luns(4)
c
c     Local variables.
c
...
c
         ...
         if (.not. status) then
            fil_close = %loc(fil_csaclose)
            inquire(short_lun,name=filename)
            call str$trim(filename,filename,file_len)
            call lib$signal(fil_csaclose,%val(2),filename(:file_len),
     .                      %val(status))
            return
         end if
...
c
c     Close any opened output files for all scan modes.
c
         do output_type = 1,6
            do mode = 1,6
               if (output_luns(output_type,mode) .ne. 0) then
...
c
      do mode = 1,4
         if (report_luns(mode) .ne. 0) then
            ...
            if (status .ne. 0) then
               fil_close = %loc(fil_repclose)
               inquire(report_luns(mode),name=filename)
               call str$trim(filename,filename,file_len)
               call lib$signal(fil_repclose,%val(2),filename(:file_len),
     .                         %val(status))
               return
            end if

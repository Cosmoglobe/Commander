      integer*4 function fil_write(input,channel,scan_mode,outfile_ext,con_rec,
...
c
      external fil_normal
      external fil_csawrite,fil_ctwrite
c
c     Functions.
c
      integer*4 fil_open_output
...
c
      ...
      fil_write = %loc(fil_normal)
c
c     If an output file for this output type, channel, and scan mode has not
c     yet been opened, call fil_open_output to do so.
c
      lun = output_luns(output_type,scan_mode)

      if (lun .eq. 0) then

         status = fil_open_output(input,channel,scan_mode,outfile_ext,
...
c
         if (output_type .eq. fac_out_con_check) then
            ...
            if (ct_status(1) .ne. ctp_normal) then
               fil_write = %loc(fil_ctwrite)
               filename = output_filenames(channel,scan_mode,output_type)
               call str$trim(filename,filename,file_len)
               call lib$signal(fil_ctwrite,%val(2),filename(:file_len),
     .                         %val(ct_status(1)))
               return
            end if

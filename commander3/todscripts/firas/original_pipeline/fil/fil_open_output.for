      integer*4 function fil_open_output(input,channel,scan_mode,outfile_ext,
...
c
      external fil_normal
      external fil_csaopen,fil_ctopen
c
...
c
      ...
      fil_open_output = %loc(fil_normal)
c
...
c
      ...
      filename = archive(:archive_len)//':'//'FIL_'//output_ext//'_'//
     .           fac_channel_ids(channel)//fac_scan_mode_idsl(scan_mode)//
...
c   
      if (input .eq. fac_sky) then
         ...
         if (output_type .eq. fac_out_con_check) then
            ...
            rec_size = fac_coad_spec_sizel/4
            pix_offset = fac_coad_spec_pix_offsetl
         end if
     ...
     .        form='unformatted',recordtype='fixed',useropen=csa_open_skymap)
         if (status .ne. 0) then
            fil_open_output = %loc(fil_csaopen)
            call str$trim(filename,filename,file_len)
            call lib$signal(fil_csaopen,%val(2),filename(:file_len),
     .                      %val(status))
            return
         end if
c
...
c   
     ...
     .        useropen=ct_connect_write)
         if (status .ne. 0) then
            fil_open_output = %loc(fil_ctopen)
            call str$trim(filename,filename,file_len)
            call lib$signal(fil_ctopen,%val(2),filename(:file_len),%val(status))
            return
         end if
      end if

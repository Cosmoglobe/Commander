      integer*4 function fil_open_input(input,channel,file,file_ext,jstart,
...
c
      external fil_normal
      external fil_nocatrecs
      external fil_csaopen
      external fil_ctopen
c
...
c
         if (.not. status) then
            fil_open_input = %loc(fil_nocatrecs)
            call lib$signal(fil_nocatrecs,%val(2),archive(:archive_len),
     .                       dataset(:dataset_len))
            return
         end if
c
...
c
     ...
     .            useropen=csa_open_skymap)
            if (status .ne. 0) then
               fil_open_input = %loc(fil_csaopen)

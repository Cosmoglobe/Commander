      integer*4 function fil_sec_template(input,channel,scan_mode,
...
c
      external fil_normal
      external fil_nosectemp
      external fil_cfdirread
      external fil_primrms
      external fil_secrms
c
c     Input parameters.
c
      ...
      integer*4 reftemps_lun,num_recs,num_cgr_recs
      integer*4 num_good,num_cgr_good,xcal_pos,num_peak_pts
...
c
      ...
      fil_sec_template = %loc(fil_normal)

      peak_pos = coa_rec.coad_spec_data.peak_pos
c
c     Check the peak position before proceeding; it may have been set to a
c     flag value in fil_qual_check or may be too small or large to compute
c     template statistics.
c
      if ((peak_pos .lt. 51) .or. (peak_pos .gt. 462)) then
         fil_sec_template = %loc(fil_nosectemp)
         call lib$signal(fil_nosectemp,%val(2),fac_channel_ids(channel),
     .                   coa_rec.ct_head.gmt)
         return
      end if
c
...
c
         if (mod(num_good,4) .eq. 2) then
            sec_template(ifg_pos) = sec_template(ifg_pos) + 0.5 * sort(first) + 
     .                                                      0.5 * sort(last+1)
         end if
...
c
     ...
     .    ((input .eq. fac_raw) .and. (xcal_pos .eq. fac_xcalin))) then
      ...
      if (status .ne. 0) then
         fil_sec_template = %loc(fil_cfdirread)
         inquire(reftemps_lun,name=filename)
         call str$trim(filename,filename,file_len)
         call lib$signal(fil_cfdirread,%val(2),filename(:file_len),%val(status))
         return
      end if
c
...
c
      if (prim_sum .eq. 0.0) then
         call lib$signal(fil_primrms,%val(2),fac_channel_ids(channel),
     .                   coa_rec.ct_head.gmt)
     ...
     .           sqrt(prim_sum/392)
      ...
      if (sec_sum .eq. 0.0) then
         call lib$signal(fil_secrms,%val(2),fac_channel_ids(channel),
     .                   coa_rec.ct_head.gmt)
...
c     coefficients, excluding neighbors.
c
      ave = sum/num_cgr_good
      coa_rec.coad_spec_data.sec_template.b_average = ave
      sum = 0.0
c
c     Record a flag value if there is only one good interferogram in the group.
c
      if (num_cgr_good .eq. 1) then
         ...
         coa_rec.coad_spec_data.sec_template.b_variance = sum/(num_cgr_good-1)
      end if

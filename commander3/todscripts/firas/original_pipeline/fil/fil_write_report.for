      integer*4 function fil_write_report(channel,scan_mode,deglitch,
...
c            glitch_pos         i*4(1000,num_cgr_recs)  Glitch positions.
c            glitch_amp         r*4(1000,num_cgr_recs)  Glitch amplitudes.
...
c
      ...
      integer*4 num_cgr_recs,first_sci_rec,last_sci_rec
      ...
      real*4 avg_noise
      integer*4 glitch_pos(fac_max_deglitch,num_cgr_recs)
      real*4 glitch_amp(fac_max_deglitch,num_cgr_recs)
c
...
c
      write(lun,10,iostat=status) con_recs(1).coadd_gmt,num_cgr_recs,
...
10    format(/x,'Coadd Time    Num Recs First Sci Time  Last Sci Time Fake ',
     .      'SciMode Adds Noise',/x,a,3x,i3,3x,a,2x,a,2x,i2,3x,i2,4x,i2,2x,g9.3)
      if (status .ne. 0) then
         fil_write_report = %loc(fil_repwrite)
...
c
      percent_good = nint(num_good * 100.0 / num_cgr_recs)

      do pos = 1,32
         percent_fail(pos) = nint(num_fail(pos) * 100.0 / num_cgr_recs)
      end do

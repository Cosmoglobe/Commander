      integer*4 function fil_open_summary(file,file_ext,jstart,jstop,rse_input,
...
c            parse_status       i*4  Return status from fil_parse.
c
...
c
      ...
      fil_open_summary = %loc(fil_normal)         
c
c     Get current time.
c
...
c
         ...
         report_name = 'FIL_'//file_ext(:file_ext_len)//
...
c
      ...
      if (.not. status) then
         fil_open_summary = status
         return
      end if

      open(unit=fut_report_lun,file=report_name,status='new',iostat=status)
      if (status .ne. 0) then
         fil_open_summary = %loc(fil_repopen)
         call lib$signal(fil_repopen,%val(2),report_name(:rep_len),
     .                   %val(status))
         return
      else
...
10    format(/x,2a)
      if (status .ne. 0) then
         fil_open_summary = %loc(fil_repwrite)
         call lib$signal(fil_repwrite,%val(2),report_name(:rep_len),
     .                   %val(status))
         return
      end if
...
c
c     If jstop was less than jstart or there was a mismatch among values
c     for ifgs_max, group_max, and pool_max in fil_parse, signal the error now
c     so that it is written to the summary report file.
c
      if (parse_status .eq. %loc(fil_invtime)) then
         fil_open_summary = %loc(fil_invtime)
         call lib$signal(fil_invtime,%val(2),jstop,jstart)
         return
      else if (parse_status .eq. %loc(fil_invnum)) then
         fil_open_summary = %loc(fil_invnum)
         call lib$signal(fil_invnum)
         return
      endif
c

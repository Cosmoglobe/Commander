      integer*4 function fil_transient(channel,scan_mode,dtrf_lun,num_recs,
...
c
      ...
      if (status .ne. 0) then
         fil_transient = %loc(fil_cfdirread)
...
c
         if (num_cgr_good .eq. 1) then
            ...
            coa_rec.coad_spec_data.transient.c_variance = sum/(num_cgr_good-1)
         end if
      end if

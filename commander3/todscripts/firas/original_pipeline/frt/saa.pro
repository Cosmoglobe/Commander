Pro SAA, DATE, CHAN, PLTDEV, F0_SDF, READ
;+
; Pro SAA, DATE, CHAN, PLTDEV, F0_SDF, READ
; Author:  Harte Wang, STX, August 1991
; Purpose:  Plot FDQ_SDF data that is within the SAA, on terrestrial longitude
;   & lattitude map.
; Parameters:
...
; Output:
;       F0_SDF: IDL structure of FDQ_SDF records.
;       Plot to screen or post script file SAA.PS in local directory.
;
...
;-
print,' '
print,"SAA, 'DATE', 'CHAN', 'PLTDEV', F0_SDF (,'READ')"
print,' '
!P.MULTI(2) =1
pltdev = strupcase(pltdev)
chan=strupcase(chan)
set_plot, pltdev
If (pltdev eq 'PS') then device, filename='SAA.PS',/landscape
...
f0_tmp=f0_sdf(where(f0_sdf.attitude.terr_rad_byte eq 8))

Pro SAA_EXCLUDE, DATE, CHAN, PLTDEV, F0_SDF, READ
;+
; Pro SAA_EXCLUDE, DATE, CHAN, PLTDEV, F0_SDF, READ
; Author:  Harte Wang, STX, August 1991
; Purpose:  Plot FDQ_SDF data that is outside the SAA, on terrestrial longitude
;   & lattitude map.
; Parameters:
...
; Output:
;       F0_SDF: IDL structure of FDQ_SDF records.
;       Plot to screen or post script file SAA_EXCLUDE.PS in local directory.
;
...
;-
print,' '
print,"SAA_EXCLUDE, 'DATE', 'CHAN', 'PLTDEV', F0_SDF (,'READ')"
print,' '
!P.MULTI(2) =1
pltdev = strupcase(pltdev)
chan=strupcase(chan)
set_plot, pltdev
If (pltdev eq 'PS') then device, filename='SAA_EXCLUDE.PS',/landscape
...
f0_fex=r0(0)
data_saa1y = f0_fex.saa.latn
data_saa1x = FIndgen(31)*f0_fex.saa.lonstep + f0_fex.saa.lonmin
data_saa2y = f0_fex.saa.lats
data_saa2x = FIndgen(31)*f0_fex.saa.lonstep + f0_fex.saa.lonmin

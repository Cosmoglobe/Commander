Pro VAB_NORTH, DATE, CHAN, PLTDEV, F0_SDF, READ
;+
; Pro VAB_NORTH, DATE, CHAN, PLTDEV, F0_SDF, READ
; Author:  Harte Wang, STX, August 1991
; Purpose:  Plot FDQ_SDF data that is inside the North Van Allen Belt, on
...
; Output:
;       F0_SDF: IDL structure of FDQ_SDF records.
;       Plot to screen or post script file VAB_NORTH.PS in local directory.
;
...
;-
print,' '
print,"VAB_NORTH, 'DATE', 'CHAN', 'PLTDEV', F0_SDF (, 'READ')"
print,' '
!P.MULTI(2) =1
chan=strupcase(chan)
pltdev = strupcase(pltdev)
set_plot, pltdev
If (pltdev eq 'PS') then device, filename='VAB_NORTH.PS',/landscape
...
f0_tmp=f0_sdf(where(f0_sdf.attitude.terr_rad_byte eq 2))

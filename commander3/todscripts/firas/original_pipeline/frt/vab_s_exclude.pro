Pro VAB_S_EXCLUDE, DATE, CHAN, PLTDEV, F0_SDF, READ
;+
; Pro VAB_S_EXCLUDE, DATE, CHAN, PLTDEV, F0_SDF, READ
; Author:  Harte Wang, STX, August 1991
; Purpose:  Plot FDQ_SDF data that is outside the South Van Allen Belt, on
...
;       F0_SDF: IDL structure of FDQ_SDF records.
;       Plot to screen or post script file VAB_S_EXCLUDE.PS in local directory.
;
...
;-
print,"VAB_S_EXCLUDE, 'DATE', 'CHAN', 'PLTDEV', F0_SDF (, 'READ')"
!P.MULTI(2) =1
chan=strupcase(chan)
pltdev = strupcase(pltdev)
set_plot, pltdev
If (pltdev eq 'PS') then device, filename='VAB_S_EXCLUDE.PS',/landscape
...
f0_fex=r0(0)
data_vab1y = f0_fex.vab(1).latn
data_vab1x = FIndgen(41)*f0_fex.vab(1).lonstep - 180.
data_vab2y = f0_fex.vab(1).lats
data_vab2x = FIndgen(41)*f0_fex.vab(1).lonstep - 180.
...
f0_tmp=f0_sdf(where(f0_sdf.attitude.terr_rad_byte ne 4))
;code ='data_fdq=f0_fdq'+'.'+field_fdq
;status=execute(code)

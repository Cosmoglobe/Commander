...
C-------------------------------------------------------------------------------
	...
	Integer*4 Function FTB_Check_Pre_IT_Sci_Time 
...
C		STX, November, 1988
C
C	Invocation: Status = FTB_Check_Pre_IT_Sci_Time 
C		    ( Channel, Sci_Rec, First, Eof, Rpt_Lun, Dump )
C
CH	Change Log:
CH
C	  ----------------------------------------------------------------------
...
!	Local Declarations.
	...
	Integer*4  Transmit_Time(2)     ! Computed transmit time
	Character*14 Transmit_Gmt       ! Corresponding Ascii transmit time
...
c
	...
	FTB_Check_Pre_IT_Sci_Time = %loc(FTB_Normal)
	Check = .False.
	...
	Call CT_Binary_to_GMT (Sci_Rec(Start).CT_Head.Time , Contime)
c
...
!	Compute the transmit time by calling the FIRAS Stripper's routine
!       with transmit and collect frames in reverse order and reversing the 
!       CT_Header time and Mid_Point_of_Collect time in the calling sequence.


	Neg_Sweeps = - Sweeps_per_IFG

	Status = FTB_If_Midpoint_Time ( Transmit_Frame, Collect_Frame, 
	1    Length, Speed, Neg_Sweeps, Time1, Transmit_Time )

	Call CT_Binary_to_GMT ( Transmit_Time , Transmit_Gmt )

	If ( Time_GT( Time2, Time1 )) Then	! Time OK on current records 
	  If (Eof) Time_Flag = 'EF'
	  If ( Fault .OR. Eof ) Then            ! Previous time seq. bad
	    Status = FTB_Dump_Sci_Time ( Channel, Gmt, Status_Bits, 
 ...
 100         Format(1x, 130a)
	...
	If ((Check) .AND. (Status .NE. %loc(FTB_Normal))) Then	    
	  FTB_Check_Pre_IT_Sci_Time = %loc(FTB_Aberr)
	Endif

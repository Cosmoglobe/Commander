...
c------------------------------------------------------------------------------
c
c     Purpose:  To rewrite output of FFL (FIRAS_Fish_LongInput) to ASCII files 
c               so that they may be used by FISH.
c
c     Author:  Alice Trenholme, GSC, 12/95, SPR 12285.
c
c     Modifications:
c
c------------------------------------------------------------------------------
...
c 
      ...
      real*4 adj_nifgs,temps(7),temp_sigs(7)
      real*8 time,volt,bias,sigma(360)
      complex*16 spec(360)
c
...
c
      ...
      rstatus = cut_display_banner(lun_out,num_vol,'FIRAS Facility FTB_LTrans')
c
...
c
     ...
     .     recordtype='fixed',recl=2183,iostat=iostatus)
     ...
     .         nifgs,adj_nifgs,time,temps,temp_sigs,volt,bias,gain,spec,sigma

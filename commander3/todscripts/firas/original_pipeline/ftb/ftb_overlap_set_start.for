C-----------------------------------------------------------------------------
	...
	Integer*4 Function FTB_Overlap_Set_Start ( Cat_Rec, Report, Report_LUN )

C-----------------------------------------------------------------------------
C
C	Purpose: To refine an overlapping segment start time by using the
C		 timetag of the first raw science record in the overlapping
C		 timerange.
C		 
C
C       Author:  R. Kummerer / STX, October 31, 1989
C
C   	Invocation:  Status = FTB_Overlap_Set_Start ( Cat_Rec, Report,
C						      Report_LUN )
C
CH	Change Log:
CH
...
C-----------------------------------------------------------------------------
C
C	 Processing Method : PDL for FTB_Overlap_Set_Start
C
C			PDL for FTB_Overlap_Set_Start
C			--------------------------------
C
...
C	      timerange
C	 CALL CT_READ_ARCV to read the first raw science record
...
C Read the entry from the archive.
	...
	1			Cat_Rec.Dataset_Name, %Val(CT_Status(1)) )
		 ...
		 Endif
	      Else
		 Cat_Rec.Initial_Time(1) = SCI_REC.CT_Head.Time(1)
		 Cat_Rec.Initial_Time(2) = SCI_REC.CT_Head.Time(2)
	      Endif
...
C Close the file.
	...
	If (Retstat .Eq. Success) Then
	   FTB_Overlap_Set_Start = %Loc(FTB_Normal)
	Else
	   FTB_Overlap_Set_Start = %Loc(FTB_Aberr)
	Endif

C	Formats statements.

10	Format (//, 5x, '*** FTB_Overlap_Set_Start CT READ error = ', i)
20	Format (//, 5x, '*** FTB_Overlap_Set_Start CT CLOSE error = ', i)
30	Format (//, 5x, '*** FTB_Overlap_Set_Start LIB$FREE_LUN error = ', i)
40	Format (//, 5x, '*** FTB_Overlap_Set_Start CT OPEN error = ', i)
50	Format (//, 5x, '*** FTB_Overlap_Set_Start LIB$GET_LUN error = ', i)

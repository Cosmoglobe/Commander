...
channel_dependent = ["ifg", "bol_cmd_bias", "bol_volt", "gain"]
...
for mode, mode_value in modes.items():
    for channel, channel_value in channels.items():
        if not (mode == "lf" and (channel == "lh" or channel == "rh")):
            ...
            etf = (
                fits_data[1].data["RELEX_GA"][0] + 1j * fits_data[1].data["IELEX_GA"][0]
            )
            etf = etf[np.abs(etf) > 0]
            ...
            cal[f"{channel}_{mode}"] = (
                ...
                - (
                    ...
                    + (bb_bolometer_rh * bolometer_emiss)
...
np.savez(g.PROCESSED_DATA_PATH_CAL, **variablesm, **cal, **spec)

...
for i in range(i0, i0 + 1):
    ...
    for j, mode in enumerate(["ll", "lh", "rh", "rl"]):
        ...
        inds = np.where((sci[f"fdq_sdf_{mode}/dq_data/eng_time"][()] == eng_times[i]))[
            0
        ]
        print(inds)
        if len(inds) > 0:
            t = pd.to_datetime(
                sci[f"fdq_sdf_{mode}/ct_head/gmt"][inds[0]].astype(str),
                format="%y%j%H%M%S%f",
            )
            ...
            axes[j].plot(
                sci[f"fdq_sdf_{mode}/ifg_data/ifg"][inds[0]], color=f"C{xcal_pos}"
            )
    if xcal_pos == 1:
        fig.suptitle(
            f"{lengths[length]}{speeds[speed]}, XCAL = {xcal_tip:.3f}, ical = {ical:.3f}"
        )
    else:

...
emissivity_changes = [f"emissivity_{i}" for i in range(47)]
change_parameters = [
    "xcal",
    "bol_cmd_bias",
    "bol_volt",
...
change_parameters = emissivity_changes + change_parameters
...
for param in change_parameters:
    print(param)
    detector_factors = {}
    for param2 in detector_factors_defaults.keys():
        if param == param2:
            detector_factors[param2] = detector_factors_defaults[param2]
        else:
            detector_factors[param2] = 1
    if param == "xcal":
        detector_factors["xcal"] = 1.005
    else:
        detector_factors["xcal"] = 1
    if param == "ical":
        detector_factors["ical"] = 1.001
    else:
        detector_factors["ical"] = 1
    emissivity_factors = np.ones(210)
    for j in range(len(emissivity_changes)):
        if param == f"emissivity_{j}":
            emissivity_factors[j] = 1.5
            detector_factors[param] = 1.5
    ...
    bolom_emiss = (
        pub_model[1].data["RBOLOMET"][0] + 1j * pub_model[1].data["IBOLOMET"][0]
    )
    refhorn_emiss = (
        pub_model[1].data["RREFHORN"][0] + 1j * pub_model[1].data["IREFHORN"][0]
    )
    skyhorn_emiss = (
        pub_model[1].data["RSKYHORN"][0] + 1j * pub_model[1].data["ISKYHORN"][0]
    )
    dihedral_emiss = (
        pub_model[1].data["RDIHEDRA"][0] + 1j * pub_model[1].data["IDIHEDRA"][0]
    )
    struct_emiss = (
        pub_model[1].data["RSTRUCTU"][0] + 1j * pub_model[1].data["ISTRUCTU"][0]
    )
    ...
    otf257 = np.zeros(257, dtype=complex)
    otf257[offset : offset + NUM_FREQ] = otf
    ...
    icals = eng["en_analog/grt/b_lo_ical"][()] * detector_factors["ical"]
    xcals = eng["en_analog/grt/b_lo_xcal_cone"][()] * detector_factors["xcal"]
    ...
    a_bol_assem_rh = eng["en_analog/grt/a_lo_bol_assem"][:, 0]
    a_bol_assem_rl = eng["en_analog/grt/a_lo_bol_assem"][:, 1]
    a_bol_assem_lh = eng["en_analog/grt/a_lo_bol_assem"][:, 2]
    b_bol_assem_lh = eng["en_analog/grt/b_lo_bol_assem"][:, 2]
    a_bol_assem_ll = eng["en_analog/grt/a_lo_bol_assem"][:, 3]
    b_bol_assem_ll = eng["en_analog/grt/b_lo_bol_assem"][:, 3]
    ...
    Tbol = (
        a_bol_assem_ll + b_bol_assem_ll
    ) / 2  # TODO: change this when we have the new temperatures
    Tbol = Tbol[eng_inds]
    ...
    spec_mbb[0][offset : offset + NUM_FREQ] = (
        BlackBody(temperature=20 * u.K)(nu).value
        * (nu / (353 * u.GHz)).to("").value ** 1.6
    )
    spec_mbb[1][offset : offset + NUM_FREQ] = BlackBody(temperature=10 * u.K)(nu).value
    spec_mbb[2][offset : offset + NUM_FREQ] = BlackBody(temperature=2.73 * u.K)(
        nu
    ).value
    ...
    ifg_mbb = ifg_spec.spec_to_ifg(
        ...
        channel=ch,
        mode=sm,
        adds_per_group=adds_per_group[:5],
    ...
    plt.savefig(f"ifg_spec_atlas_{param}.png", bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(f"ifg_powlaw_atlas_{param}.png", bbox_inches="tight")
    ...
    for i in tqdm(range(len(spec_th))):
        ...
        if i == 0:
            ...
            axes[0].plot(
                nu, (bb_skyhorn(nu) * skyhorn_emiss).to("MJy/sr") / otf, label="SKYHORN"
            )
            axes[0].plot(
                nu, (bb_refhorn(nu) * refhorn_emiss).to("MJy/sr") / otf, label="REFHORN"
            )
            axes[0].plot(
                nu, (bb_dihedr(nu) * dihedral_emiss).to("MJy/sr") / otf, label="MIRROR"
            )
            axes[0].plot(
                nu, (bb_bolassem(nu) * bolom_emiss).to("MJy/sr") / otf, label="BOL"
            )
            axes[0].plot(
                nu, (bb_struct(nu) * struct_emiss).to("MJy/sr") / otf, label="STRUCT"
            )
        # Equation (14) of the Explanatory Supplement.
        R = (
            bb_ical(nu) * ical_emiss
        ...
        spec_ical[i][offset : offset + NUM_FREQ] = (bb_ical(nu) * ical_emiss).to(
            "MJy/sr"
        ) / otf
        spec_skyh[i][offset : offset + NUM_FREQ] = (bb_skyhorn(nu) * skyhorn_emiss).to(
            "MJy/sr"
        ) / otf
        spec_refh[i][offset : offset + NUM_FREQ] = (bb_refhorn(nu) * refhorn_emiss).to(
            "MJy/sr"
        ) / otf
        spec_dih[i][offset : offset + NUM_FREQ] = (bb_dihedr(nu) * dihedral_emiss).to(
            "MJy/sr"
        ) / otf
        spec_bol[i][offset : offset + NUM_FREQ] = (bb_bolassem(nu) * bolom_emiss).to(
            "MJy/sr"
        ) / otf
        spec_struct[i][offset : offset + NUM_FREQ] = (bb_struct(nu) * struct_emiss).to(
            "MJy/sr"
        ) / otf
    ...
    plt.savefig(f"calibrated_versus_model_{param}.png", bbox_inches="tight")
    ...
    ifg_th = ifg_spec.spec_to_ifg(
        ...
        channel=ch,
        mode=sm,
        adds_per_group=adds_per_group[: len(spec_th)],
    ...
    ifg_xcal = ifg_spec.spec_to_ifg(
        ...
        channel=ch,
        mode=sm,
        adds_per_group=adds_per_group[: len(spec_xcal)],
        bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
        bol_volt=bol_volt[: len(spec_th)],
        fnyq_icm=fnyq_icm,
        Tbol=Tbol[: len(spec_th)],
    ...
    ifg_ical = ifg_spec.spec_to_ifg(
        ...
        channel=ch,
        mode=sm,
        adds_per_group=adds_per_group[: len(spec_ical)],
        bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
        bol_volt=bol_volt[: len(spec_th)],
        fnyq_icm=fnyq_icm,
        Tbol=Tbol[: len(spec_th)],
    ...
    plt.title(rf"{param}*{detector_factors[param]}, sim_ifg")
    plt.ylim([-2100, 4500])
    plt.savefig(f"xcal_ifg_{param}.png", dpi=200)

...
b_bol_assem_rh = eng["en_analog/grt/b_lo_bol_assem"][:, 0]
b_bol_assem_rl = eng["en_analog/grt/b_lo_bol_assem"][:, 1]
...
plt.savefig(f"ifgs_medsub_{channels[ch]}.png", bbox_inches="tight")
...
axes[1].plot(dtrf)
plt.savefig(f"ifg_med_{channels[ch]}.png", bbox_inches="tight")
...
ifg_skyh = mu.spec_to_ifg(
    spec=spec_skyh,
    channel=ch,
    mode=sm,
    adds_per_group=adds_per_group[: len(spec_skyh)],
    bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
    bol_volt=bol_volt[: len(spec_th)],
    fnyq_icm=fnyq_icm,
    Jo=Jo,
...
ifg_refh = mu.spec_to_ifg(
    spec=spec_refh,
    channel=ch,
    mode=sm,
    adds_per_group=adds_per_group[: len(spec_refh)],
    bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
    bol_volt=bol_volt[: len(spec_th)],
    fnyq_icm=fnyq_icm,
    Jo=Jo,
...
ifg_dih = mu.spec_to_ifg(
    spec=spec_dih,
    channel=ch,
    mode=sm,
    adds_per_group=adds_per_group[: len(spec_dih)],
    bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
    bol_volt=bol_volt[: len(spec_th)],
    fnyq_icm=fnyq_icm,
    Jo=Jo,
...
ifg_struct = mu.spec_to_ifg(
    spec=spec_struct,
    channel=ch,
    mode=sm,
    adds_per_group=adds_per_group[: len(spec_struct)],
    bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
    bol_volt=bol_volt[: len(spec_th)],
    fnyq_icm=fnyq_icm,
    Jo=Jo,
...
ifg_bol = mu.spec_to_ifg(
    spec=spec_bol,
    channel=ch,
    mode=sm,
    adds_per_group=adds_per_group[: len(spec_bol)],
    bol_cmd_bias=bol_cmd_bias[: len(spec_th)],
    bol_volt=bol_volt[: len(spec_th)],
    fnyq_icm=fnyq_icm,
    Jo=Jo,
...
plt.plot(
    nu,
    np.nanmedian(spec_out.real, axis=0)[offset : offset + NUM_FREQ],
    color="C0",
    label="Data",
)
plt.plot(
    nu,
    np.nanmedian(spec_th.real, axis=0)[offset : offset + NUM_FREQ],
    color="C1",
    label="Model",
)
plt.xlabel("Frequency [GHz]")

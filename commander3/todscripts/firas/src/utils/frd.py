...
def apod_fcnl(ictr, good_peak, short, xhsf, xhlf):
    ...
    if not good_peak:
        ...
        x = (
            ictr - 1.0
        )  # since x is a RHS divisor and ictr is off-by-one compared to Fortran
    else:
        ...
        if ictr < halflim:
            x = 1.0 + upperlim - ictr
            for j in prange(twidth - 1):
                apod_fcn[j + lowerlim] = 0.5 * (1.0 - np.cos((j + 1) * np.pi / twidth))
                apod_fcn[j + int(2 * ictr) - lowerlim - twidth + 2] = 0.5 * (
                    3.0 - np.cos((j + 1) * np.pi / twidth)
                )
            for j in prange(int(2 * ictr) - lowerlim + 1, upperlim + 1):
                apod_fcn[j] = 2.0
        else:
            x = ictr + 1.0 - lowerlim
            for j in prange(twidth - 1):
                apod_fcn[j + int(2 * ictr) - upperlim] = 0.5 * (
                    3.0 + np.cos((j + 1) * np.pi / twidth)
                )
                apod_fcn[j + upperlim - twidth + 2] = 0.5 * (
                    1.0 + np.cos((j + 1) * np.pi / twidth)
                )
            for j in prange(lowerlim, int(2 * ictr) - upperlim):
                apod_fcn[j] = 2.0

    # Smooth the apodization function. This is the only place "x" appears
    # in this function
    for j in prange(lowerlim, upperlim + 1):
...
def apodl():
    ...
    short = 0
    nrec = 0
    for ngroup in prange(1, 13):
        for mtm_speed in prange(2):
            for mtm_length in prange(2):
                for chan in prange(1, 3):  # low/high (LL/RL, LH/RH)
                    xhsf = (chan - 1) * (1 - mtm_length) * mtm_speed
                    xhlf = (chan - 1) * mtm_length * mtm_speed
                    for upmode in prange(1, 3):
                        for linearized in prange(2):
                            # For each instrument state, determine the IFG peak position, then generate
                            # the apoziation function
                            ictr, rstatus = fut.default_peak(
                                mtm_speed, mtm_length, chan, ngroup, upmode, linearized
                            )
...
def elex_transfcnl(samprate, nfreq):
    ...
    nrec = 0
    for isampl in prange(3):  # 0, 1 = MTM modes, 2=fakeit
        fsamp = samp_hz[isampl]
        for ichan in prange(4):
            tauboost = tau[ichan]
            for micro in prange(2):  # dig fltr on/off (opposite order than apodization)
                for ncompress in prange(1, 13):
...
def _digfltr(freqhz, micromode, ichan, samplrate):
    ...
    zdigfil = 1.0
    if micromode == 1:
        return zdigfil  # digital filter off

...
@njit(parallel=True)
def apod_recnuml(channel, scan_mode, adds_per_group):
    ...
    ---- Changed to be able to handle arrays of values ----
    ...
    np.where((channel == 0) | (channel == 2), ch, 1)
    np.where((channel == 1) | (channel == 3), ch, 0)

    # Determine whether digital filters are on or off - all sci_modes should be 4 (digital filters on)
    sc = np.zeros_like(channel) * 1

    recnum = np.zeros_like(scan_mode)

    # Apodization function for fake-it data is the last one (432); functions for FS and FL data are after 383
    np.where(scan_mode >= 4, recnum, 384 + 4 * (adds_per_group - 1) + 2 * sc)
    np.where(
        scan_mode < 4,
        recnum,
        32 * (adds_per_group - 1)
...
# @njit(parallel=True)
def get_recnum(mtmspeed, ichan, iadds_grp):
    ...
    irecno += 24 * ichan
    idig_fltr = np.zeros_like(
        irecno
    )  # science mode is always 4 so digital filter is always on

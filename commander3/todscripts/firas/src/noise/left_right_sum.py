...
if __name__ == "__main__":
    ...
    for channel in g.CHANNELS.keys():
        ...
        for mode in g.MODES.keys():
            ...
            ifg = cal_data["ifg"][mode_filter]
            ifg = ifg - np.median(ifg, axis=1, keepdims=True)

            gain = cal_data["gain"][mode_filter]
            sweeps = cal_data["sweeps"][mode_filter]
            ...
            ifg_norm[f"{channel}_{mode}"] = (
                ifg / gain[:, np.newaxis] / sweeps[:, np.newaxis]
            )

            print(f"Shape of ifg: {ifg.shape}")


# TODO: match right/left in order to be able to compare
ifg = ifg_norm
ifg_low_normr = ifg["rl"] - (
    ifg["ll"]
    * np.divide(
        ifg["rl"][:, g.PEAK_POSITIONS["rl_ss"]],
        ifg["ll"][:, g.PEAK_POSITIONS["ll_ss"]],
        out=np.zeros_like(ifg["rl"][:, g.PEAK_POSITIONS["rl_ss"]]),
        where=ifg["ll"][:, g.PEAK_POSITIONS["ll_ss"]] != 0,
    )[:, np.newaxis]
)
ifg_low_norml = ifg["ll"] - (
    ifg["rl"]
    * np.divide(
        ifg["ll"][:, g.PEAK_POSITIONS["ll_ss"]],
        ifg["rl"][:, g.PEAK_POSITIONS["rl_ss"]],
        out=np.zeros_like(ifg["ll"][:, g.PEAK_POSITIONS["ll_ss"]]),
        where=ifg["rl"][:, g.PEAK_POSITIONS["rl_ss"]] != 0,
    )[:, np.newaxis]
)
ifg_high_normr = ifg["rh"] - (
    ifg["lh"]
    * np.divide(
        ifg["rh"][:, g.PEAK_POSITIONS["lh_ss"]],
        ifg["lh"][:, g.PEAK_POSITIONS["rh_ss"]],
        out=np.zeros_like(ifg["lh"][:, g.PEAK_POSITIONS["lh_ss"]]),
        where=ifg["lh"][:, g.PEAK_POSITIONS["rh_ss"]] != 0,
    )[:, np.newaxis]
)
ifg_high_norml = ifg["lh"] - (
    ifg["rh"]
    * np.divide(
        ifg["lh"][:, g.PEAK_POSITIONS["lh_ss"]],
        ifg["rh"][:, g.PEAK_POSITIONS["rh_ss"]],
        out=np.zeros_like(ifg["lh"][:, g.PEAK_POSITIONS["lh_ss"]]),
        where=ifg["rh"][:, g.PEAK_POSITIONS["rh_ss"]] != 0,
    )[:, np.newaxis]
)

n = random.randint(0, ifg["rh"].shape[0])

max_amp_high = np.max([np.abs(ifg["lh"][n]), np.abs(ifg["rh"][n])])
max_amp_low = np.max([np.abs(ifg["ll"][n]), np.abs(ifg["rl"][n])])
print(f"Shape of max_amp_high: {max_amp_high.shape}")

fig, ax = plt.subplots(4, 2, figsize=(10, 6))

ax[0, 0].plot(ifg["rh"][n])
...
print(f"Nyquist frequency LL: {fnyq_ll} Hz")

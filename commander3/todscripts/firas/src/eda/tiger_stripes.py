...
print(
    f"Filtering {np.sum(ical_mask)} pixels out of {len(ical)} where the temperature of the ICAL is bigger than T_CMB"
)
...
print(
    f"Filtering {np.sum(xcal_mask)} pixels out of {len(ical)} where the temperature of the ICAL is smaller than T_CMB"
)
...
period_filter[0] = (gmt < mission_periods_dt[0]) & (gmt > start_dt)
...
for channel in channels:
    ...
    f_ghz = utils.generate_frequencies(channel, "ss")
    ...
    for freqi in range(len(f_ghz)):
        hp.mollview(
            m_ical[:, freqi],
            title=f"{int(f_ghz[freqi]):04d} GHz",
            min=1,
            max=max_amp[channel],
            unit="MJy/sr",
            norm="log",
    ...
    for j, period in enumerate(period_filter):
        ...
        utils.save_mollview(m, f_ghz, save_path, max_amp[channel], norm="log")

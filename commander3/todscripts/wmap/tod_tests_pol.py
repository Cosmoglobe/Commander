...
axes_test.plot(
    time - time[0], p, "k.", ms=1, alpha=0.2, zorder=-1, label="p timestream"
)
...
for i in range(len(list(f.keys())) - 1):
    ...
    for num, label in enumerate(labels):
        ...
        if label == f"{band}13":
            pixA.append(
                h.Decoder(np.array(f[obsid + "/" + label + "/pixA"]))
                .astype("int")
                .tolist()
            )
            pixB.append(
                h.Decoder(np.array(f[obsid + "/" + label + "/pixB"]))
                .astype("int")
                .tolist()
            )
            psiA.append(
                psiBins[
                    h.Decoder(np.array(f[obsid + "/" + label + "/psiA"])).astype("int")
                ]
            )
            psiB.append(
                psiBins[
                    h.Decoder(np.array(f[obsid + "/" + label + "/psiB"])).astype("int")
                ]
            )
...
for t in range(len(pixA)):
    d_sol[t] = (
        ...
        * (
            (
                ...
                + u_sol[pixA[t]] * np.sin(2 * psiA[t])
                + s_sol[pixA[t]]
            )
            - (
                q_sol[pixB[t]] * np.cos(2 * psiB[t])
                + u_sol[pixB[t]] * np.sin(2 * psiB[t])
                + s_sol[pixB[t]]
            )
        )
    )
    ...
    d_solA[t] = (1 + xbar) * (i_sol[pixA[t]]) + dxbar * (
        (
            q_sol[pixA[t]] * np.cos(2 * psiA[t])
            + u_sol[pixA[t]] * np.sin(2 * psiA[t])
            + s_sol[pixA[t]]
    ...
    d_solB[t] = -(1 - xbar) * (i_sol[pixB[t]]) - dxbar * (
        (
            q_sol[pixB[t]] * np.cos(2 * psiB[t])
            + u_sol[pixB[t]] * np.sin(2 * psiB[t])
            + s_sol[pixB[t]]

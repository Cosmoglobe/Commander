...
ra_from, ra_to = np.deg2rad([-45, 80])
...
alm = curvedsky.map2alm(imap, lmax=2*nside)
...
#inds = (lon > box[0][0]) & (lon < box[1][0]) & (lat > box[0][1]) & (lat < box[1][0])
...
#cosmoglobe.plot(simulated_emission,
#    sig=1, min=-10, max=10)
#
emission_GC = rot_gal2eq.rotate_map_alms(simulated_emission)

cosmoglobe.plot(TQU, min=-10, max=10, sig=1, fwhm=0.1*u.deg)
...
cosmoglobe.plot(emission_GC,
    sig=1, min=-10, max=10, xsize=8000, cbar=False)

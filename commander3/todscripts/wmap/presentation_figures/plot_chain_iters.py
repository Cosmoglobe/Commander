...
data = h5py.File(
    "/mn/stornext/d16/cmbco/bp/dwatts/WMAP/chains_WMAP_burnin_Q1_220501/chain_c0001.h5",
    "r",
)
thin = 1
burn = 1
...
bands = ["030-WMAP_Ka"]
label_list = [["Ka113", "Ka114", "Ka123", "Ka124"]]
bands = ["023-WMAP_K"]
label_list = [["K113", "K114", "K123", "K124"]]
bands = ["040-WMAP_Q1"]
label_list = [["Q113", "Q114", "Q123", "Q124"]]
# bands = ['040-WMAP_Q2']
# label_list = [
#         ['Q213', 'Q214', 'Q223', 'Q224']
#         ]
    ...
    for j in range(4):
        ...
        if j == 0:
            for k in range(1, len(g) + 1):
                ...
                orig = h5py.File(
                    f"{DATA_DIR}/wmap_{labels[j][:-2]}_{str(k).zfill(6)}.h5"
                )
        ...
        for i in range(burn, len(data.keys()) - 1, thin):
            ...
            c = "k"
            inds = (
                (data[str(i).zfill(6) + f"/tod/{band}/accept"][j] == 1)
                & (abs(data[str(i).zfill(6) + f"/tod/{band}/chisq"][j]) < 100)
                & (data[str(i).zfill(6) + f"/tod/{band}/gain"][j] > 0)
            )
            inds = np.array(inds)
            ...
            axes[1].plot(
                t[inds],
                data[str(i).zfill(6) + f"/tod/{band}/xi_n"][1][j][inds],
                ".",
                color=c,
                ms=1,
            )
            axes[2].plot(
                t[inds],
                data[str(i).zfill(6) + f"/tod/{band}/xi_n"][2][j][inds],
                ".",
                color=c,
                ms=1,
            )
            ...
            axes[4].plot(
                t[inds],
                data[str(i).zfill(6) + f"/tod/{band}/chisq"][j][inds],
                color=c,
                ms=1,
            )
            ...
            if i > 0:
                plt.savefig(
                    f"plots/{labels[j]}_iter_{str(i).zfill(4)}.png", bbox_inches="tight"
                )
            plt.close()

...
fnames = glob(
    "/mn/stornext/d16/cmbco/bp/dwatts/WMAP/chains_WMAP_beamtest/tod_K113_pid000021_samp*.dat"
)
...
fnames = glob(
    "/mn/stornext/d16/cmbco/bp/dwatts/WMAP/chains_WMAP_beamtest/tod_K113_pid000033_samp*.dat"
)
fnames.sort()
...
for fname in fnames:
    ...
    axes[0].plot(
        t[ind], gain[ind] * (s_totA[ind] - s_totB[ind]) + baseline[ind], label=r"$gs+b$"
    )
    ...
    axes[1].plot(
        t[ind],
        d[ind] - gain[ind] * (s_totA[ind] - s_totB[ind]) - baseline[ind] - ncorr[ind],
        "k.",
        ms=2,
    )

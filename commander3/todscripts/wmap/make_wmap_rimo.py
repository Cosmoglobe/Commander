    ...
    with h5py.File(fname_out, "a") as f:
        ...
        for i in range(len(fnames)):
            ...
            for j in range(len(labels)):
                print(labels[j], band[:-1], len(labels))
                if labels[j] == band[:-1]:
        ...
        for i in range(len(labels)):
            ...
            f.create_dataset(full_labels[i] + "/bandpassx", data=nu)
            f.create_dataset(full_labels[i] + "/bandpass", data=B / sum(B))
        ...
        radii = (
            np.pi / 180 * np.array([2.8, 2.5, 2.2, 2.2, 1.8, 1.8, 1.5, 1.5, 1.5, 1.5])
        )
        ...
        for beam_ind, fname in enumerate(fnames):
            ...
            inds = (abs(xx.flatten() - mu_x) < 6 * sd_x) & (
                abs(yy.flatten() - mu_y) < 6 * sd_y
            )
            ...
            popt, pcov = curve_fit(
                ...
                beamA.flatten()[inds],
                p0=p0,
                sigma=sigmA.flatten()[inds],
                bounds=(
                    (
                        np.array([0, -np.inf, -np.inf, 0, 0, -np.pi / 4]),
            ...
            r = hp.rotator.Rotator(rot=(0, 0, psi), deg=False, eulertype="Y")
...
if __name__ == "__main__":
    fname_out = "/mn/stornext/d16/cmbco/bp/dwatts/WMAP/data_WMAP/WMAP_instrument_v14.h5"
    # fname_out = 'test.h5'

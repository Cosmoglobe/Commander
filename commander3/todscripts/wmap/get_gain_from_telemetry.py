...
def get_gain(data, band):
    ...
    included here.
    """
    t_RJD = data[60].data["RJD"]
    t_JD = t_RJD + 2.45e6
    # 2452131.5 is 0:00 of day 222 of 2001 (August 10)
    t = t_JD - 2452131.50000
    t_MJD = t_JD - 2400000.5

    match = [(f'DR{band}RFB' in s) for s in data[60].columns.names]
    ind = np.where(match)[0]
    if len(ind) != 0:
        ind = ind[0]
        names = data[60].columns.names
        d = data[60]
    else:
        match = [(f'DR{band}RFB' in s) for s in data[61].columns.names]
        ind = np.where(match)[0]
        ind = ind[0]
        d = data[61]
        names = data[61].columns.names
    RF_bias = d.data[names[ind]]

    match = [f'DF{band[:-2]}' in s for s in data[60].columns.names]
    ind = np.where(match)[0]
    if len(ind) != 0:
        ind = ind[0]
        names = data[60].columns.names
        ...
        match = [f'DF{band[:-2]}' in s for s in data[61].columns.names]
        ind = np.where(match)[0]
        if len(ind) == 0:
            match = [f'DFW3' in s for s in data[61].columns.names]
            ind = np.where(match)[0]
        ind = ind[0]
        d = data[61]
        names = data[61].columns.names
    T_FPA = d.data[names[ind]]

    match = [(f'DR{band[:-2]}' in s) & ('RXB' in s) for s in data[60].columns.names]
    ind = np.where(match)[0]
    if len(ind) != 0:
        ind = ind[0]
        names = data[60].columns.names
        ...
        match = [((f'DR{band[:-2]}' in s) & ('RXB' in s)) for s in data[61].columns.names]
        ind = np.where(match)[0]
        if len(ind) == 0:
            match = [((f'DRW3' in s) & ('RXB' in s)) for s in data[61].columns.names]
            ind = np.where(match)[0]
        names = data[61].columns.names
        ind = ind[0]
        d = data[61]
    T_RXB = d.data[names[ind]]
...
def fullgain_tests():
    files = glob(prefix + "tods/telemetry/*.fits")
    files.sort()
    #files = np.array(files)
    bands = ["K1", "KA1", "Q1", "Q2", "V1", "V2", "W1", "W2", "W3", "W4"]
    bands = [[b + "13", b + "14", b + "23", b + "24"] for b in bands]
    ...
    fig, axes = plt.subplots(nrows=10, ncols=4, figsize=(16, 10), sharex=True)
    axs = axes.flatten()
    #for f in files[10:100]:
    #for j, f in enumerate(files[100:]):
    times = []
    gains = []
    for i in range(len(bands)):
        gains.append([])
    print(len(files[10:]))
    for j, f in enumerate(files[10:]):
    #for j, f in enumerate(files[10:110]):
        if (j % 10) == 0:
            print(j)
        data = fits.open(f)
        for i, b in enumerate(bands):
            t_MJD, G_b = get_gain(data, b)
            axs[i].plot(t_MJD, abs(G_b), color='k')
            if i == 0:
                times += t_MJD.tolist()
            gains[i] += G_b.tolist()
            #axs[i].set_title(b)
    ...
    gains = np.concatenate((times[None, :], gains)).astype(np.float32)
    plt.tight_layout()
    plt.savefig('wmap_gains.png', bbox_inches='tight')
    np.save('WMAP_gain', gains)

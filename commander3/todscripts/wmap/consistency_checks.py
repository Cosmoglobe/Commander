...
if plot_map:
    for band in range(len(gal)):
        ...
        plt.savefig(
            f"plots/time_ordered_pointing_{band_labels[band]}.png", bbox_inches="tight"
        )
        ...
        for i in range(len(gal[band])):
            hp.projscatter(
                gal[band, i, 0],
                gal[band, i, 1],
                color=plt.cm.viridis((sin2g[band][i] + 1) / 2),
                lonlat=True,
                s=1,
            )

        plt.savefig(
            f"plots/sin2gamma_stream_{band_labels[band]}.png", bbox_inches="tight"
        )
        ...
        for i in range(len(gal[band])):
            hp.projscatter(
                gal[band, i, 0],
                gal[band, i, 1],
                color=plt.cm.viridis((cos2g[band][i] + 1) / 2),
                lonlat=True,
                s=1,
            )

        plt.savefig(
            f"plots/cos2gamma_stream_{band_labels[band]}.png", bbox_inches="tight"
        )

...
def main():
    ...
    inst_file = inst.commander_instrument(
        outDir, wmap.instrument_filename(version), version, "w"
    )
    ...
    for freq in wmap.freqs:
        ...
        for horn in wmap.horns[freq]:
            for hornType in ["1", "2"]:
                nu, B3, B4 = np.loadtxt(
                    f"data/wmap_bandpass_{freq}{horn}{hornType}_v5.cbp"
                ).T
                ...
                # beam parameters
                inst_file.add_field(
                    prefix + "/fwhm", data=[wmap.fwhms[freq + str(horn)]]
                )

...
def get_data(fname, band, xbar, dxbar, nside=256, pol=False, mask=True):
    ...
    for obsind in range(len(obsids)):
        ...
        for num, label in enumerate(labels):
            ...
            if label == f"{band}13":
                pixA = h.Decoder(np.array(f[obsid + "/" + label + "/pixA"])).astype(
                    "int"
                )
                pixB = h.Decoder(np.array(f[obsid + "/" + label + "/pixB"])).astype(
                    "int"
                )
                if pol:
                    psiA = psiBins[
                        h.Decoder(np.array(f[obsid + "/" + label + "/psiA"])).astype(
                            "int"
                        )
                    ]
                    psiB = psiBins[
                        h.Decoder(np.array(f[obsid + "/" + label + "/psiB"])).astype(
                            "int"
                        )
                    ]
        ...
        p = 0.5 * (
            d1 - d2
        )  # = q_A*cos(2*g_A) + u_A*sin(2*g_A) - q_B*cos(2*g_B) - u_B*sin(2*g_B)
        ...
        if mask:
            pm = hp.read_map(
                "analysis_masks/wmap_processing_mask_K_yr2_r9_9yr_v5.fits",
                verbose=False,
                dtype=None,
            )
...
def innerproduct_pol(pixA, pixB, psiA, psiB, f_A, f_B, xbar, dxbar, x, nside=256):
    ...
    for t in tqdm(range(len(pixA))):
        Px_d = (
            ...
            - (1 - xbar) * T[pixB[t]]
            + dxbar
            * (Q[pixA[t]] * np.cos(2 * psiA[t]) + Q[pixB[t]] * np.cos(2 * psiB[t]))
            + dxbar
            * (U[pixA[t]] * np.sin(2 * psiA[t]) + U[pixB[t]] * np.sin(2 * psiB[t]))
            + dxbar * (S[pixA[t]] + S[pixB[t]])
        )
        Px_p = (
            dxbar * (T[pixA[t]] + T[pixB[t]])
            + (1 + xbar) * Q[pixA[t]] * np.cos(2 * psiA[t])
            - (1 - xbar) * Q[pixB[t]] * np.cos(psiB[t])
            + (1 + xbar) * U[pixA[t]] * np.sin(2 * psiA[t])
            - (1 - xbar) * U[pixB[t]] * np.sin(psiB[t])
            + (1 + xbar) * S[pixA[t]]
            - (1 - xbar) * S[pixB[t]]
        )
        ...
        Q_[pixA[t]] += f_A[t] * (
            np.cos(2 * psiA[t]) * (dxbar * Px_d + (1 + xbar) * Px_p)
        )
        Q_[pixB[t]] += f_B[t] * (
            np.cos(2 * psiB[t]) * (dxbar * Px_d - (1 - xbar) * Px_p)
        )
        U_[pixA[t]] += f_A[t] * (
            np.sin(2 * psiA[t]) * (dxbar * Px_d + (1 + xbar) * Px_p)
        )
        U_[pixB[t]] += f_B[t] * (
            np.sin(2 * psiB[t]) * (dxbar * Px_d - (1 - xbar) * Px_p)
        )
...
def innerproduct_pol_t(pixA, pixB, psiA, psiB, f_A, f_B, xbar, dxbar, x, nside=256):
    ...
    Px_p = (
        dxbar * (T[pixB] + T[pixB])
        + (1 + xbar) * Q[pixB] * np.cos(2 * psiA)
        - (1 - xbar) * Q[pixB] * np.cos(psiB)
        + (1 + xbar) * U[pixB] * np.sin(2 * psiA)
        - (1 - xbar) * U[pixB] * np.sin(psiB)
        + (1 + xbar) * S[pixB]
        - (1 - xbar) * S[pixB]
    )
...
def get_cg(
    ...
    funcs = [
        pool.apply_async(
            get_data, (fname, band, xbar, dxbar), dict(pol=pol, mask=mask, nside=nside)
        )
        for fname in fnames
    ]
    pixA = []
    ...
    if sparse_test or sparse_only:
        if pol:
            ...
            A = (1 + xbar) * sparse.csr_matrix(
                (f_A * flags / sigma0s, (times, pixA))
    ...
    if pol:
        ...
        while (i < imax) & (delta_new > eps**2 * delta_0):
            ...
            if sparse_only:
                ...
                q *= 0
                funcs = [
                    pool.apply_async(inner_productdq, args=[i])
                    for i in tqdm(range(len(pixA)))
                ]
                for res in funcs:
                    result = res.get()
                    pA, pB, dq = result
            ...
            if i % 50 == 0:
                ...
                r = r - alpha * q
            if i % 10 == 0:
                exp = 10.0 ** np.round(int(np.log10(delta_new)))
...
def plot_maps_pol(band="K1", version="13", nside=256):
    ...
    data = hp.ud_grade(
        hp.read_map(
            f"data/wmap_iqusmap_r9_9yr_{band}_v5.fits",
            verbose=False,
            field=(0, 1, 2, 3),
        ),
        nside,
    ...
    for i in range(4):
        ...
        hp.mollview(
            abs(x_tot[i] - data[i]), norm="hist", title="CG - WMAP", sub=(2, 2, i + 1)
        )
        ...
        hp.mollview(
            abs(x_tot[i] - data[i]), min=0, max=1, title="Difference", sub=(2, 2, i + 1)
        )
    ...
    plt.figure()
    lim = 0.05
    hp.mollview(x_smooth[1], sub=321, min=-lim, max=lim, title="CG Q", cmap="RdBu_r")
    hp.mollview(x_smooth[2], sub=322, min=-lim, max=lim, title="CG U", cmap="RdBu_r")
    hp.mollview(d_smooth[1], sub=323, min=-lim, max=lim, title="WM Q", cmap="RdBu_r")
    hp.mollview(d_smooth[2], sub=324, min=-lim, max=lim, title="WM U", cmap="RdBu_r")
    hp.mollview(
        x_smooth[1] - d_smooth[1],
        sub=325,
        min=-lim,
        max=lim,
        title="Diff Q",

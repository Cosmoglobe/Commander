...
for i, fname in enumerate(fnames):
    ...
    m_A, footprint_A = reproject.reproject_to_healpix(
        (data[0].data[0], target_header), "G", nside=nside_beam, order=3
    )

    m_B, footprint_B = reproject.reproject_to_healpix(
        (data[0].data[2], target_header), "G", nside=nside_beam, order=3
    )
    m_A[footprint_A == 0] = 0
    m_B[footprint_B == 0] = 0
...
for i in range(len(labels)):
    ...
    hp.mollview(beamtot, min=-0.5, max=0.5, cmap="RdBu_r", cbar=False, title=fname)
    hp.orthview(beamtot, min=-0.5, max=0.5, cmap="RdBu_r", cbar=False, title=fname)
    hp.graticule()
    ...
    hp.graticule()
    hp.gnomview(
        beam_A, cmap="Reds", min=0, max=0.5, rot=(0, 90, 0), title=fname, reso=20
    )
    hp.graticule()
    #
    # dir_B = dir_B_los[i]

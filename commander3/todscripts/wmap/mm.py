...
def orb_vel(kind="previous"):
    ...
    fx = interp1d(
        tod_inds, v_suns[:, 0], fill_value="extrapolate", assume_sorted=True, kind=kind
    )
    fy = interp1d(
        tod_inds, v_suns[:, 1], fill_value="extrapolate", assume_sorted=True, kind=kind
    )
    fz = interp1d(
        tod_inds, v_suns[:, 2], fill_value="extrapolate", assume_sorted=True, kind=kind
    )
...
def dot_prod(b_map, pixA, pixB, d, p, psiA, psiB, flags, pmask, x1, x2, sigma):
    """
...
def make_M(M, Prec, pixA, pixB, psiA, psiB, flags, pmask, x1, x2, sigma):
    ...
    P_A = scipy.sparse.csr_matrix(
        (data_A / sigma**2, (t, pixA)), shape=(len(t) // 4, 4 * npix)
    )
    P_B = scipy.sparse.csr_matrix(
        (data_B / sigma**2, (t, pixB)), shape=(len(t) // 4, 4 * npix)
    )
    P = P_A - P_B
...
def accumulate(tod_ind, x1, x2, x1hat, x2hat, pmask, band):
    ...
    s_dip_A = 1e3 * T_CMB * (b_dot_A + q * (b_dot_A**2 - b**2 / 3.0))
    s_dip_B = 1e3 * T_CMB * (b_dot_B + q * (b_dot_B**2 - b**2 / 3.0))
    ...
    M_i, Prec_i = make_M(
        M, Prec, pixA, pixB, psiA_i, psiB_i, flags, pmask, x1hat, x2hat, sigma
    )
    b_i = dot_prod(
        b_map, pixA, pixB, d, p, psiA_i, psiB_i, flags, pmask, x1hat, x2hat, sigma
    )

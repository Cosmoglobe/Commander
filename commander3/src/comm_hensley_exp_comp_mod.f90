...
!
!================================================================================
module comm_hensley_exp_comp_mod
  use comm_param_mod
  use comm_comp_mod
  ...
  private
  public comm_hensley_exp_comp

  !**************************************************
  !           Power-law component
  !**************************************************
  type, extends (comm_diffuse_comp) :: comm_hensley_exp_comp
   contains
     procedure :: S    => evalSED
  end type comm_hensley_exp_comp

  interface comm_hensley_exp_comp
     procedure constructor
  end interface comm_hensley_exp_comp
...
contains
  ...
  function constructor(cpar, id, id_abs)
    ...
    class(comm_hensley_exp_comp), pointer   :: constructor
  ...
  ! Definition:
  !    SED  = exp(1-(nu/theta(1)))
  ! where 
  !    nu_p = theta(1)
  function evalSED(self, nu, band, pol, theta)
    implicit none
    class(comm_hensley_exp_comp), intent(in)           :: self
    ...
    if (nu < 10d9) then
       ...
       evalSED = exp(1 - (nu/theta(1))**2)
    end if

  end function evalSED
  
end module comm_hensley_exp_comp_mod

...
contains
  ...
  subroutine smooth_spline(x, y, s, weight_type, alpha, variance)
    implicit none
    real(dp), dimension(:), intent(in)    :: x
    real(dp), dimension(:), intent(in)    :: y
    type(spline_type),      intent(out)   :: s
    character(len=*),       intent(in)    :: weight_type
    real(dp),               intent(in)    :: alpha         ! Stiffness
    ...
    if (trim(weight_type) == 'inv_var') then
       do i = 1, n
    ...
    else if (trim(weight_type) == 'uniform') then
       W = 0.d0
       do i = 1, n
          W(i) = 1.d0
       end do
    else
       write(*,*) 'smooth_spline: Unknown weighting scheme = ', trim(weight_type)
       stop
    end if
    ...
    if (info /= 0) then
       ...
       open(58,file='smooth_spline.dat')
       do i = 1, n
    ...
    do i = 1, n
       ...
       if (i > 1) Q_col(1) = s%y2(i-1)
                  Q_col(2) = s%y2(i)
       if (i < n) Q_col(3) = s%y2(i+1)

...
!
!================================================================================
submodule (comm_tod_iras_mod) comm_tod_iras_smod
contains

  !**************************************************
   !             Constructor
   !**************************************************
  module function constructor_iras(cpar, id, id_abs, info, tod_type) result(c)
      ! 
    ...
    !   type(comm_scandata) :: sd
      ...
      if (trim(c%freq) == 'IRAS_012') then
         band_samprate = 16.00 ! Hz
      else if (trim(c%freq) == 'IRAS_025') then
         band_samprate = 16.00 ! Hz 
      else if (trim(c%freq) == 'IRAS_060') then
         band_samprate = 8.00 ! Hz
      else if (trim(c%freq) == 'IRAS_100') then
         band_samprate = 4.00 ! Hz
      end if
...
!!$      end if
   ...
   !             Driver routine
   !**************************************************
   module subroutine process_iras_tod(self, chaindir, chain, iter, handle, map_in, delta, map_out, rms_out, map_gain)
      ! 
...
!               call write_hdf(tod_file, '/accept', real(self%scans(i)%d(:)%accept,dp))
   ...
   module subroutine apply_fast_flags_iras(self, sd)
   ...
   end subroutine apply_fast_flags_iras

   module subroutine construct_corrtemp_iras(self, sd, det)
     ! construct iras instrument-specific correction template from ramp template
     ! put into sd%s_inst
     ! ramp template is calculated in sample_ramp routine below and stored in self%tod_correction_templ
     implicit none
     class(comm_iras_tod),  intent(in)             :: self ! full data object with compressed tods
     ...
     !deallocate(res)
   end subroutine construct_corrtemp_iras
   ...
   module subroutine sample_ramp(self, sd)
     ...
     implicit none
     class(comm_iras_tod),  intent(inout)          :: self  ! full data object with compressed tods
...
end submodule comm_tod_iras_smod

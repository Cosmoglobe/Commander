...
contains
  ...
  !             Routine definitions
  !**************************************************
  function constructor_md(cpar, id, id_abs, band, label, mu, rms, def, unit) result(c)
    implicit none
...
!       stop
  ...
  function initialize_md_comps(cpar, id, id_abs, n) result(md_comps)
    ...
    n = 0
    open(funit, file=trim(cpar%cs_SED_template(1,id_abs)), recl=1024)
    do while (.true.)
       ...
       if (n == 0) then
          md_comps => comm_md_comp(cpar, id+n, id_abs, i, label, mu, rms, def, funit)
       else
          c => comm_md_comp(cpar, id+n, id_abs, i, label, mu, rms, def, funit)
          call md_comps%addComp(c)
       end if
       n = n+1
    end do
1   close(funit)

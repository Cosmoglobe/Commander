...
!     public :: initialize_zodi_mod, get_zodi_emission, update_zodi_spline_obj, init_zodi_spline_objects
    public comm_zodi, comm_zodi_ptr, initialize_zodi_mod
...
!         contains
!             procedure :: get_zodi_emission

!     end type comm_zodi


!     type comm_zodi_ptr
!         class(comm_zodi), pointer :: p => null()
!     end type comm_zodi_ptr


!     interface comm_zodi
!         procedure constructor
!     end interface comm_zodi
    ...
    integer(i4b) :: gauss_degree
    integer(i4b) :: n_interpolation_points=100
    real(dp) :: min_ipd_temperature=40, max_ipd_temperature=550, EPS = 3.d-14, R_cutoff, delta_t_reset
    logical(lgt) :: use_cloud, use_band1, use_band2, use_band3, use_ring, use_feature, &
                    use_unit_emissivity, use_unit_albedo

    real(dp), allocatable :: tabulated_earth_time(:), temperature_grid(:)
    real(dp), allocatable :: tabulated_earth_pos(:, :)
    type(spline_type) :: solar_irradiance_spline_obj
    type(spline_type) :: spline_earth_pos_obj(3), spline_obs_pos_obj(3)
...
contains
    subroutine initialize_zodi_mod(cpar)
        ...
        call init_model_parameters(cpar)
        ...
        ! MOVE THIS TO CONSTRUCTOR OF ZODI OBJ
    ...
    function constructor(tod)
        implicit none
        class(comm_tod),                     intent(inout) :: tod
        class(comm_zodi),     pointer             :: constructor
...
!     ! ----------------------------------------------------
    ...
    subroutine init_model_parameters(cpar)
    ...
    end subroutine init_model_parameters

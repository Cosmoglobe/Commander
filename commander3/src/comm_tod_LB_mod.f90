...
!                     & self%scans(i)%d(:)%accept)
          ...
          if (self%enable_tod_simulations) then !.and. (main_iter == 1)) then
            call wall_time(t1)
            call self%simulate_LB_tod(i, s_tot, handle)
...
!!$               & ', accept rate = ', real(naccept,sp) / ntot
  ...
  !
  ! ************************************************
  subroutine simulate_LB_tod(self, scan_id, s_tot, handle)
    use omp_lib
    implicit none
    class(comm_LB_tod), intent(inout) :: self
    ...
    !call MPI_Finalize(mpi_err)
    !stop
  end subroutine simulate_LB_tod

...
!!$         end if
  ...
  subroutine sample_admbbtab_mh(outdir, cpar, handle, handle_noise, l)
    implicit none
    character(len=*),               intent(in)    :: outdir
    type(planck_rng),               intent(inout) :: handle, handle_noise
    integer(i4b),                   intent(in)    :: l
    ...
    ! Should be updated to loop over polarization, but tabulated dust currently does not support?
    if (cpar%myid == 0) then
       write(*,fmt='(a,i3,a,a)') ' | MH astrodust MBBtab adscale sampling group ', l, ', active CG sampling groups = ', trim(cpar%mcmc_update_cg_groups(l))
       if (mh_accept_stat(l,1) > 30) then
          if (real(mh_accept_stat(l,2),sp)/mh_accept_stat(l,1) < 0.1d0) then
              write(*,*) 'MH MBBtab ADJUSTING STEP LENGTH for group', l
              mh_scale(l)         = mh_scale(l) / 2.
    ...
    do m = 1, cpar%mcmc_samp_group_numstep(l)
       ...
       do while (associated(c))
          k = k + 1
          select type (c)
          type is (comm_adMBBtab_comp)
          ! check if any values greater than npar in theta_steplen are greater than 0 (npar=2, for beta and T)
          ! these are only none-zero if they are meant to be sampled
              if (maxval(c%theta_steplen(c%npar+1:,l)) > 0) then
                mval = maxval(c%theta_steplen(c%npar+1:,l))
                mval_0 = max(mval, mval_0)
                exit
       ...
       do while (associated(c))
          ...
          select type (c)
          type is (comm_adMBBtab_comp)
           ! first value after npar is the adscale
             if (c%theta_steplen(c%npar+1,l) > 0.) then 
              if (cpar%myid_chain .eq. 0) then
               c%adscale_buff =c%adscale
               write(*,fmt='(a,a12)',advance='no') ' |   ', trim(c%label)
               write(*,fmt='(a,f16.8)',advance='no') ', adscale old = ', c%adscale
               c%adscale = c%adscale + rand_gauss(handle) * c%theta_steplen(c%npar+1,l) * mh_scale(l)
               write(*,fmt='(a,f16.8)') ', prop = ',  c%adscale
              end if 
             end if
            ! second value after npar and above are the tabulated values
            if (maxval(c%theta_steplen(c%npar+2:,l)) > 0) then
               if (cpar%myid_chain .eq. 0) then
                 c%SEDtab_buff = c%SEDtab
                 write(*,fmt='(a,a12)',advance='no') ' |   ', trim(c%label)
                 do i = 1, c%ntab
                    if (c%theta_steplen(c%npar+1+i,l) > 0) then
                       if (i == 1) then
                        write(*,fmt='(a,i4,a,f16.8)',advance='no') ', bin = ', i, ', old = ', c%SEDtab(2,i)
                       else
                        write(*,fmt='(a,i4,a,f16.8)',advance='no') ' |                 bin = ', i, ', old = ', c%SEDtab(2,i)
                       end if
                       c%SEDtab(2,i) = c%SEDtab(2,i) + rand_gauss(handle) * c%theta_steplen(c%npar+1+i,l) * mh_scale(l)
                       write(*,fmt='(a,f16.8)') ', prop = ',  c%SEDtab(2,i)
                    end if
                 end do
               end if
               negative = negative .or. any(c%SEDtab(2:,:) < 0.d0)
            end if
            ...
            call mpi_bcast(c%SEDtab_buff, size(c%SEDtab), MPI_DOUBLE_PRECISION, &
              & 0, data(1)%info%comm, ierr)
            call mpi_bcast(c%adscale, 1, MPI_DOUBLE_PRECISION, &
              & 0, data(1)%info%comm, ierr)
          ...
          do while (associated(c))                         
             ...
             type is (comm_adMBBtab_comp)
                ...
                if (maxval(c%theta_steplen(c%npar+2:,l)) > 0) then
                   if (cpar%myid_chain .eq. 0) then
                      do i = 1, c%ntab
                         c%SEDtab(2,i) = c%SEDtab_buff(2,i)
                      end do
                   end if
                   call mpi_bcast(c%SEDtab, size(c%SEDtab), MPI_DOUBLE_PRECISION, &
                        & 0, data(1)%info%comm, ierr)
...
!!$         end if
  ...
  end subroutine sample_admbbtab_mh
      ...
      do j = 1, n_tokens
        ...
        if (n == 2) then
          ...
          data(wire_to_ind)%gain_stat(i) = wire_from_ind
            ...
            if (trim(comp_names(1)) == 'gain') then
            ...
            else if (comp_names(2)(1:4) == 'node') then
              ! Get bin index
              call get_tokens(comp_names(2), '@', comp_bands)
              if (cpar%myid == 0 .and. .not. is_numeric(comp_bands(2))) then
                call int2string(i,itext)
                write(*,*) 'MCMC_SAMPLING_GROUP_PARAMS'//itext, '   ', trim(comp_bands(2)), ' should refer to an integer row'
                write(*,*) trim(cpar%mcmc_samp_groups(i))
                stop
              else
                call mpi_barrier(cpar%comm_chain, ierr)
              ...
              do while (associated(c))
                 if (trim(c%label) == trim(comp_names(1))) then
                   !       (beta+T+ntab, n_mcmc_samp_groups)
                   c%theta_steplen(c%npar+1+m,i) = sigma !! changed this to c%npar from 2, +1 for adscale
                 end if
                 c => c%nextComp()
              end do

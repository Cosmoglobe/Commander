...
interface
  ...
  module subroutine initDiffuse(self, cpar, id, id_abs)
    import comm_diffuse_comp, comm_params, i4b
    !
  ...
  module subroutine initSpecindProp(self,cpar, id, id_abs)
    import comm_diffuse_comp, comm_params, i4b
    !
    !  Module Subroutine that initializes the spectral index proposal matrix
    !
    !  Arguments:
    !  ----------
  ...
  module subroutine updateDiffuseMixmat(self, theta, beta, band, df, par)
    import comm_diffuse_comp, comm_map, dp, i4b, map_ptr
    implicit none
  ...
  module function evalDiffuseBand(self, band, amp_in, pix, alm_out, det)
    import comm_diffuse_comp, i4b, dp, lgt
    implicit none
  ...
  module function projectDiffuseBand(self, band, map, alm_in, det)
    import comm_diffuse_comp, i4b, lgt, dp, comm_map
    implicit none
    class(comm_diffuse_comp),                     intent(in)            :: self
    integer(i4b),                                 intent(in)            :: band
  ...
  module subroutine dumpDiffuseToFITS(self, iter, chainfile, output_hdf, postfix, dir)
    import comm_diffuse_comp, i4b, hdf_file, lgt
    !
  ...
  module subroutine initDiffuseHDF(self, cpar, hdffile, hdfpath)
    import comm_diffuse_comp, comm_params, hdf_file
    implicit none
    class(comm_diffuse_comp),  intent(inout) :: self
    type(comm_params),         intent(in)    :: cpar    
  ...
  module subroutine sampleDiffuseSpecInd(self, cpar, handle, id, iter)
    import comm_diffuse_comp, comm_params, planck_rng, i4b
    implicit none
    class(comm_diffuse_comp),                intent(inout)        :: self
    type(comm_params),                       intent(in)           :: cpar
  ...
  module subroutine sampleDiffuseSpecIndSinglePix(self, handle, id, p, iter)
    import comm_diffuse_comp, planck_rng, i4b
    implicit none
    class(comm_diffuse_comp),                intent(inout)        :: self
    type(planck_rng),                        intent(inout)        :: handle
    integer(i4b),                            intent(in)           :: id
  ...
  !  This subroutine is obsolete, it is defined and used in comm_nonlin_mod instead
  module subroutine sampleDiffuseSpecIndFullsky(self, handle, id, p, iter)
    import comm_diffuse_comp, comm_params, planck_rng, i4b
    implicit none
    class(comm_diffuse_comp),                intent(inout)        :: self
    type(planck_rng),                        intent(inout)        :: handle
  ...
  ! This subroutine is obsolete, it is defined and used in comm_nonlin_mod instead
  module subroutine sampleDiffuseSpecIndPixReg(self, cpar, handle, id, p, iter)
    import comm_diffuse_comp, comm_params, planck_rng, i4b
    implicit none
    class(comm_diffuse_comp),                intent(inout)        :: self
    type(comm_params),                       intent(in)           :: cpar
  ...
  module subroutine setup_needlets(info, nside_def, defmask, Z, ndef)
    import comm_mapinfo, comm_map, i4b, dp
    implicit none
  ...
  module subroutine applyMonoDipolePrior(self)
    import comm_diffuse_comp
    implicit none
    class(comm_diffuse_comp), intent(inout)          :: self

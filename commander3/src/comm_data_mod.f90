       ...
       if (cpar%enable_TOD_analysis) then
          ...
          !else if (trim(data(n)%tod_type) == 'QUIET') then
            ! Class initialisation 
             ...
             data(n)%ndet = data(n)%tod%ndet
          else if (trim(data(n)%tod_type) == 'LAT') then
             data(n)%tod => comm_SOLAT_tod(cpar, n, i, data(n)%info, data(n)%tod_type)
             data(n)%ndet = data(n)%tod%ndet
...
!!$             end if
  ...
  subroutine dump_unit_conversion(dir)
    ...
    do i = 1, numband
       q = ind_ds(i)
       write(unit,fmt='(a20,a10,f12.3,f14.3,3e18.5,3e17.5,3e18.5,3e14.5)') trim(data(q)%label), trim(data(q)%bp(0)%p%type), &

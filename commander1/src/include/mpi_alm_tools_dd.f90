...
!
  ...
  subroutine mpi_map2alm_simple_d(comm_in, map, alms, zbounds_in, w8ring)
    implicit none
  ...
  end subroutine mpi_map2alm_simple_d


  subroutine mpi_alm2map_simple_d(comm_in, alms, map)
    implicit none
  ...
  end subroutine mpi_alm2map_simple_d
  ...
  subroutine mpi_map2alm_d(map, alms)
    implicit none
  ...
  end subroutine mpi_map2alm_d


  subroutine mpi_alm2map_d(alms, map)
    implicit none
  ...
  end subroutine mpi_alm2map_d


  subroutine mpi_map2alm_dist_d(map, alms)
    implicit none
  ...
  end subroutine mpi_map2alm_dist_d


  subroutine mpi_alm2map_dist_d(alms, map)
    implicit none
    ...
    call mpi_bcast(local_alms, size(local_alms), MPI_DOUBLE_COMPLEX, root, comm, ierr)
    if (all(abs(local_alms) == 0.d0)) then
       map = 0.d0
       return
    end if
  ...
  end subroutine mpi_alm2map_dist_d


  subroutine mpi_map2alm_dist_slave_d(map)
    implicit none
  ...
  end subroutine mpi_map2alm_dist_slave_d


  subroutine mpi_alm2map_dist_slave_d(map)
    implicit none
  ...
  end subroutine mpi_alm2map_dist_slave_d
  ...
  subroutine distribute_map_d(map)
  ...
  end subroutine distribute_map_d


  subroutine gather_map_d(map)
    ...
    do i=1,numprocs-1
       ...
       if (num_segments(i) == 2) then
          ...
          map(segments(0,0,i):segments(0,1,i),1:nmaps) = &
               & real(buffer(0:seg_size-1,1:nmaps),DP)
          map(segments(1,0,i):segments(1,1,i),1:nmaps) = &
               & real(buffer(seg_size:2*seg_size-1,1:nmaps),DP)
          ...
          map(segments(0,0,i):segments(0,1,i),1:nmaps) = real(buffer(0:seg_size-1,1:nmaps),DP)
    ...
    if (num_segments(0) == 2) then
       ...
       map(segments(0,0,0):segments(0,1,0),1:nmaps) = &
            & real(local_map(0:seg_size-1,1:nmaps),DP)       
       map(segments(1,0,0):segments(1,1,0),1:nmaps) = &         
            & real(local_map(seg_size:2*seg_size-1,1:nmaps),DP)
       
    else
       
       seg_size = segment(0,1)-segment(0,0)+1
       map(segment(0,0):segment(0,1),1:nmaps) = real(local_map(0:seg_size-1,1:nmaps),DP)
       
    end if

  end subroutine gather_map_d

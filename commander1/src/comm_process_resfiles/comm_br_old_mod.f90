module comm_br_old_mod
  implicit none

  ! *********************************************************************
  ! *  comm_br_old_mod -- An F90 module for computing the Blackwell-Rao     *
  ...
  end type comm_br_old_data

  integer(i4b),       parameter       :: N_br = 100
  type(comm_br_old_data), dimension(N_br) :: comm_br_old
  

contains

  ! Initialization routines
  subroutine comm_br_old_initialize_object(sigmafile, clfile, lmin, lmax, firstchain, lastchain, &
       & firstsample, lastsample, thinstep, handle)
    implicit none
    ...
    if (present(handle)) then
       do while (allocated(comm_br_old(id)%sigmas) .and. id < N_br)
          id = id+1
       end do
       handle = id
    end if
    if (id < 1 .or. id > N_br) stop 'Error -- planck_br_old_mod: BR id out of range'

    ! Initialize BR object
    unit = comm_br_old_getlun()
    if (allocated(comm_br_old(id)%sigmas)) deallocate(comm_br_old(id)%sigmas)
    call comm_br_old_read_chain(sigmafile, unit, lmax_chain, numchains, numsamples, comm_br_old(id)%sigmas)
    if (lmax_chain < lmax .or. numchains < lastchain .or. numsamples < lastsample) then
       write(*,*) 'Error -- comm_br_old_mod: Requested parameters inconsistent with sigma file content'
       write(*,*) ''
    ...
    comm_br_old(id)%lmin        = lmin
    comm_br_old(id)%lmax        = lmax
    comm_br_old(id)%firstsample = firstsample
    comm_br_old(id)%lastsample  = lastsample
    comm_br_old(id)%firstchain  = firstchain
    comm_br_old(id)%lastchain   = lastchain
    comm_br_old(id)%thinstep    = thinstep

    do j = comm_br_old(id)%firstsample, comm_br_old(id)%lastsample
       do i = comm_br_old(id)%firstchain, comm_br_old(id)%lastchain
          do l = comm_br_old(id)%lmin, comm_br_old(id)%lmax
             if (comm_br_old(id)%sigmas(l,i,j) <= 0.0) then
                write(*,*) 'Error -- comm_br_old_mod: Negative sigma_l = ', comm_br_old(id)%sigmas(l,i,j)
                write(*,*) '   l      = ', l
                write(*,*) '   chain  = ', i
                write(*,*) '   sample = ', j
                stop
...
99  close(unit)
    ...
    comm_br_old(id)%offset = comm_br_old_compute_offset(cls(2:lmax), handle)

  end subroutine comm_br_old_initialize_object


  subroutine comm_br_old_deallocate_object(handle)
    implicit none
    ...
    do i = id_min, id_max
       if (allocated(comm_br_old(i)%sigmas)) deallocate(comm_br_old(i)%sigmas)
       comm_br_old(i)%lmin        = -1
       comm_br_old(i)%lmax        = -1
       comm_br_old(i)%firstchain  = -1
       comm_br_old(i)%lastchain   = -1
       comm_br_old(i)%firstsample = -1
       comm_br_old(i)%lastsample  = -1
       comm_br_old(i)%thinstep    =  0
    end do

  end subroutine comm_br_old_deallocate_object



  ! Base computation routine
  function comm_br_old_compute_lnL(cls, handle)
    implicit none

    real(dp),     dimension(2:), intent(in)           :: cls
    real(dp)                                          :: comm_br_old_compute_lnL
    ...
    if (.not. allocated(comm_br_old(id)%sigmas)) then
       write(*,*) 'Error -- comm_br_old_mod: Requested BR handle ', id, ' is not allocated'
       stop
    end if

    ! Compute the Blackwell-Rao estimator
    lnL = 0.d0
    do j = comm_br_old(id)%firstsample, comm_br_old(id)%lastsample, comm_br_old(id)%thinstep
       do i = comm_br_old(id)%firstchain, comm_br_old(id)%lastchain
          subtotal = 0.d0
          do l = comm_br_old(id)%lmin, comm_br_old(id)%lmax
             x = comm_br_old(id)%sigmas(l,i,j)/cls(l)
             subtotal = subtotal + &
                  & 0.5d0 * real(2*l+1,dp) * (-x + log(x)) - log(comm_br_old(id)%sigmas(l,i,j))
          end do
          lnL = lnL + exp(subtotal-comm_br_old(id)%offset)
       end do
    end do
    ...
    !comm_br_old_compute_lnL = lnL
    comm_br_old_compute_lnL = lnL - 0.5d0 * (comm_br_old(id)%lmax-comm_br_old(id)%lmin+1)

  end function comm_br_old_compute_lnL
  ...
  function comm_br_old_compute_offset(cls, handle)
    implicit none

    real(dp),     dimension(2:), intent(in)           :: cls
    real(dp)                                          :: comm_br_old_compute_offset
    integer(i4b),                intent(in), optional :: handle
    ...
    if (offset < -1.637e30) then
       write(*,*) 'Error -- comm_br_old_mod: Offset evaluation failed'
       write(*,*) '    handle      = ', id
       write(*,*) '    lmin        = ', comm_br_old(id)%lmin
       write(*,*) '    lmax        = ', comm_br_old(id)%lmax
       write(*,*) '    firstchain  = ', comm_br_old(id)%firstchain
       write(*,*) '    lastchain   = ', comm_br_old(id)%lastchain
       write(*,*) '    firstsample = ', comm_br_old(id)%firstsample
       write(*,*) '    lastsample  = ', comm_br_old(id)%lastsample
       write(*,*) '    thinstep    = ', comm_br_old(id)%thinstep
       write(*,*) ''
       write(*,*) '    cls         = ', cls(comm_br_old(id)%lmin:comm_br_old(id)%lmax)
       write(*,*) '    offset      = ', offset
       stop
    endif

    comm_br_old_compute_offset = offset

  end function comm_br_old_compute_offset

  function comm_br_old_getlun()
    implicit none
    integer(i4b) :: comm_br_old_getlun
    logical(lgt) :: exists, isopen
    comm_br_old_getlun = 9
    do
       comm_br_old_getlun = comm_br_old_getlun+1
       inquire(unit=comm_br_old_getlun,exist=exists)
       if(exists) then
          inquire(unit=comm_br_old_getlun,opened=isopen)
          if(.not. isopen) return
       end if
    end do
  end function comm_br_old_getlun
  
  ! Routine for reading the Gibbs sigma samples 
  subroutine comm_br_old_read_chain(filename, unit, lmax, numchains, numsamples, data)
    implicit none
    ...
    do k = 1, numsamples
       do j = 1, numchains
          do i = 0, lmax
             data(i, j, k) = indata(i, 1, j, k+numargs)
          enddo
       enddo
    enddo
    deallocate(indata)

  end subroutine comm_br_old_read_chain

end module comm_br_old_mod
